...
import {
  DEFAULT_TRADING_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS,
  buildTradingLiveAdapterImportBoundaryFixtures,
} from "./trading-live-adapter-import-boundary-fixtures.mjs";

export const DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_OUT_DIR = "artifacts/trading-secret-handle-boundary-fixtures/latest";
export const DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS = {
  ...DEFAULT_TRADING_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS,
  liveAdapterImportBoundaryFixturesSchemaPath: DEFAULT_TRADING_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.schemaPath,
  ...
  schemaPath: "schemas/trading/trading-secret-handle-boundary-fixtures.schema.json",
};

const SCHEMA_VERSION = "trading-secret-handle-boundary-fixtures.v1";
const CAPABILITY_ID = "trading.secret_handle_boundary_fixtures";
const PHASE_SLOT = "P423";
const PREVIOUS_PHASE_SLOT = "P422";
const NEXT_PHASE_SLOT = "P424";
const READY_STATUS = "ready_for_trading_secret_handle_boundary";
const SOURCE_READY_STATUS = "ready_for_trading_live_adapter_import_boundary";
const COMMAND_NAME = "trading:secret-handle-boundary-fixtures";
const WORKFLOW_ID = "workflow.trading.secret_handle_boundary_fixtures.v1";
const REQUIRED_ROW_KEYS = [
  "p422_live_adapter_import_boundary_ready",
...
export async function runTradingSecretHandleBoundaryFixtures(options = {}) {
  const result = await buildTradingSecretHandleBoundaryFixtures(options);
  if (options.write !== false) await writeTradingSecretHandleBoundaryFixtures(result, result.output_dir);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Trading secret handle boundary fixtures failed with ${result.validation.errors.length} error(s).`);
...
export async function buildTradingSecretHandleBoundaryFixtures(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_OUT_DIR);
  const inputs = normalizeInputs(options);
  const liveAdapterImportBoundary = await buildTradingLiveAdapterImportBoundaryFixtures({
    ...options,
    runAt: generatedAt,
  ...
  const sourceReady = liveAdapterImportBoundary.validation.valid && liveAdapterImportBoundary.summary.trading_live_adapter_import_boundary_fixtures_status === SOURCE_READY_STATUS;
  ...
  const result = {
    ...
    trading_secret_handle_boundary_fixtures_id: `trading-secret-handle-boundary-fixtures.${dateStamp(generatedAt)}`,
  ...
  const schemaErrors = schema.available ? validateAgainstSchema(result, schema.data, {}, "trading_secret_handle_boundary_fixtures") : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  ...
  result.summary.trading_secret_handle_boundary_fixtures_id = result.trading_secret_handle_boundary_fixtures_id;
  return { ...result, markdown: renderMarkdown(result) };
}

export async function writeTradingSecretHandleBoundaryFixtures(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "trading-secret-handle-boundary-fixtures.json"), serializableResult(result));
  await writeJson(path.join(outDir, "secret-handle-boundary-rows.json"), collectionEnvelope("trading-secret-handle-boundary-rows.v1", "secret_handle_boundary_rows", result.secret_handle_boundary_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-handle-boundary-gate-rows.json"), collectionEnvelope("trading-secret-handle-boundary-gate-rows.v1", "secret_handle_boundary_gate_rows", result.secret_handle_boundary_gate_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-handle-boundary-boundary.json"), result.secret_handle_boundary_boundary);
  await writeJson(path.join(outDir, "validation-report.json"), {
    schema_version: "trading-secret-handle-boundary-fixtures-validation-report.v1",
...
export async function runTradingSecretHandleBoundaryFixturesCli(argv = process.argv.slice(2)) {
  ...
  try {
    const result = await runTradingSecretHandleBoundaryFixtures(args);
    console.log(`Trading secret handle boundary fixtures ${args.check ? "validated" : "written"} at ${result.output_dir}`);
    console.log(`Status: ${result.summary.trading_secret_handle_boundary_fixtures_status}`);
...
function buildSecretHandleBoundaryAnchor(liveAdapterImportBoundary, secretScan) {
  return {
    schema_version: "trading-secret-handle-boundary-fixtures-anchor.v1",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_live_adapter_import_boundary_fixtures_id: liveAdapterImportBoundary.trading_live_adapter_import_boundary_fixtures_id,
    source_live_adapter_import_boundary_status: liveAdapterImportBoundary.summary.trading_live_adapter_import_boundary_fixtures_status,
    ...
    source_hash: hashValue({
      id: liveAdapterImportBoundary.trading_live_adapter_import_boundary_fixtures_id,
      status: liveAdapterImportBoundary.summary.trading_live_adapter_import_boundary_fixtures_status,
...
function buildSecretHandleBoundaryRows({ sourceReady, liveAdapterImportBoundary, executionEngine, researchBacktestPaper, desktopCompanionText, hermesConfigText, secretsScanText, secretScan }) {
  ...
  const rowInputs = [
    secretBoundaryRowInput("p422_live_adapter_import_boundary_ready", "live_adapter_import_boundary_fixtures", "summary", [
      condition("source.trading_live_adapter_import_boundary_fixtures_status", liveAdapterImportBoundary.summary.trading_live_adapter_import_boundary_fixtures_status, SOURCE_READY_STATUS),
...
function buildSecretHandleBoundaryRow(input, sourceReady, index) {
  ...
  const row = {
    schema_version: "trading-secret-handle-boundary-row.v1",
    secret_handle_boundary_row_id: `trading-secret-handle-boundary.row.${input.rowKey}`,
    phase_slot: PHASE_SLOT,
    ...
    source_live_adapter_import_boundary_ready: sourceReady,
...
function buildSecretHandleBoundary({ generatedAt, writeRequested, liveAdapterImportBoundary, rows, secretScan }) {
  const sourceReady = liveAdapterImportBoundary.validation.valid && liveAdapterImportBoundary.summary.trading_live_adapter_import_boundary_fixtures_status === SOURCE_READY_STATUS;
  return {
    schema_version: "trading-secret-handle-boundary-fixtures-boundary.v1",
    generated_at: generatedAt,
...
function buildSecretHandleBoundaryGateRows({ liveAdapterImportBoundary, packageJson, platformOpsLedger, sources, rows, boundary }) {
  ...
  const gateInputs = [
    ["p422_live_adapter_import_boundary_ready", "P422 live adapter import boundary source is ready.", liveAdapterImportBoundary.validation.valid && liveAdapterImportBoundary.summary.trading_live_adapter_import_boundary_fixtures_status === SOURCE_READY_STATUS],
    ["platform_package_script_registered", "package.json registers the P423 trading secret handle boundary fixtures command.", typeof scripts[COMMAND_NAME] === "string" && scripts[COMMAND_NAME].length > 0],
    ["platform_validation_chain_registered", "Validation chain includes the P423 trading secret handle boundary fixtures command.", validateScript.includes(`npm run ${COMMAND_NAME} -- --check`)],
    ["p423_ledger_acceptance_declared", "P423 acceptance row is declared in the platform operations ledger.", ledgerText.includes("P423: `trading:secret-handle-boundary-fixtures`")],
    ...
    ["no_secret_or_trading_mutation", "P423 performs no credential lookup, live execution, trading writes, artifact mutation, release, git, or protected action.", !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.command_execution_performed && !boundary.artifact_write_performed && !boundary.release_published && !boundary.git_operation_performed && !boundary.protected_action_executed],
  ];
...
function gateRow(rowKey, description, passed) {
  return {
    schema_version: "trading-secret-handle-boundary-gate-row.v1",
    secret_handle_boundary_gate_row_id: `trading-secret-handle-boundary-fixtures.gate.${rowKey}`,
    phase_slot: PHASE_SLOT,
    row_key: rowKey,
    description,
...
function buildValidationItems({ liveAdapterImportBoundary, packageJson, platformOpsLedger, sources, rows, gateRows, boundary }) {
  return [
    validationItem("source.live_adapter_import_boundary", "p422_live_adapter_import_boundary_ready", liveAdapterImportBoundary.validation.valid && liveAdapterImportBoundary.summary.trading_live_adapter_import_boundary_fixtures_status === SOURCE_READY_STATUS, "P422 live adapter import boundary fixtures must be ready."),
    validationItem("source.package_json", "package_json_available", packageJson.available, "package.json is readable for P423 secret handle boundary fixtures."),
    ...
    validationItem("secret_handle_boundary_gate_rows", "secret_handle_boundary_gates_ready", gateRows.length >= 10 && gateRows.every((row) => row.gate_status === "ready" && !row.protected_action_executed_by_gate), "P423 secret handle boundary gates are ready."),
...
function renderMarkdown(result) {
  const lines = [
    ...
    "",
    `Status: ${result.summary.trading_secret_handle_boundary_fixtures_status}`,
    `Phase: ${result.summary.phase_slot}`,
    `Source live adapter import boundary: ${result.summary.source_live_adapter_import_boundary_status}`,
...
function parseArgs(argv) {
  const parsed = { outDir: DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_OUT_DIR };
...
function printHelp() {
  console.log(`Usage: node scripts/trading-secret-handle-boundary-fixtures.mjs [options]

Options:
  --out-dir <folder>                         Output directory. Default: ${DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_OUT_DIR}
  ...
  --live-adapter-import-boundary-fixtures-schema <path>
                                             P422 live adapter import boundary fixtures schema path.
...
function normalizeInputs(options = {}) {
  return {
    package_path: path.resolve(options.packagePath ?? DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.packagePath),
    platform_ops_ledger_path: path.resolve(options.platformOpsLedgerPath ?? DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.platformOpsLedgerPath),
    live_adapter_import_boundary_fixtures_schema_path: path.resolve(options.liveAdapterImportBoundaryFixturesSchemaPath ?? DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.liveAdapterImportBoundaryFixturesSchemaPath),
    execution_engine_path: path.resolve(options.executionEnginePath ?? DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.executionEnginePath),
    research_backtest_paper_path: path.resolve(options.researchBacktestPaperPath ?? DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.researchBacktestPaperPath),
    desktop_companion_integration_path: path.resolve(options.desktopCompanionIntegrationPath ?? DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.desktopCompanionIntegrationPath),
    hermes_config_example_path: path.resolve(options.hermesConfigExamplePath ?? DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.hermesConfigExamplePath),
    secrets_scan_gate_doc_path: path.resolve(options.secretsScanGateDocPath ?? DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.secretsScanGateDocPath),
    schema_path: path.resolve(options.schemaPath ?? DEFAULT_TRADING_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.schemaPath),
  };
}
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    for (const pattern of RAW_SECRET_PATTERNS) {
      ...
      if (pattern.regex.test(line)) {
        findings.push({
          schema_version: "trading-secret-handle-boundary-raw-secret-finding.v1",
          source_id: sourceId,
