...
import {
  ALL_FALSE_FLAGS as P41600_FALSE_FLAGS,
  buildWorkOsStaticBundleReviewUiImplementationReviewLocalUiBindingSmoke,
} from "./work-os-static-bundle-review-ui-implementation-review-local-ui-binding-smoke.mjs";

export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_HANDOFF_OUT_DIR = "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-handoff/latest";
export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_HANDOFF_INPUTS = {
  schemaPath: "schemas/work-os-static-bundle-review-ui-implementation-review-static-shell-handoff.schema.json",
  packagePath: "package.json",
  roadmapDocPath: "docs/hermes-roadmap-p41601-p42000.md",
  architectureDocPath: "docs/architecture.md",
  sourceWorkOsStaticBundleReviewUiImplementationReviewLocalUiBindingSmokePath: "artifacts/work-os-static-bundle-review-ui-implementation-review-local-ui-binding-smoke/latest/work-os-static-bundle-review-ui-implementation-review-local-ui-binding-smoke.json",
};

const COMMAND_NAME = "platform:work-os-static-bundle-review-ui-implementation-review-static-shell-handoff";
const SCHEMA_VERSION = "work-os-static-bundle-review-ui-implementation-review-static-shell-handoff.v1";
const CAPABILITY_ID = "platform.work_os_static_bundle_review_ui_implementation_review_static_shell_handoff";
const PROGRAM_RANGE = "P41601-P42000";
const SOURCE_PROGRAM_RANGE = "P41201-P41600";
const READY_STATUS = "ready_for_work_os_static_bundle_review_ui_implementation_review_static_shell_handoff";
const BLOCK_PENDING_STATUS = "valid_block_work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_pending";
const BLOCKED_STATUS = "blocked_work_os_static_bundle_review_ui_implementation_review_static_shell_handoff";
...
export const ALL_FALSE_FLAGS = [...new Set([...STATIC_SHELL_HANDOFF_FALSE_FLAGS, ...P41600_FALSE_FLAGS])];

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoff(options = {}) {
  const result = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoff(options);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Work OS Static Bundle Review UI Implementation Review Static Shell Handoff failed with ${result.validation.errors.length} error(s).`);
    error.validation = result.validation;
    throw error;
  }
  if (!options.check && options.write !== false) await writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoff(result, result.output_dir);
  return result;
}

export async function buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoff(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_HANDOFF_OUT_DIR);
  const inputs = normalizeInputs(options);
  const schema = await readJsonSource(inputs.schema_path);
  const packageJson = await readJsonSource(inputs.package_path);
  const roadmapDoc = await readTextSource(inputs.roadmap_doc_path);
  const architectureDoc = await readTextSource(inputs.architecture_doc_path);
  const source = Object.prototype.hasOwnProperty.call(options, "workOsStaticBundleReviewUiImplementationReviewLocalUiBindingSmoke")
    ? normalizeInlineJsonSource("inline.work_os_static_bundle_review_ui_implementation_review_local_ui_binding_smoke", options.workOsStaticBundleReviewUiImplementationReviewLocalUiBindingSmoke)
    : await readJsonOrBuildP41600(inputs.source_work_os_static_bundle_review_ui_implementation_review_local_ui_binding_smoke_path, generatedAt);
  ...
  const result = {
    ...
    inputs,
    source_refs: {
      work_os_static_bundle_review_ui_implementation_review_local_ui_binding_smoke_path: source.path,
      source_commit_ref: commitRef || null,
    },
    source_work_os_static_bundle_review_ui_implementation_review_local_ui_binding_smoke_summary: source.data?.summary ?? null,
    work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_contract: buildContract(generatedAt),
    work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_phase_rows: phaseRows,
    p41600_source_binding_rows: sourceRows,
    ...
    p42000_clean_checkpoint_rows: checkpointRows,
    work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_boundary: boundary,
    work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_validation_items: validationItems,
    validation: preliminaryValidation,
    summary: buildSummary({ boundary, validation: preliminaryValidation }),
  };

  const schemaErrors = schema.available
    ? validateAgainstSchema(result, schema.data, {}, "work_os_static_bundle_review_ui_implementation_review_static_shell_handoff")
    : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  const schemaValidationItems = schemaErrors.map((error, index) => validationItem(`schema.${index}`, "schema_validation", false, error.message));
  result.work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_validation_items = [...validationItems, ...schemaValidationItems];
  result.validation = summarizeValidation(result.work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_validation_items);
  result.summary = buildSummary({ boundary, validation: result.validation });
  return { ...result, markdown: renderMarkdown(result), html: renderHtml(result) };
}

export async function writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoff(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-handoff.json"), serializableResult(result));
  await writeJson(path.join(outDir, "p41600-source-binding-rows.json"), collectionEnvelope("p41600-source-binding-rows.v1", "p41600_source_binding_rows", result.p41600_source_binding_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "p42000-clean-checkpoint-rows.json"), collectionEnvelope("p42000-clean-checkpoint-rows.v1", "p42000_clean_checkpoint_rows", result.p42000_clean_checkpoint_rows, result.generated_at));
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-handoff-boundary.json"), result.work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_boundary);
  await writeFile(path.join(outDir, "summary.md"), result.markdown, "utf8");
  await writeFile(path.join(outDir, "index.html"), result.html, "utf8");
}

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoffCli(argv = process.argv.slice(2)) {
  const args = parseArgs(argv);
  if (args.help) {
    printHelp();
    return null;
  }
  const result = await runWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoff(args);
  console.log(`Work OS Static Bundle Review UI Implementation Review Static Shell Handoff ${args.check ? "validated" : "written"} at ${result.output_dir}`);
  console.log(`Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_status}`);
  console.log(`Program: ${result.program_range}`);
  console.log(`P41600 ready for P41601 handoff: ${result.summary.source_p41600_ready_for_p41601_handoff}`);
  ...
  console.log(`Ready for P42001 handoff: ${result.summary.ready_for_p42001_handoff}`);
...
function buildSourceState(source) {
  ...
  const boundary = source.data?.work_os_static_bundle_review_ui_implementation_review_local_ui_binding_smoke_boundary ?? {};
  return {
    available: source.available === true,
    programRangeOk: source.data?.program_range === SOURCE_PROGRAM_RANGE,
    validationValid: source.data?.validation?.valid === true,
    sourceReady: summary.ready_for_p41601_handoff === true,
    status: summary.work_os_static_bundle_review_ui_implementation_review_local_ui_binding_smoke_status ?? "missing",
    p41600ContractReady: boundary.p41600_contract_ready === true,
    ...
    boundaryClosed: P41600_FALSE_FLAGS.every((flag) => boundary[flag] === false),
  };
}
...
function buildPhaseRows(roadmapText, generatedAt) {
  return PHASE_SPECS.map(([range, label, outputRef]) => row({
    ...
    evidence_ref: "docs/hermes-roadmap-p41601-p42000.md",
...
function buildSourceRows({ source, sourceState, commitRef, generatedAt }) {
  return [
    ["artifact_available", "P41600 implementation review local UI binding smoke source is available", sourceState.available],
    ["program_range", "P41600 source program range is P41201-P41600", sourceState.programRangeOk],
    ["validation_valid", "P41600 source validation is valid", sourceState.validationValid],
    ["p41601_handoff_open", "P41600 source opened P41601 handoff", sourceState.sourceReady],
    ["p41600_contract_ready", "P41600 source contract is ready", sourceState.p41600ContractReady],
    ["binding_smoke_visible", "P41600 local UI binding smoke rows are visible", sourceState.bindingSmokeVisible],
    ["static_shell_binding_visible", "P41600 static shell binding map rows are visible", sourceState.staticShellBindingVisible],
    ["fixture_fetch_visible", "P41600 GET-only fixture fetch rows are visible", sourceState.fixtureFetchVisible],
    ["blocker_no_action_visible", "P41600 blocker/no-action rows are visible", sourceState.blockerNoActionVisible],
    ["no_server_no_browser_closed", "P41600 no-server/no-browser boundary is closed", sourceState.noServerNoBrowserClosed],
    ["commit_ref_present", "Current commit ref is present for static shell handoff", Boolean(commitRef)],
    ["source_blocker_visible", "P41600 source blocker is visible when static shell handoff is closed", sourceState.available],
  ].map(([id, label, observed]) => row({
    row_id: `source_binding.${id}`,
    category: "p41600_source_binding",
    label,
    observed,
...
function buildNoServeNoRenderAuthorityRows(generatedAt) {
  return ALL_FALSE_FLAGS.map((flag) => row({
    ...
    observed: true,
    evidence_ref: "work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_boundary",
    boundary_flag: flag,
    allowed_now: false,
    generated_at: generatedAt,
  }));
...
function buildCheckpointRows(context) {
  ...
  return [
    ["source_ready", "P41600 source is ready for P41601", context.sourceState.sourceReady],
    ...
    ["p42001_handoff_gate", "P42001 handoff opens only when static shell handoff conditions pass", handoffReady],
    ["p42001_handoff_blocker_visible", "P42001 handoff blocker is visible when the gate is closed", true],
  ].map(([id, label, observed]) => row({
    row_id: `p42000_checkpoint.${id}`,
    category: "p42000_clean_checkpoint",
    label,
    observed,
    evidence_ref: "p42000_clean_checkpoint_rows",
    generated_at: context.generatedAt,
  }));
}

function buildBoundary(context) {
  const p42000ContractReady = allPass(context.phaseRows)
    ...
    && allPass(context.copyRows)
    && allPass(context.authorityRows)
    && visibleOrPassed(context.checkpointRows, "p42000_checkpoint.p42001_handoff_blocker_visible");
  const handoffReady = context.sourceState.sourceReady
    && context.sourceState.validationValid
    && context.sourceState.boundaryClosed
    && allPass(context.handoffRows)
    ...
    && allPass(context.authorityRows);
  return {
    p42000_contract_ready: p42000ContractReady,
    ready_for_p42001_handoff: handoffReady,
    source_p41600_ready_for_p41601_handoff: context.sourceState.sourceReady,
...
function buildValidationItems(context) {
  return [
    ...
    validationItem("roadmap.phase_coverage", "roadmap", allPass(context.phaseRows), "P41601-P42000 phase rows are incomplete"),
    validationItem("architecture.reference", "architecture", context.architectureDoc.text.includes("P41601-P42000 Work OS Static Bundle Review UI Implementation Review Static Shell Handoff"), "Architecture doc missing P41601-P42000 reference"),
    validationItem("source.visible", "source", visibleOrPassed(context.sourceRows, "source_binding.source_blocker_visible"), "P41600 source state is not visible"),
    ...
    validationItem("checkpoint.visible", "checkpoint", visibleOrPassed(context.checkpointRows, "p42000_checkpoint.p42001_handoff_blocker_visible"), "P42000 checkpoint blocker is not visible"),
  ];
}

function buildContract(generatedAt) {
  return {
    contract_id: "work_os_static_bundle_review_ui_implementation_review_static_shell_handoff.contract.v1",
    generated_at: generatedAt,
    source_p41600_required_or_rebuilt: true,
    ...
    p42001_handoff_is_not_server_route_dom_browser_action_write_approval_closeout_or_enterprise_trust: true,
    protected_authority: "closed",
  };
}

function buildSummary({ boundary, validation }) {
  const status = validation.valid && boundary.ready_for_p42001_handoff
    ? READY_STATUS
    : validation.valid && boundary.p42000_contract_ready
      ? BLOCK_PENDING_STATUS
      : BLOCKED_STATUS;
  return {
    work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_status: status,
    source_p41600_ready_for_p41601_handoff: boundary.source_p41600_ready_for_p41601_handoff,
    ...
    ready_for_p42001_handoff: validation.valid && boundary.ready_for_p42001_handoff,
...
function renderMarkdown(result) {
  return [
    "# Work OS Static Bundle Review UI Implementation Review Static Shell Handoff",
    "",
    `Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_status}`,
    `Program: ${result.program_range}`,
    `P41600 ready for P41601 handoff: ${result.summary.source_p41600_ready_for_p41601_handoff}`,
    ...
    `Ready for P42001 handoff: ${result.summary.ready_for_p42001_handoff}`,
...
<head>
  ...
  <title>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Handoff</title>
  <style>
...
<body>
  <main>
    <h1>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Handoff</h1>
...
async function readJsonOrBuildP41600(filePath, generatedAt) {
  const source = await readJsonSource(filePath);
  if (source.available) return source;
  const built = await buildWorkOsStaticBundleReviewUiImplementationReviewLocalUiBindingSmoke({ runAt: generatedAt, write: false });
  return normalizeInlineJsonSource("built.work_os_static_bundle_review_ui_implementation_review_local_ui_binding_smoke", built);
}
...
function normalizeInputs(options) {
  const defaults = DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_HANDOFF_INPUTS;
  ...
  return {
    ...
    source_work_os_static_bundle_review_ui_implementation_review_local_ui_binding_smoke_path: path.resolve(repoRoot, options.sourceWorkOsStaticBundleReviewUiImplementationReviewLocalUiBindingSmokePath ?? defaults.sourceWorkOsStaticBundleReviewUiImplementationReviewLocalUiBindingSmokePath),
  };
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--check") {
      ...
      args.sourceWorkOsStaticBundleReviewUiImplementationReviewLocalUiBindingSmokePath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/work-os-static-bundle-review-ui-implementation-review-static-shell-handoff.mjs [--check] [--out-dir DIR] [--source FILE] [--commit-ref SHA]

Validates or writes the Hermes P41601-P42000 Work OS Static Bundle Review UI Implementation Review Static Shell Handoff.
`);
}
