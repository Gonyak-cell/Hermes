...
import {
  DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS,
  buildTradingSecretScanRemediationReceiptChainSecretHandleBoundaryFixtures,
} from "./trading-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures.mjs";

export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_LEAKAGE_REGRESSION_FIXTURES_OUT_DIR = "artifacts/trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures/latest";
export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_LEAKAGE_REGRESSION_FIXTURES_INPUTS = {
  ...DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS,
  secretHandleBoundaryFixturesSchemaPath: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.schemaPath,
  schemaPath: "schemas/trading/trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures.schema.json",
};

const SCHEMA_VERSION = "trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures.v1";
const CAPABILITY_ID = "trading.secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures";
const PHASE_SLOT = "P450";
const PREVIOUS_PHASE_SLOT = "P449";
const NEXT_PHASE_SLOT = "P451";
const READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_secret_leakage_regression";
const SOURCE_READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_secret_handle_boundary";
const COMMAND_NAME = "trading:secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures";
const REQUIRED_ROW_KEYS = [
  "p449_secret_scan_remediation_receipt_chain_secret_handle_boundary_ready",
...
export async function runTradingSecretScanRemediationReceiptChainSecretLeakageRegressionFixtures(options = {}) {
  const result = await buildTradingSecretScanRemediationReceiptChainSecretLeakageRegressionFixtures(options);
  if (options.write !== false) await writeTradingSecretScanRemediationReceiptChainSecretLeakageRegressionFixtures(result, result.output_dir);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Trading secret scan remediation receipt chain secret leakage regression fixtures failed with ${result.validation.errors.length} error(s).`);
...
export async function buildTradingSecretScanRemediationReceiptChainSecretLeakageRegressionFixtures(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_LEAKAGE_REGRESSION_FIXTURES_OUT_DIR);
  const inputs = normalizeInputs(options);
  const { fixtureOverrides: _fixtureOverrides, ...sourceOptions } = options;
  const secretHandleBoundary = await buildTradingSecretScanRemediationReceiptChainSecretHandleBoundaryFixtures({
    ...sourceOptions,
    runAt: generatedAt,
  ...
  const sourceReady = secretHandleBoundary.validation.valid && secretHandleBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_status === SOURCE_READY_STATUS;
  ...
  const result = {
    ...
    trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_id: `trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures.${dateStamp(generatedAt)}`,
  ...
  const schemaErrors = schema.available ? validateAgainstSchema(result, schema.data, {}, "trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures") : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  ...
  result.summary.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_id = result.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_id;
  return { ...result, markdown: renderMarkdown(result) };
}

export async function writeTradingSecretScanRemediationReceiptChainSecretLeakageRegressionFixtures(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures.json"), serializableResult(result));
  await writeJson(path.join(outDir, "secret-leakage-regression-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-rows.v1", "secret_leakage_regression_rows", result.secret_leakage_regression_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-leakage-regression-gate-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-gate-rows.v1", "secret_leakage_regression_gate_rows", result.secret_leakage_regression_gate_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "validation-report.json"), {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures-validation-report.v1",
...
export async function runTradingSecretScanRemediationReceiptChainSecretLeakageRegressionFixturesCli(argv = process.argv.slice(2)) {
  ...
  try {
    const result = await runTradingSecretScanRemediationReceiptChainSecretLeakageRegressionFixtures(args);
    console.log(`Trading secret scan remediation receipt chain secret leakage regression fixtures ${args.check ? "validated" : "written"} at ${result.output_dir}`);
    console.log(`Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status}`);
...
function buildSecretLeakageRegressionAnchor(secretHandleBoundary, cases) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures-anchor.v1",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_id: secretHandleBoundary.trading_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_id,
    source_secret_scan_remediation_receipt_chain_secret_handle_boundary_status: secretHandleBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_status,
    ...
    source_hash: hashValue({
      id: secretHandleBoundary.trading_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_id,
      status: secretHandleBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_status,
...
function buildSourceReadyRow(sourceReady, secretHandleBoundary, index) {
  const observedConditions = [
    condition("source.trading_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_status", secretHandleBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_status, SOURCE_READY_STATUS),
  ...
  return withOrdinalAndHash(baseRow({
    rowKey: "p449_secret_scan_remediation_receipt_chain_secret_handle_boundary_ready",
    sourceReady,
    observedConditions,
    unsafeConditions,
...
function baseRow({ rowKey, sourceReady, observedConditions, unsafeConditions, expectedOutcome, detectedOutcome, regressionPassed, secretPatternKind, rawPatternBlocked, externalHandleAllowed }) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-row.v1",
    secret_leakage_regression_row_id: `trading-secret-scan-remediation-receipt-chain-secret-leakage-regression.row.${rowKey}`,
    phase_slot: PHASE_SLOT,
    row_key: rowKey,
    ...
    source_secret_scan_remediation_receipt_chain_secret_handle_boundary_ready: sourceReady,
...
function buildSecretLeakageRegressionBoundary({ generatedAt, writeRequested, secretHandleBoundary, rows, cases }) {
  const sourceReady = secretHandleBoundary.validation.valid && secretHandleBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_status === SOURCE_READY_STATUS;
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures-boundary.v1",
    generated_at: generatedAt,
...
function buildSecretLeakageRegressionGateRows({ secretHandleBoundary, packageJson, platformOpsLedger, rows, boundary }) {
  ...
  const gateInputs = [
    ["p449_secret_scan_remediation_receipt_chain_secret_handle_boundary_ready", "P449 secret scan remediation receipt chain secret handle boundary source is ready.", secretHandleBoundary.validation.valid && secretHandleBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_status === SOURCE_READY_STATUS],
    ["platform_package_script_registered", "package.json registers the P450 trading secret leakage regression fixtures command.", typeof scripts[COMMAND_NAME] === "string" && scripts[COMMAND_NAME].length > 0],
    ["platform_validation_chain_registered", "Validation chain includes the P450 trading secret leakage regression fixtures command.", validateScript.includes(`npm run ${COMMAND_NAME} -- --check`)],
    ["p450_ledger_acceptance_declared", "P450 acceptance row is declared in the platform operations ledger.", ledgerText.includes("P450: `trading:secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures`")],
...
function gateRow(rowKey, description, passed) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-gate-row.v1",
    secret_leakage_regression_gate_row_id: `trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures.gate.${rowKey}`,
    phase_slot: PHASE_SLOT,
    row_key: rowKey,
    description,
...
function buildValidationItems({ secretHandleBoundary, packageJson, platformOpsLedger, rows, gateRows, boundary }) {
  return [
    validationItem("source.secret_scan_remediation_receipt_chain_secret_handle_boundary", "p449_secret_scan_remediation_receipt_chain_secret_handle_boundary_ready", secretHandleBoundary.validation.valid && secretHandleBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_status === SOURCE_READY_STATUS, "P449 secret scan remediation receipt chain secret handle boundary fixtures must be ready."),
    validationItem("source.package_json", "package_json_available", packageJson.available, "package.json is readable for P450 secret leakage regression fixtures."),
    ...
    validationItem("secret_leakage_regression_gate_rows", "secret_leakage_gates_ready", gateRows.length >= 10 && gateRows.every((row) => row.gate_status === "ready" && !row.protected_action_executed_by_gate), "P450 secret leakage regression gates are ready."),
    ...
    validationItem("boundary.no_mutation", "no_secret_or_trading_mutation", boundary.no_secret_or_trading_mutation && !boundary.secret_scan_remediation_action_allowed && !boundary.credential_lookup_allowed && !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.command_execution_performed && !boundary.package_command_execution_performed && !boundary.release_check_execution_performed && !boundary.artifact_read_performed && !boundary.artifact_write_performed && !boundary.release_published && !boundary.git_operation_performed && !boundary.protected_action_executed, "P450 performs no secret scan remediation action, trading, artifact, release, git, or protected mutation."),
  ];
}

function buildSummary({ secretHandleBoundary, rows, gateRows, boundary, validation }) {
  return {
    trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status: validation.valid ? READY_STATUS : "blocked",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_secret_handle_boundary_status: secretHandleBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_status,
    source_secret_scan_remediation_receipt_chain_secret_handle_boundary_ready: boundary.source_secret_scan_remediation_receipt_chain_secret_handle_boundary_ready,
    required_row_count: REQUIRED_ROW_KEYS.length,
    secret_leakage_regression_row_count: rows.length,
...
function renderMarkdown(result) {
  const lines = [
    "# Trading Secret Scan Remediation Receipt Chain Secret Leakage Regression Fixtures",
    "",
    `Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status}`,
    `Phase: ${result.summary.phase_slot}`,
    `Source secret scan remediation receipt chain secret handle boundary: ${result.summary.source_secret_scan_remediation_receipt_chain_secret_handle_boundary_status}`,
...
function parseArgs(argv) {
  const parsed = { outDir: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_LEAKAGE_REGRESSION_FIXTURES_OUT_DIR };
...
function printHelp() {
  console.log(`Usage: node scripts/trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures.mjs [options]

Options:
  --out-dir <folder>                         Output directory. Default: ${DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_LEAKAGE_REGRESSION_FIXTURES_OUT_DIR}
  --run-at <iso>                             Deterministic generated_at timestamp.
  --package <path>                           package.json path.
  --platform-ops-ledger <path>               P341-P500 platform operations ledger path.
  --secret-handle-boundary-fixtures-schema <path>
                                             P449 secret scan remediation receipt chain secret handle boundary fixtures schema path.
...
function normalizeInputs(options = {}) {
  return {
    package_path: path.resolve(options.packagePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_LEAKAGE_REGRESSION_FIXTURES_INPUTS.packagePath),
    platform_ops_ledger_path: path.resolve(options.platformOpsLedgerPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_LEAKAGE_REGRESSION_FIXTURES_INPUTS.platformOpsLedgerPath),
    secret_handle_boundary_fixtures_schema_path: path.resolve(options.secretHandleBoundaryFixturesSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_LEAKAGE_REGRESSION_FIXTURES_INPUTS.secretHandleBoundaryFixturesSchemaPath),
    schema_path: path.resolve(options.schemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_LEAKAGE_REGRESSION_FIXTURES_INPUTS.schemaPath),
  };
}
