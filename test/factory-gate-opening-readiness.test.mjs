...
test("Factory Gate Opening Readiness exposes G-series rows without opening authority", async () => {
  ...
  assert.equal(result.summary.source_literal_gate_open_commit_count, 0);
  assert.equal(result.summary.owner_gate_opening_receipt_count, 0);
  assert.equal(result.summary.first_use_audit_count, 0);
  ...
  assert.equal(g1a.gate_status, "ready_for_owner_gate_receipt_and_source_literal_commit");
  assert.equal(g1a.gate_open_now, false);
  assert.deepEqual(g1a.blocked_reason_ids, [
    "gate_opening_receipt_missing",
    "source_literal_gate_open_commit_missing",
    "post_open_first_use_audit_missing",
  ]);
  assert.equal(g1b.gate_status, "blocked_previous_gate_closed");
