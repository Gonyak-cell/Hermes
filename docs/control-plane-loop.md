...
Phase 142부터 loop는 `npm run resource:citations`를 실행해 issue graph store를 review-pending output paragraph, citation object, paragraph-source binding, citation review queue로 갱신한다.
...
Phase 144부터 loop는 `npm run resource:evidence-coverage`도 실행해 각 lineage path의 claim/date/party/amount/legal-basis coverage score와 review-pending/not-client-facing gate를 갱신한다.

Phase 145부터 loop는 `npm run resource:evidence-flags`도 실행해 coverage score별 extraction, human confirmation, privilege, redaction, external-transfer flag를 분리하고 review-pending/not-client-facing gate를 갱신한다.
