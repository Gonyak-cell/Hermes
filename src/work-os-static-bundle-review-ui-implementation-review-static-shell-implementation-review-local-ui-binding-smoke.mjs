...
import {
  ALL_FALSE_FLAGS as P46000_FALSE_FLAGS,
  buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewUiHandoffBundle,
} from "./work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-ui-handoff-bundle.mjs";

export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_REVIEW_LOCAL_UI_BINDING_SMOKE_OUT_DIR = "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-local-ui-binding-smoke/latest";
export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_REVIEW_LOCAL_UI_BINDING_SMOKE_INPUTS = {
  schemaPath: "schemas/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-local-ui-binding-smoke.schema.json",
  packagePath: "package.json",
  roadmapDocPath: "docs/hermes-roadmap-p46001-p46400.md",
  architectureDocPath: "docs/architecture.md",
  sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewUiHandoffBundlePath: "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-ui-handoff-bundle/latest/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-ui-handoff-bundle.json",
};

const COMMAND_NAME = "platform:work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-local-ui-binding-smoke";
const SCHEMA_VERSION = "work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-local-ui-binding-smoke.v1";
const CAPABILITY_ID = "platform.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke";
const PROGRAM_RANGE = "P46001-P46400";
const SOURCE_PROGRAM_RANGE = "P45601-P46000";
const READY_STATUS = "ready_for_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke";
const BLOCK_PENDING_STATUS = "valid_block_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke_pending";
const BLOCKED_STATUS = "blocked_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke";
...
export const ALL_FALSE_FLAGS = [...new Set([...LOCAL_UI_BINDING_FALSE_FLAGS, ...P46000_FALSE_FLAGS])];

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewLocalUiBindingSmoke(options = {}) {
  const result = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewLocalUiBindingSmoke(options);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Local UI Binding Smoke failed with ${result.validation.errors.length} error(s).`);
    error.validation = result.validation;
    throw error;
  }
  if (!options.check && options.write !== false) await writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewLocalUiBindingSmoke(result, result.output_dir);
  return result;
}

export async function buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewLocalUiBindingSmoke(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_REVIEW_LOCAL_UI_BINDING_SMOKE_OUT_DIR);
  const inputs = normalizeInputs(options);
  const schema = await readJsonSource(inputs.schema_path);
  const packageJson = await readJsonSource(inputs.package_path);
  const roadmapDoc = await readTextSource(inputs.roadmap_doc_path);
  const architectureDoc = await readTextSource(inputs.architecture_doc_path);
  const source = Object.prototype.hasOwnProperty.call(options, "workOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewUiHandoffBundle")
    ? normalizeInlineJsonSource("inline.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_ui_handoff_bundle", options.workOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewUiHandoffBundle)
    : await readJsonOrBuildP46000(inputs.source_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_ui_handoff_bundle_path, generatedAt);
  ...
  const result = {
    ...
    inputs,
    source_refs: {
      work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_ui_handoff_bundle_path: source.path,
      source_commit_ref: commitRef || null,
    },
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_ui_handoff_bundle_summary: source.data?.summary ?? null,
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke_contract: buildContract(generatedAt),
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke_phase_rows: phaseRows,
    p46000_source_binding_rows: sourceRows,
    ...
    p46400_clean_checkpoint_rows: checkpointRows,
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke_boundary: boundary,
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke_validation_items: validationItems,
    validation: preliminaryValidation,
    summary: buildSummary({ boundary, validation: preliminaryValidation }),
  };

  const schemaErrors = schema.available
    ? validateAgainstSchema(result, schema.data, {}, "work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke")
    : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  const schemaValidationItems = schemaErrors.map((error, index) => validationItem(`schema.${index}`, "schema_validation", false, error.message));
  result.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke_validation_items = [...validationItems, ...schemaValidationItems];
  result.validation = summarizeValidation(result.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke_validation_items);
  result.summary = buildSummary({ boundary, validation: result.validation });
  return { ...result, markdown: renderMarkdown(result), html: renderHtml(result) };
}

export async function writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewLocalUiBindingSmoke(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-local-ui-binding-smoke.json"), serializableResult(result));
  await writeJson(path.join(outDir, "p46000-source-binding-rows.json"), collectionEnvelope("p46000-source-binding-rows.v1", "p46000_source_binding_rows", result.p46000_source_binding_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "p46400-clean-checkpoint-rows.json"), collectionEnvelope("p46400-clean-checkpoint-rows.v1", "p46400_clean_checkpoint_rows", result.p46400_clean_checkpoint_rows, result.generated_at));
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-local-ui-binding-smoke-boundary.json"), result.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke_boundary);
  await writeFile(path.join(outDir, "summary.md"), result.markdown, "utf8");
  await writeFile(path.join(outDir, "index.html"), result.html, "utf8");
}

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewLocalUiBindingSmokeCli(argv = process.argv.slice(2)) {
  const args = parseArgs(argv);
  if (args.help) {
    printHelp();
    return null;
  }
  const result = await runWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewLocalUiBindingSmoke(args);
  console.log(`Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Local UI Binding Smoke ${args.check ? "validated" : "written"} at ${result.output_dir}`);
  console.log(`Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke_status}`);
  console.log(`Program: ${result.program_range}`);
  console.log(`P46000 ready for P46001 handoff: ${result.summary.source_p46000_ready_for_p46001_handoff}`);
  ...
  console.log(`Ready for P46401 handoff: ${result.summary.ready_for_p46401_handoff}`);
...
function buildSourceState(source) {
  ...
  const boundary = source.data?.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_ui_handoff_boundary ?? {};
  return {
    available: source.available === true,
    programRangeOk: source.data?.program_range === SOURCE_PROGRAM_RANGE,
    validationValid: source.data?.validation?.valid === true,
    sourceReady: summary.ready_for_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_ui_handoff_bundle === true,
    status: summary.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_ui_handoff_status ?? "missing",
    p46000ContractReady: boundary.p46000_contract_ready === true,
    ...
    boundaryClosed: P46000_FALSE_FLAGS.every((flag) => boundary[flag] === false),
  };
}
...
function buildPhaseRows(roadmapText, generatedAt) {
  return PHASE_SPECS.map(([range, label, outputRef]) => row({
    ...
    evidence_ref: "docs/hermes-roadmap-p46001-p46400.md",
...
function buildSourceRows({ source, sourceState, commitRef, generatedAt }) {
  return [
    ["artifact_available", "P46000 implementation review UI handoff bundle source is available", sourceState.available],
    ["program_range", "P46000 source program range is P45601-P46000", sourceState.programRangeOk],
    ["validation_valid", "P46000 source validation is valid", sourceState.validationValid],
    ["p46001_handoff_open", "P46000 source opened P46001 handoff", sourceState.sourceReady],
    ["p46000_contract_ready", "P46000 source contract is ready", sourceState.p46000ContractReady],
    ["ui_handoff_bundle_visible", "P46000 UI handoff bundle rows are visible", sourceState.uiHandoffBundleVisible],
    ["screen_package_visible", "P46000 read-only screen package rows are visible", sourceState.screenPackageVisible],
    ["handoff_view_visible", "P46000 handoff view rows are visible", sourceState.handoffViewVisible],
    ["review_affordance_visible", "P46000 review affordance rows are visible", sourceState.reviewAffordanceVisible],
    ["no_serve_no_receipt_accept_closed", "P46000 no-serve/no-receipt-accept boundary is closed", sourceState.noServeNoRenderClosed],
    ["commit_ref_present", "Current commit ref is present for local UI binding smoke", Boolean(commitRef)],
    ["source_blocker_visible", "P46000 source blocker is visible when local UI binding is closed", sourceState.available],
  ].map(([id, label, observed]) => row({
    row_id: `source_binding.${id}`,
    category: "p46000_source_binding",
    label,
    observed,
...
function buildNoServerNoBrowserBoundaryRows(generatedAt) {
  return ALL_FALSE_FLAGS.map((flag) => row({
    ...
    observed: true,
    evidence_ref: "work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke_boundary",
    boundary_flag: flag,
    allowed_now: false,
    generated_at: generatedAt,
  }));
...
function buildCheckpointRows(context) {
  ...
  return [
    ["source_ready", "P46000 source is ready for P46001", context.sourceState.sourceReady],
    ...
    ["p46401_handoff_gate", "P46401 handoff opens only when local UI binding smoke conditions pass", handoffReady],
    ["p46401_handoff_blocker_visible", "P46401 handoff blocker is visible when the gate is closed", true],
  ].map(([id, label, observed]) => row({
    row_id: `p46400_checkpoint.${id}`,
    category: "p46400_clean_checkpoint",
    label,
    observed,
    evidence_ref: "p46400_clean_checkpoint_rows",
    generated_at: context.generatedAt,
  }));
}

function buildBoundary(context) {
  const p46400ContractReady = allPass(context.phaseRows)
    ...
    && allPass(context.blockerRows)
    && allPass(context.boundaryRows)
    && visibleOrPassed(context.checkpointRows, "p46400_checkpoint.p46401_handoff_blocker_visible");
  const handoffReady = context.sourceState.sourceReady
    && context.sourceState.validationValid
    && context.sourceState.boundaryClosed
    && allPass(context.bindingRows)
    ...
    && allPass(context.boundaryRows);
  return {
    p46400_contract_ready: p46400ContractReady,
    ready_for_p46401_handoff: handoffReady,
    source_p46000_ready_for_p46001_handoff: context.sourceState.sourceReady,
...
function buildValidationItems(context) {
  return [
    ...
    validationItem("roadmap.phase_coverage", "roadmap", allPass(context.phaseRows), "P46001-P46400 phase rows are incomplete"),
    validationItem("architecture.reference", "architecture", context.architectureDoc.text.includes("P46001-P46400 Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Local UI Binding Smoke"), "Architecture doc missing P46001-P46400 reference"),
    validationItem("source.visible", "source", visibleOrPassed(context.sourceRows, "source_binding.source_blocker_visible"), "P46000 source state is not visible"),
    ...
    validationItem("checkpoint.visible", "checkpoint", visibleOrPassed(context.checkpointRows, "p46400_checkpoint.p46401_handoff_blocker_visible"), "P46400 checkpoint blocker is not visible"),
  ];
}

function buildContract(generatedAt) {
  return {
    contract_id: "work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke.contract.v1",
    generated_at: generatedAt,
    source_p46000_required_or_rebuilt: true,
    ...
    p46401_handoff_is_not_server_route_dom_browser_action_write_approval_closeout_or_enterprise_trust: true,
    protected_authority: "closed",
  };
}

function buildSummary({ boundary, validation }) {
  const status = validation.valid && boundary.ready_for_p46401_handoff
    ? READY_STATUS
    : validation.valid && boundary.p46400_contract_ready
      ? BLOCK_PENDING_STATUS
      : BLOCKED_STATUS;
  return {
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke_status: status,
    source_p46000_ready_for_p46001_handoff: boundary.source_p46000_ready_for_p46001_handoff,
    ...
    ready_for_p46401_handoff: validation.valid && boundary.ready_for_p46401_handoff,
...
function renderMarkdown(result) {
  return [
    "# Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Local UI Binding Smoke",
    "",
    `Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_local_ui_binding_smoke_status}`,
    `Program: ${result.program_range}`,
    `P46000 ready for P46001 handoff: ${result.summary.source_p46000_ready_for_p46001_handoff}`,
    ...
    `Ready for P46401 handoff: ${result.summary.ready_for_p46401_handoff}`,
...
<head>
  ...
  <title>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Local UI Binding Smoke</title>
  <style>
...
<body>
  <main>
    <h1>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Local UI Binding Smoke</h1>
...
async function readJsonOrBuildP46000(filePath, generatedAt) {
  const source = await readJsonSource(filePath);
  if (source.available) return source;
  const built = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewUiHandoffBundle({ runAt: generatedAt, write: false });
  return normalizeInlineJsonSource("built.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_ui_handoff_bundle", built);
}
...
function normalizeInputs(options) {
  const defaults = DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_REVIEW_LOCAL_UI_BINDING_SMOKE_INPUTS;
  ...
  return {
    ...
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_ui_handoff_bundle_path: path.resolve(repoRoot, options.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewUiHandoffBundlePath ?? defaults.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewUiHandoffBundlePath),
  };
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--check") {
      ...
      args.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewUiHandoffBundlePath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-local-ui-binding-smoke.mjs [--check] [--out-dir DIR] [--source FILE] [--commit-ref SHA]

Validates or writes the Hermes P46001-P46400 Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Local UI Binding Smoke.
`);
}
