...
import {
  DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_LEAKAGE_REGRESSION_FIXTURES_INPUTS,
  buildTradingSecretScanRemediationReceiptChainSecretLeakageRegressionFixtures,
} from "./trading-secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures.mjs";

export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_OUT_DIR = "artifacts/trading-secret-scan-remediation-receipt-chain-secret-scan-gate-fixtures/latest";
export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_INPUTS = {
  ...DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_LEAKAGE_REGRESSION_FIXTURES_INPUTS,
  secretLeakageRegressionFixturesSchemaPath: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_LEAKAGE_REGRESSION_FIXTURES_INPUTS.schemaPath,
  ...
  schemaPath: "schemas/trading/trading-secret-scan-remediation-receipt-chain-secret-scan-gate-fixtures.schema.json",
};

const SCHEMA_VERSION = "trading-secret-scan-remediation-receipt-chain-secret-scan-gate-fixtures.v1";
const CAPABILITY_ID = "trading.secret_scan_remediation_receipt_chain_secret_scan_gate_fixtures";
const PHASE_SLOT = "P451";
const PREVIOUS_PHASE_SLOT = "P450";
const NEXT_PHASE_SLOT = "P452";
const READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_secret_scan_gate";
const SOURCE_READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_secret_leakage_regression";
const COMMAND_NAME = "trading:secret-scan-remediation-receipt-chain-secret-scan-gate-fixtures";
const SECURITY_SCAN_COMMAND_NAME = "security:secrets-scan-gate";
const REQUIRED_ROW_KEYS = [
  "p450_secret_scan_remediation_receipt_chain_secret_leakage_regression_ready",
  ...
  "p450_synthetic_regressions_feed_gate",
  "secret_scan_gate_no_mutation",
];

export async function runTradingSecretScanRemediationReceiptChainSecretScanGateFixtures(options = {}) {
  const result = await buildTradingSecretScanRemediationReceiptChainSecretScanGateFixtures(options);
  if (options.write !== false) await writeTradingSecretScanRemediationReceiptChainSecretScanGateFixtures(result, result.output_dir);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Trading secret scan remediation receipt chain secret scan gate fixtures failed with ${result.validation.errors.length} error(s).`);
...
export async function buildTradingSecretScanRemediationReceiptChainSecretScanGateFixtures(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_OUT_DIR);
  const inputs = normalizeInputs(options);
  const leakageRegression = await buildTradingSecretScanRemediationReceiptChainSecretLeakageRegressionFixtures({
    ...options,
    runAt: generatedAt,
  ...
  const sourceReady = leakageRegression.validation.valid && leakageRegression.summary.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status === SOURCE_READY_STATUS;
  ...
  const result = {
    ...
    trading_secret_scan_remediation_receipt_chain_secret_scan_gate_fixtures_id: `trading-secret-scan-remediation-receipt-chain-secret-scan-gate-fixtures.${dateStamp(generatedAt)}`,
  ...
  const schemaErrors = schema.available ? validateAgainstSchema(result, schema.data, {}, "trading_secret_scan_remediation_receipt_chain_secret_scan_gate_fixtures") : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  ...
  result.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_gate_fixtures_id = result.trading_secret_scan_remediation_receipt_chain_secret_scan_gate_fixtures_id;
  return { ...result, markdown: renderMarkdown(result) };
}

export async function writeTradingSecretScanRemediationReceiptChainSecretScanGateFixtures(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "trading-secret-scan-remediation-receipt-chain-secret-scan-gate-fixtures.json"), serializableResult(result));
  await writeJson(path.join(outDir, "secret-scan-gate-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-secret-scan-gate-rows.v1", "secret_scan_gate_rows", result.secret_scan_gate_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-scan-gate-gate-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-secret-scan-gate-gate-rows.v1", "secret_scan_gate_gate_rows", result.secret_scan_gate_gate_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-scan-gate-boundary.json"), result.secret_scan_gate_boundary);
  await writeJson(path.join(outDir, "validation-report.json"), {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-gate-fixtures-validation-report.v1",
...
export async function runTradingSecretScanRemediationReceiptChainSecretScanGateFixturesCli(argv = process.argv.slice(2)) {
  ...
  try {
    const result = await runTradingSecretScanRemediationReceiptChainSecretScanGateFixtures(args);
    console.log(`Trading secret scan remediation receipt chain secret scan gate fixtures ${args.check ? "validated" : "written"} at ${result.output_dir}`);
    console.log(`Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_gate_fixtures_status}`);
...
function buildSecretScanGateCoverage({ leakageRegression, sources }) {
  ...
  return {
    p450_ready: leakageRegression.validation.valid && leakageRegression.summary.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status === SOURCE_READY_STATUS,
    ...
    p450_regression_registered: typeof scripts["trading:secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures"] === "string" && validateScript.includes("npm run trading:secret-scan-remediation-receipt-chain-secret-leakage-regression-fixtures -- --check"),
    ...
    p450_synthetic_regressions_ready: leakageRegression.summary.regression_failure_count === 0 && leakageRegression.summary.synthetic_pattern_only && !leakageRegression.summary.raw_secret_material_materialized,
...
function buildSecretScanGateAnchor(leakageRegression, coverage) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-gate-fixtures-anchor.v1",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_id: leakageRegression.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_id,
    source_secret_scan_remediation_receipt_chain_secret_leakage_regression_status: leakageRegression.summary.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status,
    required_row_count: REQUIRED_ROW_KEYS.length,
    required_row_keys: REQUIRED_ROW_KEYS,
    coverage_hash: hashValue(coverage),
    source_hash: hashValue({
      id: leakageRegression.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_id,
      status: leakageRegression.summary.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status,
...
function buildSecretScanGateRows({ sourceReady, leakageRegression, coverage }) {
  const rowInputs = [
    scanGateRowInput("p450_secret_scan_remediation_receipt_chain_secret_leakage_regression_ready", "trading_secret_scan_remediation_receipt_chain_secret_leakage_regression", "summary", [
      condition("source.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status", leakageRegression.summary.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status, SOURCE_READY_STATUS),
    ...
    scanGateRowInput("p450_synthetic_regressions_feed_gate", "trading_secret_scan_remediation_receipt_chain_secret_leakage_regression", "summary.synthetic_regressions", [
      condition("source.synthetic_regressions_ready", coverage.p450_synthetic_regressions_ready, true),
...
function buildSecretScanGateRow(input, sourceReady, index) {
  ...
  const row = {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-gate-row.v1",
    secret_scan_gate_row_id: `trading-secret-scan-remediation-receipt-chain-secret-scan-gate.row.${input.rowKey}`,
    phase_slot: PHASE_SLOT,
    ...
    source_secret_scan_remediation_receipt_chain_secret_leakage_regression_ready: sourceReady,
...
function buildSecretScanGateBoundary({ generatedAt, writeRequested, leakageRegression, rows, coverage }) {
  const sourceReady = leakageRegression.validation.valid && leakageRegression.summary.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status === SOURCE_READY_STATUS;
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-gate-fixtures-boundary.v1",
    generated_at: generatedAt,
    ...
    p450_regression_registered: coverage.p450_regression_registered,
    ...
    p450_synthetic_regressions_ready: coverage.p450_synthetic_regressions_ready,
...
function buildSecretScanGateGateRows({ leakageRegression, packageJson, platformOpsLedger, rows, boundary }) {
  ...
  const gateInputs = [
    ["p450_secret_scan_remediation_receipt_chain_secret_leakage_regression_ready", "P450 secret scan remediation receipt chain secret leakage regression source is ready.", leakageRegression.validation.valid && leakageRegression.summary.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status === SOURCE_READY_STATUS],
    ["platform_package_script_registered", "package.json registers the P451 trading secret scan gate fixtures command.", typeof scripts[COMMAND_NAME] === "string" && scripts[COMMAND_NAME].length > 0],
    ["platform_validation_chain_registered", "Validation chain includes the P451 trading secret scan gate fixtures command.", validateScript.includes(`npm run ${COMMAND_NAME} -- --check`)],
    ["p451_ledger_acceptance_declared", "P451 acceptance row is declared in the platform operations ledger.", ledgerText.includes("P451: `trading:secret-scan-remediation-receipt-chain-secret-scan-gate-fixtures`")],
...
function gateRow(rowKey, description, passed) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-gate-gate-row.v1",
    secret_scan_gate_gate_row_id: `trading-secret-scan-remediation-receipt-chain-secret-scan-gate-fixtures.gate.${rowKey}`,
    phase_slot: PHASE_SLOT,
    row_key: rowKey,
    description,
...
function buildValidationItems({ leakageRegression, sources, rows, gateRows, boundary }) {
  return [
    validationItem("source.secret_scan_remediation_receipt_chain_secret_leakage_regression", "p450_secret_scan_remediation_receipt_chain_secret_leakage_regression_ready", leakageRegression.validation.valid && leakageRegression.summary.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status === SOURCE_READY_STATUS, "P450 secret scan remediation receipt chain secret leakage regression fixtures must be ready."),
    validationItem("source.package_json", "package_json_available", sources.packageJson.available, "package.json is readable for P451 secret scan gate fixtures."),
    ...
    validationItem("secret_scan_gate_gate_rows", "secret_scan_gate_gates_ready", gateRows.length >= 10 && gateRows.every((row) => row.gate_status === "ready" && !row.protected_action_executed_by_gate), "P451 secret scan gate gates are ready."),
    ...
    validationItem("boundary.no_mutation", "no_secret_or_trading_mutation", boundary.no_secret_material_read && boundary.no_secret_or_trading_mutation && !boundary.secret_scan_remediation_action_allowed && !boundary.secret_values_read && !boundary.env_file_read && !boundary.desktop_config_content_inspected && !boundary.desktop_config_read && !boundary.credential_lookup_allowed && !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.command_execution_performed && !boundary.artifact_write_performed && !boundary.protected_action_executed, "P451 performs no secret reads, secret scan remediation action, trading mutation, artifact mutation, or protected action."),
  ];
}

function buildSummary({ leakageRegression, rows, gateRows, boundary, validation }) {
  return {
    trading_secret_scan_remediation_receipt_chain_secret_scan_gate_fixtures_status: validation.valid ? READY_STATUS : "blocked",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_secret_leakage_regression_status: leakageRegression.summary.trading_secret_scan_remediation_receipt_chain_secret_leakage_regression_fixtures_status,
    source_secret_scan_remediation_receipt_chain_secret_leakage_regression_ready: boundary.source_secret_scan_remediation_receipt_chain_secret_leakage_regression_ready,
    required_row_count: REQUIRED_ROW_KEYS.length,
    secret_scan_gate_row_count: rows.length,
    ...
    p450_regression_registered: boundary.p450_regression_registered,
    ...
    p450_synthetic_regressions_ready: boundary.p450_synthetic_regressions_ready,
...
function renderMarkdown(result) {
  const lines = [
    "# Trading Secret Scan Remediation Receipt Chain Secret Scan Gate Fixtures",
    "",
    `Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_gate_fixtures_status}`,
    `Phase: ${result.summary.phase_slot}`,
    `Source secret scan remediation receipt chain secret leakage regression: ${result.summary.source_secret_scan_remediation_receipt_chain_secret_leakage_regression_status}`,
...
function parseArgs(argv) {
  const parsed = { outDir: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_OUT_DIR };
...
function printHelp() {
  console.log(`Usage: node scripts/trading-secret-scan-remediation-receipt-chain-secret-scan-gate-fixtures.mjs [options]

Options:
  --out-dir <folder>                         Output directory. Default: ${DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_OUT_DIR}
  --run-at <iso>                             Deterministic generated_at timestamp.
  --package <path>                           package.json path.
  --platform-ops-ledger <path>               P341-P500 platform operations ledger path.
  --secret-leakage-regression-fixtures-schema <path>
                                             P450 secret scan remediation receipt chain secret leakage regression fixtures schema path.
...
function normalizeInputs(options = {}) {
  return {
    package_path: path.resolve(options.packagePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_INPUTS.packagePath),
    platform_ops_ledger_path: path.resolve(options.platformOpsLedgerPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_INPUTS.platformOpsLedgerPath),
    secret_leakage_regression_fixtures_schema_path: path.resolve(options.secretLeakageRegressionFixturesSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_INPUTS.secretLeakageRegressionFixturesSchemaPath),
    secrets_scan_gate_doc_path: path.resolve(options.secretsScanGateDocPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_INPUTS.secretsScanGateDocPath),
    control_plane_loop_path: path.resolve(options.controlPlaneLoopPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_INPUTS.controlPlaneLoopPath),
    contract_validation_suite_path: path.resolve(options.contractValidationSuitePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_INPUTS.contractValidationSuitePath),
    review_api_path: path.resolve(options.reviewApiPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_INPUTS.reviewApiPath),
    schema_path: path.resolve(options.schemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_GATE_FIXTURES_INPUTS.schemaPath),
  };
}
