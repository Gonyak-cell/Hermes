...
import {
  ...
  buildTradingSecretScanRemediationReceiptChainSignoffCloseoutFixtures,
} from "./trading-secret-scan-remediation-receipt-chain-signoff-closeout-fixtures.mjs";

export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_OUT_DIR = "artifacts/trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures/latest";
export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS = {
  ...DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS,
  secretScanRemediationReceiptChainSignoffCloseoutSchemaPath: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.schemaPath,
  ...
  schemaPath: "schemas/trading/trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures.schema.json",
};

const SCHEMA_VERSION = "trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures.v1";
const CAPABILITY_ID = "trading.secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures";
const PHASE_SLOT = "P447";
const PREVIOUS_PHASE_SLOT = "P446";
const NEXT_PHASE_SLOT = "P448";
const READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_broker_adapter_separation";
const SOURCE_READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_signoff_closeout";
...
export async function runTradingSecretScanRemediationReceiptChainBrokerAdapterSeparationFixtures(options = {}) {
  const result = await buildTradingSecretScanRemediationReceiptChainBrokerAdapterSeparationFixtures(options);
  if (options.write !== false) await writeTradingSecretScanRemediationReceiptChainBrokerAdapterSeparationFixtures(result, result.output_dir);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Trading secret scan remediation receipt chain broker adapter separation fixtures failed with ${result.validation.errors.length} error(s).`);
...
export async function buildTradingSecretScanRemediationReceiptChainBrokerAdapterSeparationFixtures(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_OUT_DIR);
  const inputs = normalizeInputs(options);
  const signoffCloseout = await buildTradingSecretScanRemediationReceiptChainSignoffCloseoutFixtures({
    ...options,
    runAt: generatedAt,
    packagePath: inputs.package_path,
    platformOpsLedgerPath: inputs.platform_ops_ledger_path,
    schemaPath: inputs.secret_scan_remediation_receipt_chain_signoff_closeout_schema_path,
    write: false,
  });
  ...
  const sourceReady = signoffCloseout.validation.valid && signoffCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_status === SOURCE_READY_STATUS;
  ...
  const result = {
    ...
    trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_id: `trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures.${dateStamp(generatedAt)}`,
  ...
  const schemaErrors = schema.available ? validateAgainstSchema(result, schema.data, {}, "trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures") : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  ...
  result.summary.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_id = result.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_id;
  return { ...result, markdown: renderMarkdown(result) };
}

export async function writeTradingSecretScanRemediationReceiptChainBrokerAdapterSeparationFixtures(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures.json"), serializableResult(result));
  await writeJson(path.join(outDir, "broker-adapter-separation-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-rows.v1", "broker_adapter_separation_rows", result.broker_adapter_separation_rows, result.generated_at));
  await writeJson(path.join(outDir, "broker-adapter-separation-gate-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-gate-rows.v1", "broker_adapter_separation_gate_rows", result.broker_adapter_separation_gate_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "validation-report.json"), {
    schema_version: "trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures-validation-report.v1",
...
export async function runTradingSecretScanRemediationReceiptChainBrokerAdapterSeparationFixturesCli(argv = process.argv.slice(2)) {
  ...
  try {
    const result = await runTradingSecretScanRemediationReceiptChainBrokerAdapterSeparationFixtures(args);
    console.log(`Trading secret scan remediation receipt chain broker adapter separation fixtures ${args.check ? "validated" : "written"} at ${result.output_dir}`);
    console.log(`Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_status}`);
...
function buildAdapterSeparationAnchor(signoffCloseout) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures-anchor.v1",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_id: signoffCloseout.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_id,
    source_secret_scan_remediation_receipt_chain_signoff_closeout_status: signoffCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_status,
    required_row_count: REQUIRED_ROW_KEYS.length,
    required_row_keys: REQUIRED_ROW_KEYS,
    source_hash: hashValue({
      id: signoffCloseout.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_id,
      status: signoffCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_status,
...
function buildAdapterRow(input, sourceReady, index) {
  ...
  const row = {
    schema_version: "trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-row.v1",
    broker_adapter_separation_row_id: `trading-secret-scan-remediation-receipt-chain-broker-adapter-separation.row.${input.rowKey}`,
    phase_slot: PHASE_SLOT,
    ...
    source_secret_scan_remediation_receipt_chain_signoff_closeout_ready: sourceReady,
...
function buildAdapterSeparationBoundary({ generatedAt, writeRequested, signoffCloseout, rows }) {
  ...
  const sourceReady = signoffCloseout.validation.valid && signoffCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_status === SOURCE_READY_STATUS;
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures-boundary.v1",
    generated_at: generatedAt,
...
function buildAdapterSeparationGateRows({ signoffCloseout, packageJson, platformOpsLedger, sources, rows, boundary }) {
  ...
  const gateInputs = [
    ["p446_signoff_closeout_ready", "P446 secret scan remediation receipt chain signoff closeout source is ready.", signoffCloseout.validation.valid && signoffCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_status === SOURCE_READY_STATUS],
    ["platform_package_script_registered", "package.json registers the P447 trading secret scan remediation receipt chain broker adapter separation fixtures command.", typeof scripts["trading:secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures"] === "string" && scripts["trading:secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures"].length > 0],
    ["platform_validation_chain_registered", "Validation chain includes the P447 trading secret scan remediation receipt chain broker adapter separation fixtures command.", validateScript.includes("npm run trading:secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures -- --check")],
    ["p447_ledger_acceptance_declared", "P447 acceptance row is declared in the platform operations ledger.", ledgerText.includes("P447: `trading:secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures`")],
    ...
    ["no_secret_or_trading_mutation", "P447 exposes only reference-only credential boundaries and performs no secret scan remediation, trading, artifact, release, git, or protected mutation.", boundary.credential_reference_only && boundary.no_secret_material_read && boundary.no_secret_or_trading_mutation && !boundary.credential_lookup_allowed && !boundary.plaintext_secret_allowed && !boundary.provider_key_material_present && !boundary.environment_dump_present && !boundary.secret_material_exposed && !boundary.secret_scan_remediation_action_allowed && !boundary.secret_values_read && !boundary.env_file_read && !boundary.desktop_config_content_inspected && !boundary.desktop_config_read && !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.command_execution_performed && !boundary.artifact_write_performed && !boundary.release_published && !boundary.git_operation_performed && !boundary.protected_action_executed],
  ];
...
function gateRow(rowKey, description, passed) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-gate-row.v1",
    broker_adapter_separation_gate_row_id: `trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures.gate.${rowKey}`,
    phase_slot: PHASE_SLOT,
    row_key: rowKey,
    description,
...
function buildValidationItems({ signoffCloseout, packageJson, platformOpsLedger, sources, rows, gateRows, boundary }) {
  return [
    validationItem("source.secret_scan_remediation_receipt_chain_signoff_closeout", "p446_signoff_closeout_ready", signoffCloseout.validation.valid && signoffCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_status === SOURCE_READY_STATUS, "P446 secret scan remediation receipt chain signoff closeout fixtures must be ready."),
    validationItem("source.package_json", "package_json_available", packageJson.available, "package.json is readable for P447 broker adapter separation fixtures."),
    ...
    validationItem("broker_adapter_separation_gate_rows", "adapter_separation_gates_ready", gateRows.length >= 9 && gateRows.every((row) => row.gate_status === "ready" && !row.protected_action_executed_by_gate), "P447 broker adapter separation gates are ready."),
    ...
    validationItem("boundary.secret_boundary", "reference_only_credentials", boundary.credential_reference_only && boundary.no_secret_material_read && !boundary.credential_lookup_allowed && !boundary.plaintext_secret_allowed && !boundary.provider_key_material_present && !boundary.environment_dump_present && !boundary.secret_material_exposed && !boundary.secret_values_read && !boundary.env_file_read && !boundary.desktop_config_content_inspected && !boundary.desktop_config_read, "Credential handling must remain reference-only with no plaintext, environment dump, provider key, model-context secret exposure, secret values, env content, or Desktop config content."),
    validationItem("boundary.no_mutation", "no_secret_or_trading_mutation", boundary.no_secret_or_trading_mutation && !boundary.secret_scan_remediation_action_allowed && !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.command_execution_performed && !boundary.package_command_execution_performed && !boundary.release_check_execution_performed && !boundary.artifact_read_performed && !boundary.artifact_write_performed && !boundary.release_published && !boundary.git_operation_performed && !boundary.protected_action_executed, "P447 performs no secret scan remediation action, trading, artifact, release, git, or protected mutation."),
  ];
}

function buildSummary({ signoffCloseout, rows, gateRows, boundary, validation }) {
  return {
    trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_status: validation.valid ? READY_STATUS : "blocked",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_signoff_closeout_status: signoffCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_status,
    source_secret_scan_remediation_receipt_chain_signoff_closeout_ready: boundary.source_secret_scan_remediation_receipt_chain_signoff_closeout_ready,
    required_row_count: REQUIRED_ROW_KEYS.length,
    adapter_separation_row_count: rows.length,
...
function renderMarkdown(result) {
  const lines = [
    "# Trading Secret Scan Remediation Receipt Chain Broker Adapter Separation Fixtures",
    "",
    `Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_status}`,
    `Phase: ${result.summary.phase_slot}`,
    `Source signoff closeout: ${result.summary.source_secret_scan_remediation_receipt_chain_signoff_closeout_status}`,
...
function parseArgs(argv) {
  const parsed = { outDir: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_OUT_DIR };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--secret-scan-remediation-receipt-chain-signoff-closeout-schema") parsed.secretScanRemediationReceiptChainSignoffCloseoutSchemaPath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures.mjs [options]

Options:
  --out-dir <folder>                         Output directory. Default: ${DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_OUT_DIR}
  ...
  --secret-scan-remediation-receipt-chain-signoff-closeout-schema <path>
                                             P446 secret scan remediation receipt chain signoff closeout fixtures schema path.
...
function normalizeInputs(options = {}) {
  return {
    package_path: path.resolve(options.packagePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS.packagePath),
    platform_ops_ledger_path: path.resolve(options.platformOpsLedgerPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS.platformOpsLedgerPath),
    secret_scan_remediation_receipt_chain_signoff_closeout_schema_path: path.resolve(options.secretScanRemediationReceiptChainSignoffCloseoutSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS.secretScanRemediationReceiptChainSignoffCloseoutSchemaPath),
    execution_engine_path: path.resolve(options.executionEnginePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS.executionEnginePath),
    paper_shadow_path: path.resolve(options.paperShadowPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS.paperShadowPath),
    limited_live_path: path.resolve(options.limitedLivePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS.limitedLivePath),
    full_auto_path: path.resolve(options.fullAutoPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS.fullAutoPath),
    research_backtest_paper_path: path.resolve(options.researchBacktestPaperPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS.researchBacktestPaperPath),
    schema_path: path.resolve(options.schemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS.schemaPath),
  };
}
