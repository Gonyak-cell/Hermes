...
import {
  ALL_FALSE_FLAGS as P42000_FALSE_FLAGS,
  buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoff,
} from "./work-os-static-bundle-review-ui-implementation-review-static-shell-handoff.mjs";

export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_CANDIDATE_OUT_DIR = "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-candidate/latest";
export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_CANDIDATE_INPUTS = {
  schemaPath: "schemas/work-os-static-bundle-review-ui-implementation-review-static-shell-candidate.schema.json",
  packagePath: "package.json",
  roadmapDocPath: "docs/hermes-roadmap-p42001-p42400.md",
  architectureDocPath: "docs/architecture.md",
  sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoffPath: "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-handoff/latest/work-os-static-bundle-review-ui-implementation-review-static-shell-handoff.json",
};

const COMMAND_NAME = "platform:work-os-static-bundle-review-ui-implementation-review-static-shell-candidate";
const SCHEMA_VERSION = "work-os-static-bundle-review-ui-implementation-review-static-shell-candidate.v1";
const CAPABILITY_ID = "platform.work_os_static_bundle_review_ui_implementation_review_static_shell_candidate";
const PROGRAM_RANGE = "P42001-P42400";
const SOURCE_PROGRAM_RANGE = "P41601-P42000";
const READY_STATUS = "ready_for_work_os_static_bundle_review_ui_implementation_review_static_shell_candidate";
const BLOCK_PENDING_STATUS = "valid_block_work_os_static_bundle_review_ui_implementation_review_static_shell_candidate_pending";
const BLOCKED_STATUS = "blocked_work_os_static_bundle_review_ui_implementation_review_static_shell_candidate";
...
export const ALL_FALSE_FLAGS = [...new Set([...STATIC_SHELL_CANDIDATE_FALSE_FLAGS, ...P42000_FALSE_FLAGS])];

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellCandidate(options = {}) {
  const result = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellCandidate(options);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Work OS Static Bundle Review UI Implementation Review Static Shell Candidate failed with ${result.validation.errors.length} error(s).`);
    error.validation = result.validation;
    throw error;
  }
  if (!options.check && options.write !== false) await writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellCandidate(result, result.output_dir);
  return result;
}

export async function buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellCandidate(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_CANDIDATE_OUT_DIR);
  const inputs = normalizeInputs(options);
  const schema = await readJsonSource(inputs.schema_path);
  const packageJson = await readJsonSource(inputs.package_path);
  const roadmapDoc = await readTextSource(inputs.roadmap_doc_path);
  const architectureDoc = await readTextSource(inputs.architecture_doc_path);
  const source = Object.prototype.hasOwnProperty.call(options, "workOsStaticBundleReviewUiImplementationReviewStaticShellHandoff")
    ? normalizeInlineJsonSource("inline.work_os_static_bundle_review_ui_implementation_review_static_shell_handoff", options.workOsStaticBundleReviewUiImplementationReviewStaticShellHandoff)
    : await readJsonOrBuildP42000(inputs.source_work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_path, generatedAt);
  ...
  const result = {
    ...
    inputs,
    source_refs: {
      work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_path: source.path,
      source_commit_ref: commitRef || null,
    },
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_summary: source.data?.summary ?? null,
    work_os_static_bundle_review_ui_implementation_review_static_shell_candidate_contract: buildContract(generatedAt),
    work_os_static_bundle_review_ui_implementation_review_static_shell_candidate_phase_rows: phaseRows,
    p42000_source_binding_rows: sourceRows,
    ...
    p42400_clean_checkpoint_rows: checkpointRows,
    work_os_static_bundle_review_ui_implementation_review_static_shell_candidate_boundary: boundary,
    work_os_static_bundle_review_ui_implementation_review_static_shell_candidate_validation_items: validationItems,
    validation: preliminaryValidation,
    summary: buildSummary({ boundary, validation: preliminaryValidation }),
  };

  const schemaErrors = schema.available
    ? validateAgainstSchema(result, schema.data, {}, "work_os_static_bundle_review_ui_implementation_review_static_shell_candidate")
    : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  const schemaValidationItems = schemaErrors.map((error, index) => validationItem(`schema.${index}`, "schema_validation", false, error.message));
  result.work_os_static_bundle_review_ui_implementation_review_static_shell_candidate_validation_items = [...validationItems, ...schemaValidationItems];
  result.validation = summarizeValidation(result.work_os_static_bundle_review_ui_implementation_review_static_shell_candidate_validation_items);
  result.summary = buildSummary({ boundary, validation: result.validation });
  return { ...result, markdown: renderMarkdown(result), html: renderHtml(result) };
}

export async function writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellCandidate(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-candidate.json"), serializableResult(result));
  await writeJson(path.join(outDir, "p42000-source-binding-rows.json"), collectionEnvelope("p42000-source-binding-rows.v1", "p42000_source_binding_rows", result.p42000_source_binding_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "p42400-clean-checkpoint-rows.json"), collectionEnvelope("p42400-clean-checkpoint-rows.v1", "p42400_clean_checkpoint_rows", result.p42400_clean_checkpoint_rows, result.generated_at));
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-candidate-boundary.json"), result.work_os_static_bundle_review_ui_implementation_review_static_shell_candidate_boundary);
  await writeFile(path.join(outDir, "summary.md"), result.markdown, "utf8");
  await writeFile(path.join(outDir, "index.html"), result.html, "utf8");
}

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellCandidateCli(argv = process.argv.slice(2)) {
  const args = parseArgs(argv);
  if (args.help) {
    printHelp();
    return null;
  }
  const result = await runWorkOsStaticBundleReviewUiImplementationReviewStaticShellCandidate(args);
  console.log(`Work OS Static Bundle Review UI Implementation Review Static Shell Candidate ${args.check ? "validated" : "written"} at ${result.output_dir}`);
  console.log(`Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_candidate_status}`);
  console.log(`Program: ${result.program_range}`);
  console.log(`P42000 ready for P42001 handoff: ${result.summary.source_p42000_ready_for_p42001_handoff}`);
  ...
  console.log(`Ready for P42401 handoff: ${result.summary.ready_for_p42401_handoff}`);
...
function buildSourceState(source) {
  ...
  const boundary = source.data?.work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_boundary ?? {};
  return {
    available: source.available === true,
    programRangeOk: source.data?.program_range === SOURCE_PROGRAM_RANGE,
    validationValid: source.data?.validation?.valid === true,
    sourceReady: summary.ready_for_p42001_handoff === true,
    status: summary.work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_status ?? "missing",
    p42000ContractReady: boundary.p42000_contract_ready === true,
    ...
    boundaryClosed: P42000_FALSE_FLAGS.every((flag) => boundary[flag] === false),
  };
}
...
function buildPhaseRows(roadmapText, generatedAt) {
  return PHASE_SPECS.map(([range, label, outputRef]) => row({
    ...
    evidence_ref: "docs/hermes-roadmap-p42001-p42400.md",
...
function buildSourceRows({ source, sourceState, commitRef, generatedAt }) {
  return [
    ["artifact_available", "P42000 implementation review static shell handoff source is available", sourceState.available],
    ["program_range", "P42000 source program range is P41601-P42000", sourceState.programRangeOk],
    ["validation_valid", "P42000 source validation is valid", sourceState.validationValid],
    ["p42001_handoff_open", "P42000 source opened P42001 handoff", sourceState.sourceReady],
    ["p42000_contract_ready", "P42000 source contract is ready", sourceState.p42000ContractReady],
    ["static_shell_handoff_visible", "P42000 static shell handoff rows are visible", sourceState.staticShellHandoffVisible],
    ["shell_section_binding_visible", "P42000 shell section binding rows are visible", sourceState.shellSectionBindingVisible],
    ["fixture_slot_projection_visible", "P42000 fixture slot projection rows are visible", sourceState.fixtureSlotProjectionVisible],
    ["blocked_state_copy_visible", "P42000 blocked state copy rows are visible", sourceState.blockedStateCopyVisible],
    ["no_serve_no_render_closed", "P42000 no-serve/no-render authority is closed", sourceState.noServeNoRenderClosed],
    ["commit_ref_present", "Current commit ref is present for static shell candidate", Boolean(commitRef)],
    ["source_blocker_visible", "P42000 source blocker is visible when static shell candidate is closed", sourceState.available],
  ].map(([id, label, observed]) => row({
    row_id: `source_binding.${id}`,
    category: "p42000_source_binding",
    label,
    observed,
...
function buildNoServeNoDomBoundaryRows(generatedAt) {
  return ALL_FALSE_FLAGS.map((flag) => row({
    ...
    observed: true,
    evidence_ref: "work_os_static_bundle_review_ui_implementation_review_static_shell_candidate_boundary",
    boundary_flag: flag,
    allowed_now: false,
    generated_at: generatedAt,
  }));
...
function buildCheckpointRows(context) {
  ...
  return [
    ["source_ready", "P42000 source is ready for P42001", context.sourceState.sourceReady],
    ...
    ["p42401_handoff_gate", "P42401 handoff opens only when static shell candidate conditions pass", handoffReady],
    ["p42401_handoff_blocker_visible", "P42401 handoff blocker is visible when the gate is closed", true],
  ].map(([id, label, observed]) => row({
    row_id: `p42400_checkpoint.${id}`,
    category: "p42400_clean_checkpoint",
    label,
    observed,
    evidence_ref: "p42400_clean_checkpoint_rows",
    generated_at: context.generatedAt,
  }));
}

function buildBoundary(context) {
  const p42400ContractReady = allPass(context.phaseRows)
    ...
    && allPass(context.copyRows)
    && allPass(context.boundaryRows)
    && visibleOrPassed(context.checkpointRows, "p42400_checkpoint.p42401_handoff_blocker_visible");
  const handoffReady = context.sourceState.sourceReady
    && context.sourceState.validationValid
    && context.sourceState.boundaryClosed
    && allPass(context.candidateRows)
    ...
    && allPass(context.boundaryRows);
  return {
    p42400_contract_ready: p42400ContractReady,
    ready_for_p42401_handoff: handoffReady,
    source_p42000_ready_for_p42001_handoff: context.sourceState.sourceReady,
...
function buildValidationItems(context) {
  return [
    ...
    validationItem("roadmap.phase_coverage", "roadmap", allPass(context.phaseRows), "P42001-P42400 phase rows are incomplete"),
    validationItem("architecture.reference", "architecture", context.architectureDoc.text.includes("P42001-P42400 Work OS Static Bundle Review UI Implementation Review Static Shell Candidate"), "Architecture doc missing P42001-P42400 reference"),
    validationItem("source.visible", "source", visibleOrPassed(context.sourceRows, "source_binding.source_blocker_visible"), "P42000 source state is not visible"),
    ...
    validationItem("checkpoint.visible", "checkpoint", visibleOrPassed(context.checkpointRows, "p42400_checkpoint.p42401_handoff_blocker_visible"), "P42400 checkpoint blocker is not visible"),
  ];
}

function buildContract(generatedAt) {
  return {
    contract_id: "work_os_static_bundle_review_ui_implementation_review_static_shell_candidate.contract.v1",
    generated_at: generatedAt,
    source_p42000_required_or_rebuilt: true,
    ...
    p42401_handoff_is_not_server_route_dom_browser_action_write_approval_closeout_or_enterprise_trust: true,
    protected_authority: "closed",
  };
}

function buildSummary({ boundary, validation }) {
  const status = validation.valid && boundary.ready_for_p42401_handoff
    ? READY_STATUS
    : validation.valid && boundary.p42400_contract_ready
      ? BLOCK_PENDING_STATUS
      : BLOCKED_STATUS;
  return {
    work_os_static_bundle_review_ui_implementation_review_static_shell_candidate_status: status,
    source_p42000_ready_for_p42001_handoff: boundary.source_p42000_ready_for_p42001_handoff,
    ...
    ready_for_p42401_handoff: validation.valid && boundary.ready_for_p42401_handoff,
...
function renderMarkdown(result) {
  return [
    "# Work OS Static Bundle Review UI Implementation Review Static Shell Candidate",
    "",
    `Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_candidate_status}`,
    `Program: ${result.program_range}`,
    `P42000 ready for P42001 handoff: ${result.summary.source_p42000_ready_for_p42001_handoff}`,
    ...
    `Ready for P42401 handoff: ${result.summary.ready_for_p42401_handoff}`,
...
<head>
  ...
  <title>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Candidate</title>
  <style>
...
<body>
  <main>
    <h1>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Candidate</h1>
...
async function readJsonOrBuildP42000(filePath, generatedAt) {
  const source = await readJsonSource(filePath);
  if (source.available) return source;
  const built = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoff({ runAt: generatedAt, write: false });
  return normalizeInlineJsonSource("built.work_os_static_bundle_review_ui_implementation_review_static_shell_handoff", built);
}
...
function normalizeInputs(options) {
  const defaults = DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_CANDIDATE_INPUTS;
  ...
  return {
    ...
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_handoff_path: path.resolve(repoRoot, options.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoffPath ?? defaults.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoffPath),
  };
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--check") {
      ...
      args.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellHandoffPath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/work-os-static-bundle-review-ui-implementation-review-static-shell-candidate.mjs [--check] [--out-dir DIR] [--source FILE] [--commit-ref SHA]

Validates or writes the Hermes P42001-P42400 Work OS Static Bundle Review UI Implementation Review Static Shell Candidate.
`);
}
