...
import {
  ALL_FALSE_FLAGS as P29600_FALSE_FLAGS,
  buildReceiptWorkbenchOperatorQueueStaticShellAssemblyPlan,
} from "./receipt-workbench-operator-queue-static-shell-assembly-plan.mjs";

export const DEFAULT_RECEIPT_WORKBENCH_OPERATOR_QUEUE_STATIC_SHELL_ASSEMBLY_HANDOFF_OUT_DIR = "artifacts/receipt-workbench-operator-queue-static-shell-assembly-handoff/latest";
export const DEFAULT_RECEIPT_WORKBENCH_OPERATOR_QUEUE_STATIC_SHELL_ASSEMBLY_HANDOFF_INPUTS = {
  schemaPath: "schemas/receipt-workbench-operator-queue-static-shell-assembly-handoff.schema.json",
  packagePath: "package.json",
  roadmapDocPath: "docs/hermes-roadmap-p29601-p30000.md",
  architectureDocPath: "docs/architecture.md",
  sourceReceiptWorkbenchOperatorQueueStaticShellAssemblyPlanPath: "artifacts/receipt-workbench-operator-queue-static-shell-assembly-plan/latest/receipt-workbench-operator-queue-static-shell-assembly-plan.json",
};

const COMMAND_NAME = "platform:receipt-workbench-operator-queue-static-shell-assembly-handoff";
const SCHEMA_VERSION = "receipt-workbench-operator-queue-static-shell-assembly-handoff.v1";
const CAPABILITY_ID = "platform.receipt_workbench_operator_queue_static_shell_assembly_handoff";
const PROGRAM_RANGE = "P29601-P30000";
const SOURCE_PROGRAM_RANGE = "P29201-P29600";
const READY_STATUS = "ready_for_receipt_workbench_operator_queue_static_shell_assembly_handoff";
const BLOCK_PENDING_STATUS = "valid_block_receipt_workbench_operator_queue_static_shell_assembly_handoff_pending";
const BLOCKED_STATUS = "blocked_receipt_workbench_operator_queue_static_shell_assembly_handoff";
...
export const ALL_FALSE_FLAGS = [...new Set([...STATIC_SHELL_ASSEMBLY_HANDOFF_FALSE_FLAGS, ...P29600_FALSE_FLAGS])];

export async function runReceiptWorkbenchOperatorQueueStaticShellAssemblyHandoff(options = {}) {
  const result = await buildReceiptWorkbenchOperatorQueueStaticShellAssemblyHandoff(options);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Receipt Workbench Operator Queue Static Shell Assembly Handoff failed with ${result.validation.errors.length} error(s).`);
    error.validation = result.validation;
    throw error;
  }
  if (!options.check && options.write !== false) await writeReceiptWorkbenchOperatorQueueStaticShellAssemblyHandoff(result, result.output_dir);
  return result;
}

export async function buildReceiptWorkbenchOperatorQueueStaticShellAssemblyHandoff(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_RECEIPT_WORKBENCH_OPERATOR_QUEUE_STATIC_SHELL_ASSEMBLY_HANDOFF_OUT_DIR);
  const inputs = normalizeInputs(options);
  const schema = await readJsonSource(inputs.schema_path);
  const packageJson = await readJsonSource(inputs.package_path);
  const roadmapDoc = await readTextSource(inputs.roadmap_doc_path);
  const architectureDoc = await readTextSource(inputs.architecture_doc_path);
  const source = Object.prototype.hasOwnProperty.call(options, "receiptWorkbenchOperatorQueueStaticShellAssemblyPlan")
    ? normalizeInlineJsonSource("inline.receipt_workbench_operator_queue_static_shell_assembly_plan", options.receiptWorkbenchOperatorQueueStaticShellAssemblyPlan)
    : await readJsonOrBuildP29600(inputs.source_receipt_workbench_operator_queue_static_shell_assembly_plan_path, generatedAt);
  ...
  const result = {
    ...
    inputs,
    source_refs: {
      receipt_workbench_operator_queue_static_shell_assembly_plan_path: source.path,
      source_commit_ref: commitRef || null,
    },
    source_receipt_workbench_operator_queue_static_shell_assembly_plan_summary: source.data?.summary ?? null,
    receipt_workbench_operator_queue_static_shell_assembly_handoff_contract: buildContract(generatedAt),
    receipt_workbench_operator_queue_static_shell_assembly_handoff_phase_rows: phaseRows,
    p29600_source_binding_rows: sourceRows,
    ...
    p30000_clean_checkpoint_rows: checkpointRows,
    receipt_workbench_operator_queue_static_shell_assembly_handoff_boundary: boundary,
    receipt_workbench_operator_queue_static_shell_assembly_handoff_validation_items: validationItems,
    validation: preliminaryValidation,
    summary: buildSummary({ boundary, validation: preliminaryValidation }),
  };

  const schemaErrors = schema.available
    ? validateAgainstSchema(result, schema.data, {}, "receipt_workbench_operator_queue_static_shell_assembly_handoff")
    : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  const schemaValidationItems = schemaErrors.map((error, index) => validationItem(`schema.${index}`, "schema_validation", false, error.message));
  result.receipt_workbench_operator_queue_static_shell_assembly_handoff_validation_items = [...validationItems, ...schemaValidationItems];
  result.validation = summarizeValidation(result.receipt_workbench_operator_queue_static_shell_assembly_handoff_validation_items);
  result.summary = buildSummary({ boundary, validation: result.validation });
  return { ...result, markdown: renderMarkdown(result), html: renderHtml(result) };
}

export async function writeReceiptWorkbenchOperatorQueueStaticShellAssemblyHandoff(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "receipt-workbench-operator-queue-static-shell-assembly-handoff.json"), serializableResult(result));
  await writeJson(path.join(outDir, "p29600-source-binding-rows.json"), collectionEnvelope("p29600-source-binding-rows.v1", "p29600_source_binding_rows", result.p29600_source_binding_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "p30000-clean-checkpoint-rows.json"), collectionEnvelope("p30000-clean-checkpoint-rows.v1", "p30000_clean_checkpoint_rows", result.p30000_clean_checkpoint_rows, result.generated_at));
  await writeJson(path.join(outDir, "receipt-workbench-operator-queue-static-shell-assembly-handoff-boundary.json"), result.receipt_workbench_operator_queue_static_shell_assembly_handoff_boundary);
  await writeFile(path.join(outDir, "summary.md"), result.markdown, "utf8");
  await writeFile(path.join(outDir, "index.html"), result.html, "utf8");
}

export async function runReceiptWorkbenchOperatorQueueStaticShellAssemblyHandoffCli(argv = process.argv.slice(2)) {
  const args = parseArgs(argv);
  if (args.help) {
    printHelp();
    return null;
  }
  const result = await runReceiptWorkbenchOperatorQueueStaticShellAssemblyHandoff(args);
  console.log(`Receipt Workbench Operator Queue Static Shell Assembly Handoff ${args.check ? "validated" : "written"} at ${result.output_dir}`);
  console.log(`Status: ${result.summary.receipt_workbench_operator_queue_static_shell_assembly_handoff_status}`);
  console.log(`Program: ${result.program_range}`);
  console.log(`P29600 ready for P29601 handoff: ${result.summary.source_p29600_ready_for_p29601_handoff}`);
  ...
  console.log(`Ready for P30001 handoff: ${result.summary.ready_for_p30001_handoff}`);
...
function buildSourceState(source) {
  ...
  const boundary = source.data?.receipt_workbench_operator_queue_static_shell_assembly_plan_boundary ?? {};
  return {
    available: source.available === true,
    programRangeOk: source.data?.program_range === SOURCE_PROGRAM_RANGE,
    validationValid: source.data?.validation?.valid === true,
    sourceReady: summary.ready_for_p29601_handoff === true,
    status: summary.receipt_workbench_operator_queue_static_shell_assembly_plan_status ?? "missing",
    p29600ContractReady: boundary.p29600_contract_ready === true,
    ...
    boundaryClosed: P29600_FALSE_FLAGS.every((flag) => boundary[flag] === false),
  };
}
...
function buildPhaseRows(roadmapText, generatedAt) {
  return PHASE_SPECS.map(([range, label, outputRef]) => row({
    ...
    evidence_ref: "docs/hermes-roadmap-p29601-p30000.md",
...
function buildSourceRows({ source, sourceState, commitRef, generatedAt }) {
  return [
    ["artifact_available", "P29600 operator queue static shell assembly plan source is available", sourceState.available],
    ["program_range", "P29600 source program range is P29201-P29600", sourceState.programRangeOk],
    ["validation_valid", "P29600 source validation is valid", sourceState.validationValid],
    ["p29601_handoff_open", "P29600 source opened P29601 handoff", sourceState.sourceReady],
    ["p29600_contract_ready", "P29600 source contract is ready", sourceState.p29600ContractReady],
    ["assembly_plan_visible", "P29600 static shell assembly plan rows are visible", sourceState.assemblyPlanVisible],
    ["template_composition_visible", "P29600 template composition rows are visible", sourceState.templateCompositionVisible],
    ["state_slot_visible", "P29600 read-only state slot rows are visible", sourceState.readOnlyStateSlotVisible],
    ["blocked_copy_guard_visible", "P29600 accessibility and blocked copy guard rows are visible", sourceState.accessibilityBlockedCopyVisible],
    ["no_build_no_render_closed", "P29600 no-build/no-render boundary is closed", sourceState.noBuildNoRenderClosed],
    ["commit_ref_present", "Current commit ref is present for static shell assembly handoff", Boolean(commitRef)],
    ["source_blocker_visible", "P29600 source blocker is visible when assembly handoff is closed", sourceState.available],
  ].map(([id, label, observed]) => row({
    row_id: `source_binding.${id}`,
    category: "p29600_source_binding",
    label,
    observed,
...
function buildAssemblyHandoffPacketRows({ source, generatedAt }) {
  ...
  return assemblyRows.map((assembly, index) => row({
    ...
    handoff_packet_id: `handoff_packet.operator_queue.${assembly.shell_section_id ?? index + 1}`,
...
function buildTemplateTargetMapRows({ source, handoffRows, generatedAt }) {
  ...
  return handoffRows.map((handoff, index) => {
    ...
    return row({
      ...
      target_path_hint: `operator-queue/static-shell/${handoff.shell_section_id}.fragment.html`,
...
function buildStaticAssetHookGuardRows({ handoffRows, targetRows, bindingRows, generatedAt }) {
  return handoffRows.map((handoff, index) => {
    ...
    return row({
      ...
      css_scope_hook: `hermes-operator-queue-${handoff.shell_section_id.replaceAll("_", "-")}`,
      design_token_hook: `token.operator_queue.${handoff.shell_section_id}`,
      asset_manifest_ref: `asset_manifest.operator_queue.${handoff.shell_section_id}.metadata_only`,
...
function buildNoApplyNoBuildBoundaryRows(generatedAt) {
  return ALL_FALSE_FLAGS.map((flag) => row({
    ...
    observed: true,
    evidence_ref: "receipt_workbench_operator_queue_static_shell_assembly_handoff_boundary",
    boundary_flag: flag,
    allowed_now: false,
    generated_at: generatedAt,
  }));
...
function buildCheckpointRows(context) {
  ...
  return [
    ["source_ready", "P29600 source is ready for P29601", context.sourceState.sourceReady],
    ...
    ["p30001_handoff_gate", "P30001 handoff opens only when static shell assembly handoff conditions pass", handoffReady],
    ["p30001_handoff_blocker_visible", "P30001 handoff blocker is visible when the gate is closed", true],
  ].map(([id, label, observed]) => row({
    row_id: `p30000_checkpoint.${id}`,
    category: "p30000_clean_checkpoint",
    label,
    observed,
    evidence_ref: "p30000_clean_checkpoint_rows",
    generated_at: context.generatedAt,
  }));
}

function buildBoundary(context) {
  const p30000ContractReady = allPass(context.phaseRows)
    ...
    && allPass(context.assetRows)
    && allPass(context.boundaryRows)
    && visibleOrPassed(context.checkpointRows, "p30000_checkpoint.p30001_handoff_blocker_visible");
  const handoffReady = context.sourceState.sourceReady
    && context.sourceState.validationValid
    && context.sourceState.boundaryClosed
    && allPass(context.handoffRows)
    ...
    && allPass(context.boundaryRows);
  return {
    p30000_contract_ready: p30000ContractReady,
    ready_for_p30001_handoff: handoffReady,
    source_p29600_ready_for_p29601_handoff: context.sourceState.sourceReady,
...
function buildValidationItems(context) {
  return [
    ...
    validationItem("roadmap.phase_coverage", "roadmap", allPass(context.phaseRows), "P29601-P30000 phase rows are incomplete"),
    validationItem("architecture.reference", "architecture", context.architectureDoc.text.includes("P29601-P30000 Receipt Workbench Operator Queue Static Shell Assembly Handoff"), "Architecture doc missing P29601-P30000 reference"),
    validationItem("source.visible", "source", visibleOrPassed(context.sourceRows, "source_binding.source_blocker_visible"), "P29600 source state is not visible"),
    ...
    validationItem("checkpoint.visible", "checkpoint", visibleOrPassed(context.checkpointRows, "p30000_checkpoint.p30001_handoff_blocker_visible"), "P30000 checkpoint blocker is not visible"),
  ];
}

function buildContract(generatedAt) {
  return {
    contract_id: "receipt_workbench_operator_queue_static_shell_assembly_handoff.contract.v1",
    generated_at: generatedAt,
    source_p29600_required_or_rebuilt: true,
    ...
    p30001_handoff_is_not_apply_build_route_dom_browser_action_write_approval_closeout_or_enterprise_trust: true,
    protected_authority: "closed",
  };
}

function buildSummary({ boundary, validation }) {
  const status = validation.valid && boundary.ready_for_p30001_handoff
    ? READY_STATUS
    : validation.valid && boundary.p30000_contract_ready
      ? BLOCK_PENDING_STATUS
      : BLOCKED_STATUS;
  return {
    receipt_workbench_operator_queue_static_shell_assembly_handoff_status: status,
    source_p29600_ready_for_p29601_handoff: boundary.source_p29600_ready_for_p29601_handoff,
    ...
    ready_for_p30001_handoff: validation.valid && boundary.ready_for_p30001_handoff,
...
function renderMarkdown(result) {
  return [
    "# Receipt Workbench Operator Queue Static Shell Assembly Handoff",
    "",
    `Status: ${result.summary.receipt_workbench_operator_queue_static_shell_assembly_handoff_status}`,
    `Program: ${result.program_range}`,
    `P29600 ready for P29601 handoff: ${result.summary.source_p29600_ready_for_p29601_handoff}`,
    ...
    `Ready for P30001 handoff: ${result.summary.ready_for_p30001_handoff}`,
...
<head>
  ...
  <title>Hermes Receipt Workbench Operator Queue Static Shell Assembly Handoff</title>
  <style>
...
<body>
  <main>
    <h1>Hermes Receipt Workbench Operator Queue Static Shell Assembly Handoff</h1>
...
async function readJsonOrBuildP29600(filePath, generatedAt) {
  const source = await readJsonSource(filePath);
  if (source.available) return source;
  const built = await buildReceiptWorkbenchOperatorQueueStaticShellAssemblyPlan({ runAt: generatedAt, write: false });
  return normalizeInlineJsonSource("built.receipt_workbench_operator_queue_static_shell_assembly_plan", built);
}
...
function normalizeInputs(options) {
  const defaults = DEFAULT_RECEIPT_WORKBENCH_OPERATOR_QUEUE_STATIC_SHELL_ASSEMBLY_HANDOFF_INPUTS;
  ...
  return {
    ...
    source_receipt_workbench_operator_queue_static_shell_assembly_plan_path: path.resolve(repoRoot, options.sourceReceiptWorkbenchOperatorQueueStaticShellAssemblyPlanPath ?? defaults.sourceReceiptWorkbenchOperatorQueueStaticShellAssemblyPlanPath),
  };
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--check") {
      ...
      args.sourceReceiptWorkbenchOperatorQueueStaticShellAssemblyPlanPath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/receipt-workbench-operator-queue-static-shell-assembly-handoff.mjs [--check] [--out-dir DIR] [--source FILE] [--commit-ref SHA]

Validates or writes the Hermes P29601-P30000 Receipt Workbench Operator Queue Static Shell Assembly Handoff.
`);
}
