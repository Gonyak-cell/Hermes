...
function evaluateStageAcceptance(item, stage) {
  ...
  if (item.acceptance_profile === "expansion_cursor_ledger_gate") {
    if (
      metrics.validation_error_count === 0
      && metrics.failed_checkpoint_count === 0
      && metrics.expansion_cursor_ledger_status === "complete"
      && metrics.phase_slot === "P278"
      && metrics.requested_batch_size > 0
      && metrics.batch_item_position_count > 0
      && metrics.portable_resume_key_complete_count === metrics.batch_item_position_count
      && metrics.cursor_state_row_count >= 6
      && metrics.passed_cursor_state_row_count === metrics.cursor_state_row_count
      && metrics.batch_state_row_count >= 5
      && metrics.passed_batch_state_row_count === metrics.batch_state_row_count
      && metrics.resume_checkpoint_count >= 18
      && metrics.passed_resume_checkpoint_count === metrics.resume_checkpoint_count
      && metrics.path_portability_check_count >= 6
      && metrics.passed_path_portability_check_count === metrics.path_portability_check_count
      && metrics.same_batch_resume_supported === true
      && metrics.interrupted_resume_returns_same_batch_state === true
      && metrics.resume_batch_state_stable === true
      && metrics.cursor_state_stable === true
      && metrics.state_snapshot_hash_stable === true
      && metrics.next_batch_artifact_matches_cursor === true
      && metrics.raw_cursor_material_allowed === false
      && metrics.source_absolute_path_identity_allowed === false
      && metrics.state_path_treated_as_opaque === true
      && metrics.read_only === true
      && metrics.cursor_ledger_report_only === true
      && metrics.backfill_execution_performed === false
      && metrics.source_ingest_performed === false
      && metrics.file_content_read_performed === false
      && metrics.source_mutation_performed === false
  ...
  if (item.acceptance_profile === "extractor_coverage_report_gate") {
    const rowCount = metrics.coverage_item_row_count ?? 0;
    if (
      metrics.validation_error_count === 0
      && metrics.failed_checkpoint_count === 0
      && metrics.extractor_coverage_report_status === "complete"
      && metrics.phase_slot === "P284"
      && metrics.previous_phase_slot === "P283"
      && metrics.next_phase_slot === "P285"
      && metrics.source_extractor_registry_status === "complete"
      && metrics.source_extractor_registry_phase_slot === "P283"
      && metrics.source_extractor_registry_next_phase_slot === "P284"
      && metrics.source_batch_classification_result_status === "complete"
      && metrics.source_batch_classification_phase_slot === "P281"
      && metrics.source_expansion_quarantine_ledger_status === "complete"
      && metrics.source_expansion_quarantine_phase_slot === "P280"
      ...
      && metrics.processed_item_count === rowCount
      && metrics.covered_item_count === rowCount
      && metrics.coverage_missing_item_count === 0
      && metrics.passed_document_type_coverage_row_count === metrics.document_type_coverage_row_count
      && metrics.passed_extension_coverage_row_count === metrics.extension_coverage_row_count
      && metrics.passed_status_coverage_row_count === metrics.status_coverage_row_count
      && (metrics.processing_rate ?? -1) >= 0
      && (metrics.processing_rate ?? 2) <= 1
      && (metrics.coverage_rate ?? -1) >= 0
      && (metrics.coverage_rate ?? 2) <= 1
      && (metrics.failure_rate ?? -1) >= 0
      && (metrics.failure_rate ?? 2) <= 1
      && (metrics.unsupported_rate ?? -1) >= 0
      && (metrics.unsupported_rate ?? 2) <= 1
      && metrics.human_review_required_count === rowCount
      && metrics.client_facing_ready_count === 0
      && metrics.extractor_execution_count === 0
      && metrics.file_content_read_count === 0
      ...
      && metrics.matter_data_write_performed_count === 0
      && metrics.read_only === true
      && metrics.extractor_coverage_report_only === true
      && metrics.source_artifact_read_performed === true
      && metrics.extractor_execution_performed === false
      && metrics.ocr_execution_performed === false
      && metrics.file_content_read_performed === false
  ...
  if (item.acceptance_profile === "approval_queue_ui_gate") {
    const itemCount = metrics.approval_queue_ui_item_count ?? 0;
    if (
      metrics.validation_error_count === 0
      && metrics.failed_checkpoint_count === 0
      && metrics.approval_queue_ui_status === "complete"
      && metrics.phase_slot === "P289"
      && metrics.previous_phase_slot === "P288"
      && metrics.next_phase_slot === "P290"
      && metrics.source_dashboard_information_architecture_status === "complete"
      && metrics.source_dashboard_information_architecture_phase_slot === "P288"
      && metrics.source_dashboard_information_architecture_next_phase_slot === "P289"
      && metrics.approval_queue_ui_panel_count === 5
      && metrics.required_panel_count === 5
      && metrics.ready_panel_count === 5
      && itemCount > 0
      && metrics.source_pending_approval_count === itemCount
      && metrics.pending_approval_item_count === itemCount
      && metrics.missing_required_actor_count === 0
      && metrics.target_artifact_count === itemCount
      && metrics.linked_target_artifact_count === itemCount
      && metrics.receipt_preview_count === itemCount
      && metrics.receipt_preview_available_count === itemCount
      && metrics.protected_request_preview_count === itemCount
      && metrics.actual_protected_request_preview_count > 0
      && metrics.read_only === true
      && metrics.ui_projection_only === true
      && metrics.receipt_preview_only === true
      && metrics.protected_request_preview_only === true
  ...
  if (item.acceptance_profile === "access_review_report_gate") {
    if (
      metrics.validation_error_count === 0
      && metrics.failed_checkpoint_count === 0
      && metrics.access_review_report_status === "complete"
      && metrics.phase_slot === "P302"
      && metrics.previous_phase_slot === "P301"
      && metrics.next_phase_slot === "P303"
      && metrics.source_retention_deletion_policy_status === "complete"
      && metrics.source_retention_deletion_policy_phase_slot === "P301"
      && metrics.source_retention_deletion_policy_next_phase_slot === "P302"
      && metrics.failed_source_status_count === 0
      && metrics.subject_review_count >= 1
      && metrics.matter_access_row_count >= 1
      && metrics.resource_access_row_count >= 1
      && metrics.tenant_count >= 1
      && metrics.matter_count >= 1
      && metrics.user_count >= 1
      && metrics.finding_count >= 8
      && metrics.passed_finding_count === metrics.finding_count
      && metrics.failed_finding_count === 0
      && metrics.gate_result_count >= 10
      && metrics.passed_gate_result_count === metrics.gate_result_count
      && metrics.failed_gate_result_count === 0
      && metrics.gate_violation_count === 0
      && metrics.review_without_human_gate_count === 0
      && metrics.denied_retrievable_count === 0
      && metrics.external_runtime_retrievable_count === 0
      && metrics.missing_retrieval_filter_count === 0
      && metrics.read_only === true
      && metrics.access_review_report_only === true
      && metrics.source_artifact_read_performed === true
      && metrics.source_content_read_performed === false
      && metrics.source_ingest_performed === false
      && metrics.permission_mutation_performed === false
      && metrics.access_grant_performed === false
      && metrics.access_revoke_performed === false
      && metrics.source_mutation_performed === false
      && metrics.external_transfer_performed === false
      && metrics.network_access_performed === false
      && metrics.route_execution_performed === false
  ...
  if (item.acceptance_profile === "personal_dev_e2e_report_gate") {
    if (
      metrics.validation_error_count === 0
      && metrics.failed_checkpoint_count === 0
      && metrics.personal_dev_e2e_report_status === "complete"
      && metrics.phase_slot === "P306"
      && metrics.previous_phase_slot === "P305"
      && metrics.next_phase_slot === "P307"
      && metrics.source_law_firm_e2e_report_status === "complete"
      && metrics.source_law_firm_e2e_report_phase_slot === "P305"
      && metrics.source_law_firm_e2e_report_next_phase_slot === "P306"
      && metrics.failed_source_status_count === 0
      && metrics.scenario_row_count >= 1
      && metrics.passed_scenario_row_count === metrics.scenario_row_count
      && metrics.failed_scenario_row_count === 0
      ...
      && metrics.passed_chain_stage_count >= 7
      && metrics.failed_chain_stage_count === 0
      && metrics.issue_to_audit_path_complete === true
      && metrics.issue_stage_passed_count >= 1
      && metrics.plan_stage_passed_count >= 1
      && metrics.worktree_stage_passed_count >= 1
      && metrics.diff_stage_passed_count >= 1
      && metrics.test_stage_passed_count >= 1
      && metrics.pr_draft_stage_passed_count >= 1
      && metrics.audit_stage_passed_count >= 1
      && metrics.issue_record_count >= 1
      && metrics.normalized_task_count >= 1
      && metrics.issue_task_binding_count >= 1
      && metrics.plan_request_count >= 2
      && metrics.plan_candidate_count >= 2
      && metrics.unresolved_conflict_count === 0
      && metrics.dev_lane_count >= 1
      && metrics.worktree_record_count >= 1
      && metrics.patch_record_count >= 1
      && metrics.diff_review_result_count >= 1
      && metrics.required_test_dimension_count >= 1
      && metrics.passed_required_test_dimension_count === metrics.required_test_dimension_count
      && metrics.pr_draft_output_artifact_count >= 1
      && metrics.gate_violation_count === 0
      && metrics.read_only === true
      && metrics.report_only === true
