...
import {
  DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS,
  buildTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptChainLiveAdapterImportBoundaryFixtures,
} from "./trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures.mjs";

export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_OUT_DIR = "artifacts/trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures/latest";
export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS = {
  ...DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS,
  liveAdapterImportBoundaryFixturesSchemaPath: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.schemaPath,
  ...
  schemaPath: "schemas/trading/trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures.schema.json",
};

const SCHEMA_VERSION = "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures.v1";
const CAPABILITY_ID = "trading.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures";
const PHASE_SLOT = "P475";
const PREVIOUS_PHASE_SLOT = "P474";
const NEXT_PHASE_SLOT = "P476";
const READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_secret_handle_boundary";
const SOURCE_READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary";
const COMMAND_NAME = "trading:secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures";
const WORKFLOW_ID = "workflow.trading.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures.v1";
const REQUIRED_ROW_KEYS = [
  "p474_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_ready",
...
export async function runTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptChainSecretHandleBoundaryFixtures(options = {}) {
  const result = await buildTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptChainSecretHandleBoundaryFixtures(options);
  if (options.write !== false) await writeTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptChainSecretHandleBoundaryFixtures(result, result.output_dir);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Trading secret scan remediation receipt chain secret scan remediation receipt chain secret handle boundary fixtures failed with ${result.validation.errors.length} error(s).`);
...
export async function buildTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptChainSecretHandleBoundaryFixtures(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_OUT_DIR);
  const inputs = normalizeInputs(options);
  const liveAdapterImportBoundary = await buildTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptChainLiveAdapterImportBoundaryFixtures({
    ...options,
    runAt: generatedAt,
  ...
  const sourceReady = liveAdapterImportBoundary.validation.valid && liveAdapterImportBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_status === SOURCE_READY_STATUS;
  ...
  const result = {
    ...
    trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_id: `trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures.${dateStamp(generatedAt)}`,
  ...
  const schemaErrors = schema.available ? validateAgainstSchema(result, schema.data, {}, "trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures") : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  ...
  result.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_id = result.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_id;
  return { ...result, markdown: renderMarkdown(result) };
}

export async function writeTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptChainSecretHandleBoundaryFixtures(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures.json"), serializableResult(result));
  await writeJson(path.join(outDir, "secret-handle-boundary-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-rows.v1", "secret_handle_boundary_rows", result.secret_handle_boundary_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-handle-boundary-gate-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-gate-rows.v1", "secret_handle_boundary_gate_rows", result.secret_handle_boundary_gate_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-handle-boundary-boundary.json"), result.secret_handle_boundary_boundary);
  await writeJson(path.join(outDir, "validation-report.json"), {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures-validation-report.v1",
...
export async function runTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptChainSecretHandleBoundaryFixturesCli(argv = process.argv.slice(2)) {
  ...
  try {
    const result = await runTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptChainSecretHandleBoundaryFixtures(args);
    console.log(`Trading secret scan remediation receipt chain secret scan remediation receipt chain secret handle boundary fixtures ${args.check ? "validated" : "written"} at ${result.output_dir}`);
    console.log(`Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_status}`);
...
function buildSecretHandleBoundaryAnchor(liveAdapterImportBoundary, secretScan) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures-anchor.v1",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_id: liveAdapterImportBoundary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_id,
    source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_status: liveAdapterImportBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_status,
    ...
    source_hash: hashValue({
      id: liveAdapterImportBoundary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_id,
      status: liveAdapterImportBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_status,
...
function buildSecretHandleBoundaryRows({ sourceReady, liveAdapterImportBoundary, executionEngine, researchBacktestPaper, desktopCompanionText, hermesConfigText, secretsScanText, secretScan }) {
  ...
  const rowInputs = [
    secretBoundaryRowInput("p474_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_ready", "secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures", "summary", [
      condition("source.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_status", liveAdapterImportBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_status, SOURCE_READY_STATUS),
      ...
      condition("source.raw_secret_material_materialized", liveAdapterImportBoundary.summary.raw_secret_material_materialized, false),
      condition("source.raw_secret_material_exposed", liveAdapterImportBoundary.summary.raw_secret_material_exposed, false),
      condition("source.desktop_provider_key_visible", liveAdapterImportBoundary.summary.desktop_provider_key_visible, false),
      condition("source.forbidden_receipt_fields_allowed", liveAdapterImportBoundary.summary.forbidden_receipt_fields_allowed, false),
      condition("source.auto_fix_command_registered", liveAdapterImportBoundary.summary.auto_fix_command_registered, false),
      condition("source.auto_redaction_allowed", liveAdapterImportBoundary.summary.auto_redaction_allowed, false),
      condition("source.auto_deletion_allowed", liveAdapterImportBoundary.summary.auto_deletion_allowed, false),
      condition("source.auto_rotation_allowed", liveAdapterImportBoundary.summary.auto_rotation_allowed, false),
    ]),
...
function buildSecretHandleBoundaryRow(input, sourceReady, index) {
  ...
  const row = {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-row.v1",
    secret_handle_boundary_row_id: `trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary.row.${input.rowKey}`,
    phase_slot: PHASE_SLOT,
    ...
    source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_ready: sourceReady,
    ...
    raw_secret_material_materialized: rawSecretFindings.some((finding) => finding.materialized_secret_value === true),
...
function buildSecretHandleBoundary({ generatedAt, writeRequested, liveAdapterImportBoundary, rows, secretScan }) {
  const sourceReady = liveAdapterImportBoundary.validation.valid && liveAdapterImportBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_status === SOURCE_READY_STATUS;
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures-boundary.v1",
    generated_at: generatedAt,
    ...
    raw_secret_material_materialized: liveAdapterImportBoundary.summary.raw_secret_material_materialized === true || secretScan.raw_secret_findings.some((finding) => finding.materialized_secret_value === true),
    ...
    desktop_secret_material_exposed: false,
    desktop_provider_key_visible: liveAdapterImportBoundary.summary.desktop_provider_key_visible === true,
    forbidden_receipt_fields_allowed: liveAdapterImportBoundary.summary.forbidden_receipt_fields_allowed === true || rows.some((row) => row.forbidden_receipt_fields_allowed),
    ...
    auto_fix_command_registered: liveAdapterImportBoundary.summary.auto_fix_command_registered === true || rows.some((row) => row.auto_fix_command_registered),
    auto_redaction_allowed: liveAdapterImportBoundary.summary.auto_redaction_allowed === true || rows.some((row) => row.auto_redaction_allowed),
    auto_deletion_allowed: liveAdapterImportBoundary.summary.auto_deletion_allowed === true || rows.some((row) => row.auto_deletion_allowed),
    auto_rotation_allowed: liveAdapterImportBoundary.summary.auto_rotation_allowed === true || rows.some((row) => row.auto_rotation_allowed),
...
function buildSecretHandleBoundaryGateRows({ liveAdapterImportBoundary, packageJson, platformOpsLedger, sources, rows, boundary }) {
  ...
  const gateInputs = [
    ["p474_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_ready", "P474 secret scan remediation receipt chain secret scan remediation receipt chain live adapter import boundary source is ready.", liveAdapterImportBoundary.validation.valid && liveAdapterImportBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_status === SOURCE_READY_STATUS],
    ["platform_package_script_registered", "package.json registers the P475 trading secret handle boundary fixtures command.", typeof scripts[COMMAND_NAME] === "string" && scripts[COMMAND_NAME].length > 0],
    ["platform_validation_chain_registered", "Validation chain includes the P475 trading secret handle boundary fixtures command.", validateScript.includes(`npm run ${COMMAND_NAME} -- --check`)],
    ["p475_ledger_acceptance_declared", "P475 acceptance row is declared in the platform operations ledger.", ledgerText.includes("P475: `trading:secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures`")],
    ...
    ["no_plaintext_or_provider_key_material", "No plaintext secrets, provider keys, environment dumps, or raw secret material are present.", boundary.raw_secret_finding_count === 0 && boundary.provider_key_finding_count === 0 && boundary.environment_dump_finding_count === 0 && !boundary.raw_secret_material_materialized && !boundary.raw_secret_material_exposed && !boundary.secret_material_exposed],
    ["no_secret_or_trading_mutation", "P475 performs no credential lookup, automatic fix, redaction, deletion, rotation, secret scan remediation action, live execution, trading writes, artifact mutation, release, git, or protected action.", boundary.no_secret_material_read && boundary.no_secret_or_trading_mutation && !boundary.forbidden_receipt_fields_allowed && !boundary.secret_scan_remediation_action_allowed && !boundary.auto_fix_command_registered && !boundary.auto_redaction_allowed && !boundary.auto_deletion_allowed && !boundary.auto_rotation_allowed && !boundary.secret_values_read && !boundary.env_file_read && !boundary.desktop_config_content_inspected && !boundary.desktop_config_read && !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.command_execution_performed && !boundary.artifact_write_performed && !boundary.release_published && !boundary.git_operation_performed && !boundary.protected_action_executed],
  ];
...
function gateRow(rowKey, description, passed) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-gate-row.v1",
    secret_handle_boundary_gate_row_id: `trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures.gate.${rowKey}`,
    phase_slot: PHASE_SLOT,
    row_key: rowKey,
    description,
...
function buildValidationItems({ liveAdapterImportBoundary, packageJson, platformOpsLedger, sources, rows, gateRows, boundary }) {
  return [
    validationItem("source.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary", "p474_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_ready", liveAdapterImportBoundary.validation.valid && liveAdapterImportBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_status === SOURCE_READY_STATUS, "P474 secret scan remediation receipt chain secret scan remediation receipt chain live adapter import boundary fixtures must be ready."),
    validationItem("source.package_json", "package_json_available", packageJson.available, "package.json is readable for P475 secret handle boundary fixtures."),
    ...
    validationItem("secret_handle_boundary_gate_rows", "secret_handle_boundary_gates_ready", gateRows.length >= 10 && gateRows.every((row) => row.gate_status === "ready" && !row.protected_action_executed_by_gate), "P475 secret handle boundary gates are ready."),
    ...
    validationItem("boundary.no_plaintext_secret_material", "no_plaintext_or_provider_key_material", boundary.raw_secret_finding_count === 0 && boundary.provider_key_finding_count === 0 && boundary.environment_dump_finding_count === 0 && !boundary.raw_secret_material_materialized && !boundary.raw_secret_material_exposed && !boundary.secret_material_exposed && !boundary.desktop_secret_material_exposed && !boundary.desktop_provider_key_visible && boundary.no_secret_material_read && !boundary.secret_values_read && !boundary.env_file_read && !boundary.desktop_config_content_inspected && !boundary.desktop_config_read, "Plaintext secrets, provider keys, environment dumps, Desktop secret exposure, raw secret materialization, .env reads, Desktop config reads, and secret-value reads must remain absent."),
    validationItem("boundary.no_mutation", "no_secret_or_trading_mutation", boundary.no_secret_or_trading_mutation && !boundary.forbidden_receipt_fields_allowed && !boundary.secret_scan_remediation_action_allowed && !boundary.auto_fix_command_registered && !boundary.auto_redaction_allowed && !boundary.auto_deletion_allowed && !boundary.auto_rotation_allowed && !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.command_execution_performed && !boundary.package_command_execution_performed && !boundary.release_check_execution_performed && !boundary.artifact_read_performed && !boundary.artifact_write_performed && !boundary.release_published && !boundary.git_operation_performed && !boundary.protected_action_executed, "P475 performs no automatic fix/redaction/deletion/rotation, secret scan remediation action, trading, artifact, release, git, or protected mutation."),
  ];
}

function buildSummary({ liveAdapterImportBoundary, rows, gateRows, boundary, validation }) {
  return {
    trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_status: validation.valid ? READY_STATUS : "blocked",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_status: liveAdapterImportBoundary.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_status,
    source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_ready: boundary.source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_ready,
    required_row_count: REQUIRED_ROW_KEYS.length,
    secret_handle_boundary_row_count: rows.length,
...
function renderMarkdown(result) {
  const lines = [
    "# Trading Secret Scan Remediation Receipt Chain Secret Scan Remediation Receipt Chain Secret Handle Boundary Fixtures",
    "",
    `Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_secret_handle_boundary_fixtures_status}`,
    `Phase: ${result.summary.phase_slot}`,
    `Source secret scan remediation receipt chain secret scan remediation receipt chain live adapter import boundary: ${result.summary.source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_status}`,
...
function parseArgs(argv) {
  const parsed = { outDir: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_OUT_DIR };
...
function printHelp() {
  console.log(`Usage: node scripts/trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-fixtures.mjs [options]

Options:
  --out-dir <folder>                         Output directory. Default: ${DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_OUT_DIR}
  ...
  --live-adapter-import-boundary-fixtures-schema <path>
                                             P474 secret scan remediation receipt chain secret scan remediation receipt chain live adapter import boundary fixtures schema path.
...
function normalizeInputs(options = {}) {
  return {
    package_path: path.resolve(options.packagePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.packagePath),
    platform_ops_ledger_path: path.resolve(options.platformOpsLedgerPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.platformOpsLedgerPath),
    live_adapter_import_boundary_fixtures_schema_path: path.resolve(options.liveAdapterImportBoundaryFixturesSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.liveAdapterImportBoundaryFixturesSchemaPath),
    execution_engine_path: path.resolve(options.executionEnginePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.executionEnginePath),
    research_backtest_paper_path: path.resolve(options.researchBacktestPaperPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.researchBacktestPaperPath),
    desktop_companion_integration_path: path.resolve(options.desktopCompanionIntegrationPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.desktopCompanionIntegrationPath),
    hermes_config_example_path: path.resolve(options.hermesConfigExamplePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.hermesConfigExamplePath),
    secrets_scan_gate_doc_path: path.resolve(options.secretsScanGateDocPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.secretsScanGateDocPath),
    schema_path: path.resolve(options.schemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_HANDLE_BOUNDARY_FIXTURES_INPUTS.schemaPath),
  };
}
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    for (const pattern of RAW_SECRET_PATTERNS) {
      ...
      if (pattern.regex.test(line)) {
        findings.push({
          schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-chain-secret-handle-boundary-raw-secret-finding.v1",
          source_id: sourceId,
