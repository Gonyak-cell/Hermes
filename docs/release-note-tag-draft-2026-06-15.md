...
이 문서는 Hermes desktop candidate commit `1d98ee0d0744cd419d9762bb98f4aad90a24d362` 기준 release note와 tag 초안이다. 실제 tag 생성, tag push, GitHub Release publication, package publication, production deployment는 owner approval 이후에만 수행한다.
...
```bash
git push github v0.1.0-rc.20260615.1d98ee0
```

Production tag after explicit owner production approval:

```bash
git tag -a v0.1.0 1d98ee0d0744cd419d9762bb98f4aad90a24d362 -m "Hermes v0.1.0"
...
```text
1d98ee0d0744cd419d9762bb98f4aad90a24d362
```
...
Hermes `v0.1.0-rc.20260615.1d98ee0` establishes a desktop release-candidate baseline for the Hermes project/workflow operating harness. This RC adds the read-only Hermes Operator Desktop shell and closes the first Claude review hardening loop for desktop preview, CSP, parser, and projection safety.
...
Known current checks for candidate `1d98ee0d0744cd419d9762bb98f4aad90a24d362`:
...
- Fresh Claude final review receipt is captured for this exact candidate: `PASS_WITH_FINDINGS`, `0` blocking findings, `2` P4 hardening notes.
- Owner RC re-freeze decision is not yet recorded for `1d98ee0d...`.
...
- Final `platform:release-check -- --check` envelope remains optional before tag publication unless the owner requires a single umbrella receipt.
...
- Keep `1d98ee0d0744cd419d9762bb98f4aad90a24d362` as a reviewed candidate attempt only.
...
```bash
git tag -d v0.1.0-rc.20260615.1d98ee0
```

If a tag is pushed by mistake:

```bash
git push github :refs/tags/v0.1.0-rc.20260615.1d98ee0
```
