...
## Phase 204 - Secrets Broker Contract

목표: Runtime Adapter/Sandbox/Backend Selector 위에 secrets broker contract를 추가해 agent와 Desktop Companion이 raw secret/provider key를 직접 읽거나 저장하지 못하게 하고, secret access를 handle reference, scoped runtime token, audit receipt, policy snapshot으로만 표현한다. Hermes Desktop은 secret source of truth가 아니라 read-only operator surface로 고정한다.

구현:

- `src/secrets-broker-contract.mjs`와 `scripts/secrets-broker-contract.mjs`를 추가해 `npm run runtime:secrets-broker` slice를 등록
- `schemas/secrets-broker-contract.schema.json`으로 broker contract, secret handle policy, runtime secret access binding, audit binding, Desktop boundary를 검증
- Runtime Adapter Interface v2, Sandbox Policy Model, Docker/local Backend Selector, Tool/Runtime Policy Enforcement, core policy matrix, Desktop Companion 설계 문서를 source contract로 연결
- provider API key, OAuth token, SSH key, signing certificate, database credential, webhook secret, local env secret, human-entered receipt 등 8개 secret handle policy를 locked 상태로 정규화
- Runtime 9개에 대해 Hermes/Claude Code/Codex는 brokered handle only, Local Script/Document Renderer는 handle optional, Harness는 metadata only, Manual은 receipt reference only, Browser/MCP는 blocked로 고정
- Secret audit binding 9개를 생성해 audit event ledger sink, policy snapshot, correlation id, handle ref recording을 요구하고 raw secret/provider key logging을 금지
- Secrets Desktop boundary를 read-only로 고정하고 mutation, protected execution, credential export, local secret store write, installer/gateway/SSH/cron control, source-of-truth 권한을 모두 false로 유지
- Review Dashboard와 Review API에 `/api/secrets-broker-contract`, `/api/secret-handle-policies`, `/api/runtime-secret-access-bindings`, `/api/secret-audit-bindings`, `/api/secrets-desktop-boundary`, `/api/secrets-broker-validations`를 추가
- Contract golden fixture, contract validation suite, control-plane goal checkpoint, control-plane loop에 Secrets Broker Contract를 연결

완료 기준:

- Secrets Broker Contract가 validation error 없이 `complete` 상태가 됨
- broker contract와 broker status가 `locked`이고 secret handle, audit, policy snapshot requirement가 true임
- secret handle policy 8개, runtime secret access binding 9개, secret audit binding 9개가 생성됨
- brokered handle-only runtime 3개, handle optional runtime 2개, blocked runtime secret access 2개가 보존됨
- raw secret material allowed/runtime exposed/Desktop exposed/logged count와 provider key direct access/Desktop visible/logged count가 모두 0임
- protected mutation route는 `protected_action_request_only`이고 secret exception은 human gate를 요구함
- Desktop read-only는 true이고 Desktop mutation/protected execution/secret material/provider key/source-of-truth 권한은 모두 false
- Review API와 dashboard가 Secrets Broker Contract 상태를 read-only로 노출
...
## Phase 207 - Runtime Timeout/Heartbeat

목표: Runtime/AgentRun lifecycle policy의 `timeout_seconds`와 `heartbeat_seconds`를 AgentRun ledger, WorkflowRun ledger, Runtime Log Normalization, in-run timeout gate와 결합해 장기 실행 감시 상태를 run ledger에 남긴다. Runtime self-report를 신뢰하지 않고 harness control plane이 heartbeat/timeout authority가 되며, Hermes Desktop은 lifecycle status를 보는 read-only operator surface로만 유지한다.

구현:

- `src/runtime-timeout-heartbeat.mjs`와 `scripts/runtime-timeout-heartbeat.mjs`를 추가해 `npm run runtime:timeout-heartbeat` slice를 등록
- `schemas/runtime-timeout-heartbeat.schema.json`으로 heartbeat record, timeout record, lifecycle ledger binding, Desktop boundary를 검증
- Runtime/AgentRun Contract Freeze, Agent Run Ledger, Workflow Run Ledger, Runtime Log Normalization, Workflow In-run Gate Framework, Desktop Companion 설계 문서를 source contract로 연결
- AgentRun 6개마다 terminal heartbeat record 6개와 timeout record 6개를 생성하고 lifecycle policy hash, elapsed seconds, timeout deadline, timeout gate count를 기록
- heartbeat/timeout record를 AgentRun record, run ledger id, normalized runtime log, runtime log trace binding, observability/correlation trace에 연결하는 lifecycle ledger binding 6개를 생성
- Runtime Timeout/Heartbeat Desktop boundary를 read-only로 고정하고 heartbeat write, timeout override, cancel/resume, clock edit, runtime start/process control, protected execution, source-of-truth 권한을 모두 false로 유지
- Review Dashboard와 Review API에 `/api/runtime-timeout-heartbeat`, `/api/runtime-heartbeat-records`, `/api/runtime-timeout-records`, `/api/runtime-lifecycle-ledger-bindings`, `/api/runtime-heartbeat-desktop-boundary`, `/api/runtime-timeout-heartbeat-validations`를 추가
- Contract golden fixture, contract validation suite, control-plane goal checkpoint, control-plane loop에 Runtime Timeout/Heartbeat를 연결

완료 기준:

- Runtime Timeout/Heartbeat가 validation error 없이 `complete` 상태가 됨
- lifecycle contract가 `locked`이고 heartbeat/timeout authority가 `harness_control_plane`, source of truth가 `runtime_agentrun_contract_agent_run_ledger_and_runtime_logs`임
- runtime self-report trusted가 false이고 heartbeat record 6개, timeout record 6개, lifecycle ledger binding 6개가 생성됨
- observed heartbeat count가 heartbeat record count와 일치하고 heartbeat missed count가 0임
- timeout policy bound count가 timeout record count와 일치하고 timed out, active timeout, timeout action required count가 모두 0임
- lifecycle ledger binding이 AgentRun ledger, normalized log, observability trace에 모두 bound됨
- Desktop read-only는 true이고 Desktop mutation/protected execution/source-of-truth/heartbeat write/timeout override/cancel/resume/clock edit/runtime start/runtime process control 권한은 모두 false
- Review API와 dashboard가 Runtime Timeout/Heartbeat 상태를 read-only로 노출
...
## Phase 214 - Repo Profile Detector

목표: P214 Personal Dev Domain Pack에서 현재 repository의 language/framework/tooling profile과 test/build/lint command catalog를 deterministic artifact로 고정한다. 이 감지기는 명령을 실행하지 않고, Hermes Desktop에는 read-only operator surface로만 노출한다.

구현:

- `src/repo-profile-detector.mjs`와 `scripts/repo-profile-detector.mjs`를 추가해 `npm run personal-dev:repo-profile` slice를 등록
- `schemas/repo-profile-detector.schema.json`으로 repo profile, language profile, framework profile, command profile, detection signal, Desktop boundary, checkpoint를 검증
- `package.json`, P213 Personal Dev Pack Manifest, final completion ledger, repository file markers를 source contract로 연결
- `npm test`, `npm run dashboard:build`, `npm run validate:core` 기반으로 test/build/lint command profile을 감지 또는 derived 설정으로 기록하되 execution count는 0으로 유지
- Review Dashboard와 Review API에 `/api/repo-profile-detectors`, `/api/repo-profiles`, `/api/repo-profile-languages`, `/api/repo-profile-frameworks`, `/api/repo-profile-commands`, `/api/repo-profile-signals`, `/api/repo-profile-desktop-boundary`, `/api/repo-profile-validations`를 추가
- Contract golden fixture, contract validation suite, control-plane goal checkpoint, control-plane loop에 Repo Profile Detector를 연결

완료 기준:

- Repo Profile Detector가 validation error 없이 `complete` 상태가 됨
- primary language가 `javascript`로 감지되고 Node ESM, node:test, JSON Schema contract, domain pack, npm script harness profile이 기록됨
- test/build/lint command가 각각 `npm test`, `npm run dashboard:build`, `npm run validate:core`로 감지 또는 derived 설정됨
- command execution performed count가 0이며 artifact는 profile 감지만 수행함
- Desktop read-only는 true이고 Desktop mutation/command execution/source-of-truth/secret/provider key/installer/gateway/SSH/cron 권한은 모두 false
- Review API와 dashboard가 Repo Profile Detector 상태와 command catalog를 read-only로 노출
...
## Phase 249 - Contract Draft Workflow

Goal: P249 Law Firm Domain Pack adds a deterministic contract draft workflow that turns Project Alpha negotiation points, captured client positions, LDD RFI questions, and meeting action items into attorney-reviewable contract clause scaffolds. The generated rows remain internal work product only and do not create final clause language, provide legal advice, assert legal conclusions, write matter/task/workflow state, or generate client-facing output.

Implementation:
- Added `src/contract-draft-workflow.mjs`, `scripts/contract-draft-workflow.mjs`, `schemas/contract-draft-workflow.schema.json`, and `docs/contract-draft-workflow.md`.
- Added `law-firm:contract-draft` npm script.
- The artifact emits `contract_draft_rules`, `contract_draft_packets`, `contract_clause_drafts`, `contract_client_positions`, `contract_clause_consistency_checks`, `contract_attorney_review_gates`, `contract_draft_issue_links`, and `contract_draft_matter_summaries`.
- Review Dashboard stage/summary, Review API route/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Completion criteria:
- 6 contract draft workflow rules, 1 draft packet, 2 clause scaffolds, 2 client position rows, 2 consistency checks, 2 attorney review gates, 4 issue links, and 1 matter summary are generated from the Project Alpha deal-control baseline plus P245/P248 context.
- Every clause has a client position, consistency check, attorney review gate, and at least one source issue link.
- Clause text is non-final scaffold language and remains source-referenced, draft-only, attorney/human-review required, partner-approval gated, and not client-facing-ready.
- Legal advice, legal conclusions, contract delivery readiness, client-facing output, matter data write, task state write, workflow transition, runtime execution, delivery execution, protected action, Desktop mutation, and Desktop source-of-truth are all 0/false.
...
## Phase 251 - Legal Approval Matrix

Goal: P251 Law Firm Domain Pack adds a deterministic approval matrix for attorney review and partner approval requirements across law-firm generated outputs. The matrix remains a requirement ledger only: it does not approve, reject, finalize, file, deliver, provide legal advice, assert legal conclusions, write matter/task/workflow state, or generate client-facing output.

Implementation:
- Added `src/legal-approval-matrix.mjs`, `scripts/legal-approval-matrix.mjs`, `schemas/legal-approval-matrix.schema.json`, and `docs/legal-approval-matrix.md`.
- Added `law-firm:approval-matrix` npm script.
- The artifact emits `legal_approval_matrix_rules`, `legal_approval_output_rows`, `legal_approval_requirements`, `legal_approval_gate_links`, `legal_approval_matter_summaries`, and a Desktop read-only boundary.
- Review Dashboard stage/summary, Review API route/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Completion criteria:
- 6 rules, 6 output approval rows, 12 approval requirement rows, 6 native gate links, and 2 matter summaries are generated from LDD RFI Generator, LDD Report Draft, Litigation Brief Draft, Meeting Minutes Workflow, Contract Draft Workflow, and Provided Material Review Ledger context.
- Every output is matter_id-scoped, source-referenced, attorney/human-review required, partner-approval required before client use, and blocked from client/final/delivery/filing use until approval.
- Approval decisions, attorney approvals, partner approvals, final review decisions, legal advice, legal conclusions, client-facing output, matter data write, task state write, workflow transition, runtime execution, delivery execution, protected action, approval-decision write, partner approval bypass, Desktop mutation, and Desktop source-of-truth are all 0/false.
...
## Phase 254 - Template Registry

Goal: P254 establishes the Creative and Document Domain Pack template registry. It records DOCX, PPTX, HTML, and email template metadata, version rows, format coverage, and pack bindings without writing template files, executing renderers, delivering outputs, or generating client-facing material.

Implementation:
- Added `src/creative-document-template-registry.mjs`, `scripts/creative-document-template-registry.mjs`, `schemas/template-registry.schema.json`, and `docs/template-registry.md`.
- Added `creative-document:template-registry` npm script.
- The artifact emits `template_records`, `template_version_records`, `template_format_coverage`, `template_pack_bindings`, a read-only boundary, checkpoint rows, and a validation report.
- Review Dashboard stage/summary, Review API route/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Completion criteria:
- 7 domain-pack template declarations are normalized into template records with metadata hashes.
- DOCX, PPTX, HTML, and email format coverage is complete.
- 7/7 template version rows are current and linked to pack versions plus declared template paths.
- All templates remain metadata-only, human-review gated, format-validation gated, and not client-facing ready.
- Template file writes, core registry mutation, renderer execution, delivery execution, protected action, client-facing output, and Desktop source-of-truth are all 0/false.
- Golden fixture count increased to 156 and `template_registry` is included as a regression fixture.
- `npm run creative-document:template-registry -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 255 - Style Registry

Goal: P255 establishes the Creative and Document Domain Pack style registry. It records deterministic voice, tone, brand, font, and layout metadata for DOCX, PPTX, HTML, and email templates without writing style files, executing renderers, delivering outputs, or generating client-facing material.

Implementation:
- Added `src/creative-document-style-registry.mjs`, `scripts/creative-document-style-registry.mjs`, `schemas/style-registry.schema.json`, and `docs/style-registry.md`.
- Added `creative-document:style-registry` npm script.
- The artifact emits `style_profile_records`, `style_rule_records`, `template_style_bindings`, `style_format_coverage`, a read-only boundary, checkpoint rows, and a validation report.
- Review Dashboard stage/summary, Review API route/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Completion criteria:
- DOCX, PPTX, HTML, and email each have one registered style profile.
- Voice, tone, brand, font, and layout rule types are fully registered.
- 7/7 template style bindings are linked to the Phase 254 template registry.
- All style profiles, style rules, and template bindings remain metadata-only, human-review gated, format-validation gated, and not client-facing ready.
- Style file writes, core registry mutation, renderer execution, delivery execution, protected action, client-facing output, and Desktop source-of-truth are all 0/false.
- Golden fixture count increased to 157 and `style_registry` is included as a regression fixture.
- `npm run creative-document:style-registry -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 256 - Asset Registry

Goal: P256 establishes the Creative and Document Domain Pack asset registry. It records image, logo, graph, table, and video asset metadata as artifact-managed references without media ingestion, binary writes, media generation, renderer execution, delivery, or client-facing output.

Implementation:
- Added `src/creative-document-asset-registry.mjs`, `scripts/creative-document-asset-registry.mjs`, `schemas/asset-registry.schema.json`, and `docs/asset-registry.md`.
- Added `creative-document:asset-registry` npm script.
- The artifact emits `asset_records`, `asset_type_records`, `asset_artifact_policies`, `template_asset_bindings`, `asset_format_coverage`, a read-only boundary, checkpoint rows, and a validation report.
- Review Dashboard stage/summary, Review API route/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Completion criteria:
- Image, logo, graph, table, and video each have one registered asset metadata row.
- Asset artifact policies require metadata artifacts while binary storage, file ingestion, media generation, renderer execution, delivery, and protected actions remain disabled.
- 28 template asset bindings connect the Phase 254 template registry and Phase 255 style registry to asset metadata.
- DOCX, PPTX, HTML, and email format coverage is complete.
- All assets and bindings require source attribution, license review, accessibility text, format validation, and human review.
...
## Phase 258 - PPTX Renderer

Goal: P258 establishes the Creative and Document Domain Pack PPTX renderer workflow. It converts registered PPTX template metadata into deterministic slide templates, slide deck manifests, OpenXML parts, overflow checks, and draft PPTX output artifacts while preserving attorney review, no-legal-advice, no external runtime execution, no delivery, and no client-facing readiness gates.

Implementation:
- Added `src/creative-document-pptx-renderer.mjs`, `scripts/creative-document-pptx-renderer.mjs`, `schemas/pptx-renderer.schema.json`, and `docs/pptx-renderer.md`.
- Added `creative-document:pptx-renderer` npm script.
- The artifact emits `pptx_render_jobs`, `pptx_slide_templates`, `pptx_slide_decks`, `pptx_openxml_parts`, `pptx_output_artifacts`, `pptx_overflow_checks`, `pptx_format_validation_results`, a deterministic renderer boundary, checkpoint rows, and a validation report.
- Review Dashboard stage/summary, Review API route/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Completion criteria:
- 2 PPTX templates from the Phase 254 registry are rendered into 2 deterministic draft PPTX artifacts.
- 2 slide templates and 2 slide deck manifests are generated from read-only registry metadata.
- 26 OpenXML parts are generated with stable hashes.
- 2 overflow checks pass title, bullet, slide-count, and review-note constraints.
- PPTX binary outputs are written only inside the artifact output directory and have captured hashes.
- PPTX format validation passes with required package parts, ZIP signature, slide parts, human review note, no external relationships, and draft gate checks.
...
## Phase 260 - Layout Validator

Goal: P260 establishes the Creative and Document Domain Pack layout validator. It checks the deterministic DOCX, PPTX, HTML, and PDF draft artifacts produced by the document renderers for page count, overflow, and broken table conditions while preserving the Windows baseline stabilization posture and review-only legal safety gates.

Implementation:
- Added `src/creative-document-layout-validator.mjs`, `scripts/creative-document-layout-validator.mjs`, `schemas/layout-validator.schema.json`, and `docs/layout-validator.md`.
- Added `creative-document:layout-validator` npm script.
- The artifact reads DOCX, PPTX, and PDF/HTML renderer outputs as read-only sources and emits `layout_targets`, `layout_validation_results`, `layout_validation_checks`, checkpoint rows, and a validation report.
- Review Dashboard stage/summary, Review API route/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Completion criteria:
- 7 layout targets are produced from 1 DOCX, 2 PPTX, 2 HTML, and 2 PDF draft artifacts.
- 7 layout validation results pass with 7/7 page count checks, 7/7 overflow checks, and 7/7 broken table checks.
- 42 total layout validation checks pass across page count, overflow, broken table, source hash, review gate, and safety gate checks.
- The validator remains report-only: no renderer/runtime execution, no network access, no delivery, no protected action, no legal advice, and no client-facing readiness.
- Golden fixture count increased to 162 and `layout_validator` is included as a regression fixture.
- `npm run creative-document:layout-validator -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 261 - Citation Renderer

Goal: P261 establishes the Creative and Document Domain Pack citation renderer. It renders deterministic internal citation rows for footnotes, exhibit references, and source span links against already validated document layout targets while preserving the Windows baseline stabilization posture and all legal review gates.

Implementation:
- Added `src/creative-document-citation-renderer.mjs`, `scripts/creative-document-citation-renderer.mjs`, `schemas/citation-renderer.schema.json`, and `docs/citation-renderer.md`.
- Added `creative-document:citation-renderer` npm script.
- The artifact reads Source Span Store, Citation Object Store, Exhibit Map, and Layout Validator outputs as read-only sources and emits `citation_render_units`, `footnote_renderings`, `exhibit_reference_renderings`, `source_span_link_renderings`, `citation_render_packets`, checkpoint rows, and a validation report.
- Review Dashboard stage/summary, Review API route/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Completion criteria:
- 80 source-bound citations are rendered into 80 internal citation render units.
- 80 draft footnotes, 80 bound exhibit references, and 80 bound source span links are produced.
- 7 citation render packets cover the Phase 260 layout targets for DOCX, PPTX, HTML, and PDF draft artifacts.
- The renderer remains report-only: no document runtime mutation, no external renderer execution, no network access, no delivery, no protected action, no legal advice, and no client-facing readiness.
...
## Phase 263 - Design System Profile

Goal: P263 establishes the Creative and Document Domain Pack design-system profile for PPTX report materials. It connects PPTX template/style/asset rules to renderer output and version-review packets while preserving the Windows baseline stabilization posture and all legal review gates.

Implementation:
- Added `src/creative-document-design-system-profile.mjs`, `scripts/creative-document-design-system-profile.mjs`, `schemas/design-system-profile.schema.json`, and `docs/design-system-profile.md`.
- Added `creative-document:design-system-profile` npm script.
- The artifact reads Template Registry, Style Registry, Asset Registry, PPTX Renderer, Version Comparator, and the PPTX design-system capability manifest as read-only sources and emits `design_system_profiles`, `design_system_rules`, `template_design_bindings`, `asset_design_bindings`, `design_review_packets`, checkpoint rows, and a validation report.
- Review Dashboard stage/summary, Review API route/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Completion criteria:
- 1 PPTX design-system profile is complete for the registered creative presentation style profile.
- 9 design rules cover voice, tone, brand, font, layout, color palette, spacing, asset treatment, and review gates.
- 2 PPTX template design bindings, 10 asset design bindings, and 2 design review packets are ready for attorney review.
- The profile remains metadata-only and report-only: no template/style/asset mutation, no document runtime mutation, no renderer execution, no external renderer execution, no network access, no delivery, no protected action, no legal advice, and no client-facing readiness.
- Golden fixture count increased to 165 and `design_system_profile` is included as a regression fixture.
- `npm run creative-document:design-system-profile -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 264 - Web Novel Workflow

Goal: P264 establishes the Creative and Document Domain Pack web novel workflow. It turns the creative-document brief into deterministic draft creative writing artifacts while preserving the Windows baseline stabilization posture and all human-review/no-legal-advice gates.

Implementation:
- Added `src/creative-document-web-novel-workflow.mjs`, `scripts/creative-document-web-novel-workflow.mjs`, `schemas/web-novel-workflow.schema.json`, and `docs/web-novel-workflow.md`.
- Added `creative-document:web-novel-workflow` npm script.
- The artifact reads Creative Document Brief, Creative Document Pack Manifest, Template Registry, Style Registry, Design System Profile, and Output Delivery Freeze as read-only sources and emits `web_novel_workflows`, `web_novel_synopses`, `web_novel_style_guides`, `web_novel_chapters`, `web_novel_revision_packets`, `web_novel_output_artifacts`, checkpoint rows, a validation report, and a draft markdown manuscript.
- Review Dashboard stage/summary, Review API route/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Completion criteria:
- 1 workflow row, 1 draft synopsis, 1 draft style guide, 3 draft chapters, 3 human-review revision packets, and 1 draft markdown output artifact are generated.
- All generated creative rows remain human-review and format-validation gated, source attribution is required for the output artifact, and no client-facing readiness is asserted.
- The workflow remains draft-only and deterministic: no external model execution, no network access, no template/style/asset/runtime mutation, no renderer execution, no delivery, no protected action, no legal advice, and no client-facing output generation.
...
## Phase 266 - Creative Document Freeze

Goal: P266 freezes the Creative and Document Domain Pack by proving the P253-P265 document, presentation, and content-production artifacts remain stable on the Windows baseline before connector work begins.

Implementation:
- Added `src/creative-document-freeze.mjs`, `scripts/creative-document-freeze.mjs`, `schemas/creative-document-freeze.schema.json`, and `docs/creative-document-freeze.md`.
- Added `creative-document:freeze` npm script.
- The artifact reads the P253-P265 creative-document sources plus Gate Approval and Output Delivery freezes as read-only inputs and emits `creative_document_freeze_sources`, `creative_document_freeze_paths`, `creative_document_freeze_gates`, `creative_document_freeze_boundary`, checkpoint rows, validation report, freeze note, and summary.
...
Completion criteria:
- 13 source rows, 3 representative document/presentation/content path rows, and 6 freeze gate rows are generated with no validation errors.
- Render/format validation, layout validation, citation/version review, design/content review, and approval/delivery boundary checks all pass.
- The freeze remains read-only and report-only: no source/template/style/asset/runtime mutation, no renderer execution, no media/network/model execution, no delivery, no protected action, no legal advice, and no client-facing readiness.
- Golden fixture count increased to 168 and `creative_document_freeze` is included as a regression fixture.
- `npm run creative-document:freeze -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 267 - Connector Contract v2

Goal: P267 establishes Connector Contract v2 before real connector ingestion begins. It preserves the Windows baseline stabilization posture by defining common connector source identity, cursor, external id, and authorization boundaries without executing any connector, reading credentials, mutating resources, or producing client-facing output.

Implementation:
- Added `src/connector-contract-v2.mjs`, `scripts/connector-contract-v2.mjs`, `schemas/connector-contract-v2.schema.json`, and `docs/connector-contract-v2.md`.
- Added `connectors:contract-v2` npm script.
- The artifact reads Resource Contract Freeze, Resource Store Interface, Policy Matrix Catalog, Tool Runtime Policy Enforcement, Creative Document Freeze, and source code contract surfaces as read-only inputs and emits `connector_interface_schema`, `connector_definitions`, `connector_source_contracts`, `connector_cursor_contracts`, `connector_external_id_contracts`, `connector_auth_boundaries`, checkpoint rows, validation report, summary, and markdown summary.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Completion criteria:
- 8 connector family contracts are defined for local folder, OneDrive, Outlook email, KakaoTalk export, GitHub, VDR, Plaud transcript, and ERP draft.
- Every connector has a stable source_id, resumable cursor contract, external_id namespace, auth boundary, matter boundary, classification gate, policy snapshot gate, and human-review gate.
- Auth boundaries are credential-reference-only and least-privilege; raw secret material, connector execution, external network access, source ingest, resource mutation, output delivery, protected action, legal advice, and client-facing output generation remain disabled.
- Golden fixture count increased to 169 and `connector_contract_v2` is included as a regression fixture.
- `npm run connectors:contract-v2 -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 268 - Local Folder Connector

Goal: P268 implements the first Connector and Ingestion Layer adapter by turning configured local folders into read-only, resumable discovery and ingest-candidate artifacts while preserving the Windows baseline stabilization posture and human-review/no-legal-advice gates.

Implementation:
- Added `src/local-folder-connector.mjs`, `scripts/local-folder-connector.mjs`, `schemas/local-folder-connector.schema.json`, `docs/local-folder-connector.md`, and deterministic fixture roots under `examples/local-folder-connector/`.
- Added `connectors:local-folder` npm script.
- The artifact reads Connector Contract v2, binds `connector.local_folder.v2` / `source.local_folder.v2`, scans only configured local path allowlist roots, writes resource-expansion cursor state, and emits local folder discovery rows, ingest records, cursor state, auth boundary, validation report, and summary.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Completion criteria:
- Local file discovery runs against configured local folder roots and creates deterministic external ids and external version ids for each discovered file.
- Ingest rows are resumable and human-review gated; duplicate content is skipped with duplicate lineage and cursor state records `last_seen_external_id`, high-watermark, and resume token hash.
- The connector remains path-allowlisted and read-only: no source mutation, external network access, credential material read, resource mutation, output delivery, protected action, legal advice, or client-facing output generation.
...
## Phase 271 - KakaoTalk Import Boundary

Goal: P271 implements the KakaoTalk import boundary as a deterministic adapter for operator-provided KakaoTalk text exports and attachment manifests. It preserves the Windows baseline stabilization posture by projecting chat messages, attachments, and conversation membership into review-gated resource candidates without executing the KakaoTalk app, calling live chat APIs, using network access, reading credentials, mutating sources/resources, or producing client-facing output.

Implementation:
- Added `src/kakaotalk-import-boundary.mjs`, `scripts/kakaotalk-import-boundary.mjs`, `schemas/kakaotalk-import-boundary.schema.json`, `docs/kakaotalk-import-boundary.md`, and deterministic fixture exports under `examples/kakaotalk-import-boundary/`.
- Added `connectors:kakaotalk-import-boundary` npm script.
- The artifact reads Connector Contract v2 and the completed P270 Outlook Email Connector baseline, binds `connector.kakaotalk_export.v2` / `source.kakaotalk_export.v2`, and emits chat message resource rows, attachment resource rows, conversation rows, cursor state, auth boundary, validation report, and summary.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Verification:
- KakaoTalk message rows include export id, chat room id, source line offset, author/date/time metadata, external ids, resource ids, classification, audit trail, and human-review gates.
- Attachment rows are manifest-driven metadata-only candidates linked to parent message lines and conversations.
- Conversation rows group message and attachment membership under stable chat room ids.
- Cursor state uses `export_line_offset_cursor` with hash-only resume token storage and no raw export cursor material.
- Auth boundary is operator-provided-export, credential-free, local-export-read-only, and review-gated.
- The boundary performs no KakaoTalk app execution, live API execution, external network access, credential read, source mutation, resource mutation, delivery, protected action, legal advice, or client-facing output.
- Golden fixture count increased to 173 and `kakaotalk_import_boundary` is included as a regression fixture.
- `npm run connectors:kakaotalk-import-boundary -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 272 - GitHub Connector

Goal: P272 implements the GitHub connector as a deterministic adapter for operator-provided repository export JSON. It preserves the Windows baseline stabilization posture by projecting repositories, issues, pull requests, commits, reviews, and workflow input candidates into review-gated resource rows without calling the live GitHub API, using network access, reading credentials, mutating GitHub/source/resource state, pushing branches, merging, releasing, delivering output, or producing legal/client-facing output.

Implementation:
- Added `src/github-connector.mjs`, `scripts/github-connector.mjs`, `schemas/github-connector.schema.json`, `docs/github-connector.md`, and deterministic fixture exports under `examples/github-connector/`.
- Added `connectors:github` npm script.
- The artifact reads Connector Contract v2 and the completed P271 KakaoTalk Import Boundary baseline, binds `connector.github.v2` / `source.github.v2`, and emits repository, issue, pull request, commit, review, workflow input, cursor, auth boundary, validation, and summary artifacts.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Verification:
- Issues, pull requests, commits, and reviews become human-review-gated resource candidates with external ids, external version ids, resource ids, resource version ids, metadata completeness, audit trails, and workflow input rows.
- Pull request review rows are linked back to parent pull request resources.
- Cursor state uses `repo_event_since_cursor` with hash-only resume token storage and no raw since-cursor material.
- Auth boundary is app-installation/PAT read-only, credential-reference-only, and least-privilege scoped.
- The connector performs no live GitHub API execution, external network access, credential material read, source mutation, resource mutation, issue/PR/repository mutation, branch push, merge, release, workflow state mutation, delivery, protected action, legal advice, or client-facing output.
- Golden fixture count increased to 174 and `github_connector` is included as a regression fixture.
- `npm run connectors:github -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 273 - VDR Connector

Goal: P273 implements the VDR connector as a deterministic adapter for operator-provided virtual data room export JSON. It preserves the Windows baseline stabilization posture by projecting VDR room, index, document, version, permission boundary, and resource expansion seed rows into review-gated resource candidates without calling a live VDR API, using network access, reading credentials, downloading document content, mutating permissions, mutating source/resource state, or producing legal/client-facing output.

Implementation:
- Added `src/vdr-connector.mjs`, `scripts/vdr-connector.mjs`, `schemas/vdr-connector.schema.json`, `docs/vdr-connector.md`, and deterministic fixture exports under `examples/vdr-connector/`.
- Added `connectors:vdr` npm script.
- The artifact reads Connector Contract v2 plus the completed P272 GitHub Connector and LDD VDR Inventory baselines, binds `connector.vdr.v2` / `source.vdr.v2`, and emits room, index, document, version, permission boundary, resource expansion seed, cursor, auth boundary, validation, and summary artifacts.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Verification:
- VDR index and document rows become human-review-gated resource candidates with external ids, external version ids, resource ids, resource version ids, metadata completeness, audit trails, and permission boundary links.
- Resource expansion seed rows link each VDR document candidate to enforced permission boundaries and require human review before content materialization.
- LDD VDR inventory linkage is preserved for known file records so resource expansion can reconcile source inventory with VDR metadata.
- Cursor state uses `vdr_index_revision_cursor` with hash-only resume token storage and no raw index revision cursor material.
- Auth boundary is service-account read-only, credential-reference-only, and least-privilege scoped.
- The connector performs no live VDR API execution, external network access, credential material read, document content read/download, source mutation, resource mutation, permission mutation, resource expansion mutation, matter data write, task state write, workflow transition, delivery, protected action, legal advice, or client-facing output.
- Golden fixture count increased to 175 and `vdr_connector` is included as a regression fixture.
- `npm run connectors:vdr -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 274 - Plaud Transcript Connector

Goal: P274 implements the Plaud transcript connector as a deterministic adapter for operator-provided transcript export JSON. It preserves the Windows baseline stabilization posture by projecting recording metadata, speakers, timestamped transcript segments, normalized text rows, timestamp spans, and audio metadata into review-gated resource candidates without calling a live Plaud API, using network access, reading credential material, downloading audio, mutating source/resource/normalized-text state, or producing legal/client-facing output.

Implementation:
- Added `src/plaud-transcript-connector.mjs`, `scripts/plaud-transcript-connector.mjs`, `schemas/plaud-transcript-connector.schema.json`, `docs/plaud-transcript-connector.md`, and deterministic fixture exports under `examples/plaud-transcript-connector/`.
- Added `connectors:plaud-transcript` npm script.
- The artifact reads Connector Contract v2 plus the completed P273 VDR Connector and normalized text contract baselines, binds `connector.plaud_transcript.v2` / `source.plaud_transcript.v2`, and emits recording, speaker, transcript segment, normalized text, timestamp span, audio metadata, cursor, auth boundary, validation, and summary artifacts.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Verification:
- Transcript segments become human-review-gated `meeting_transcript` resource candidates with external ids, external version ids, resource ids, resource version ids, speaker links, timestamp ranges, and metadata completeness.
- Normalized text rows store both speaker labels and timestamp ranges for each transcript segment.
- Timestamp span rows bind every segment to millisecond offsets, and audio metadata rows become `audio_metadata` resource candidates without audio download.
- Cursor state uses `transcript_timestamp_cursor` with hash-only resume token storage and no raw timestamp cursor material.
- Auth boundary is operator-export/OAuth read-only, credential-reference-only, and does not require runtime network access.
- The connector performs no live Plaud API execution, external network access, credential material read, audio download, source mutation, resource mutation, normalized-text mutation, matter data write, task state write, workflow transition, delivery, protected action, legal advice, or client-facing output.
...
## Phase 276 - Connector Freeze

Goal: P276 freezes the P267-P275 Connector and Ingestion Layer as a deterministic, read-only operational report. It proves that Connector Contract v2 and the Local Folder, OneDrive, Outlook Email, KakaoTalk Import, GitHub, VDR, Plaud Transcript, and ERP Draft connector artifacts are complete, validation-clean, representative-ingest verified, cursor-resumable, credential-reference-only, and mutation/delivery/client-facing safe.

Implementation:
- Added `src/connector-freeze.mjs`, `scripts/connector-freeze.mjs`, `schemas/connector-freeze.schema.json`, and `docs/connector-freeze.md`.
- Added `connectors:freeze` npm script.
- The artifact reads Connector Contract v2 plus the completed P268-P275 connector artifacts and emits connector freeze source rows, representative ingest path rows, gate rows, a freeze boundary, checkpoints, validation, freeze-note, and summary artifacts.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Verification:
- Connector Contract v2 covers 8 contracted connector families with source, cursor, external-id, and auth-boundary contracts.
- Seven freeze paths pass: connector contract fixture coverage plus six representative source ingest paths for local files, OneDrive cloud boundary, communications exports, GitHub/VDR resources, Plaud transcript normalized text, and ERP draft approval holds.
- All eight connector runtime artifacts expose resumable cursor state without raw cursor material and remain credential-reference-only with no credential material read.
- The freeze report performs no connector runtime execution, source ingest, network access, source/resource/billing mutation, matter/task/workflow write, delivery, protected action, legal advice, or client-facing output.
- Golden fixture count increased to 178 and `connector_freeze` is included as a regression fixture.
- `npm run connectors:freeze -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 277 - Backfill Job Contract

Goal: P277 locks the Resource Expansion backfill job contract as a deterministic, read-only schema and field validation report before any Phase 278+ cursor/batch execution work. It preserves the Windows baseline stabilization posture by proving Mac/Windows completion differences cannot destabilize completed phases.

Implementation:
- Added `src/backfill-job-contract.mjs`, `scripts/backfill-job-contract.mjs`, `schemas/backfill-job-contract.schema.json`, and `docs/backfill-job-contract.md`.
- Added `resource:backfill-job-contract` npm script.
- The artifact reads Resource Expansion Job, Resource Expansion schema, Connector Freeze, package, ledger, roadmap, loop, dashboard, and API sources as read-only inputs and emits backfill job schema, field requirements, source bindings, cursor contracts, batch contracts, count contracts, policy bindings, boundary, validation, and summary artifacts.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Verification:
- The contract validates 24/24 required job/source/cursor/batch/count/policy snapshot fields.
- Source bindings, cursor contracts, batch contracts, count contracts, and policy bindings all pass while remaining human-review gated and not client-facing.
- Raw cursor material is disallowed, policy snapshot binding is required, and Windows baseline stability plus the Mac/Windows completion instability guard are explicitly preserved.
- The report performs no backfill execution, source ingest, file content read, source/resource mutation, delivery execution, protected action, legal advice, or client-facing output.
- Golden fixture count increased to 179 and `backfill_job_contract` is included as a regression fixture.
- `npm run resource:backfill-job-contract -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 278 - Expansion Cursor Ledger

Goal: P278 locks the Resource Expansion cursor and batch-state ledger before any Phase 279+ backfill execution work. It keeps the accepted Windows baseline posture by making resume identity portable across Mac/Windows roots and by treating absolute paths as reference-only metadata.

Implementation:
- Added `src/expansion-cursor-ledger.mjs`, `scripts/expansion-cursor-ledger.mjs`, `schemas/expansion-cursor-ledger.schema.json`, and `docs/expansion-cursor-ledger.md`.
- Added `resource:expansion-cursor-ledger` npm script.
- The artifact reads Resource Expansion Job, Resource Expansion state, next-batch, Backfill Job Contract, package, ledger, roadmap, loop, dashboard, and API sources as read-only inputs and emits cursor state rows, batch state rows, resume checkpoints, batch item positions, path portability checks, boundary, validation, and summary artifacts.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Verification:
- Cursor state, batch state, resume checkpoint, and path portability rows all pass while remaining human-review gated and not client-facing.
- Portable resume keys are derived from source id, relative path, size, and modified timestamp; raw cursor material and absolute source path identity are disallowed.
- Interruption/resume is validated to return the same deterministic batch state, with the state path treated as opaque and the next-batch artifact matching the queued resume window.
- The report performs no backfill execution, source ingest, file content read, source/resource/state mutation, delivery execution, protected action, legal advice, or client-facing output.
...
## Phase 280 - Expansion Quarantine Ledger

Goal: P280 locks Resource Expansion failure and quarantine classification before later classification/matter tagging backfill work. It preserves the Windows baseline posture by proving quarantine decisions are based on expansion status/reason metadata and portable item identity, not absolute Mac/Windows path identity.

Implementation:
- Added `src/expansion-quarantine-ledger.mjs`, `scripts/expansion-quarantine-ledger.mjs`, `schemas/expansion-quarantine-ledger.schema.json`, and `docs/expansion-quarantine-ledger.md`.
- Added `resource:expansion-quarantine-ledger` npm script.
- The artifact reads Resource Expansion Job, Expansion Cursor Ledger, Expansion Dedup Ledger, Backfill Job Contract, package, ledger, roadmap, loop, dashboard, and API sources as read-only inputs and emits quarantine rule rows, quarantine decision rows, quarantine hold rows, quarantine status audits, quarantine resume checks, boundary, validation, and summary artifacts.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Verification:
- Failed, sensitive/secret, dataless/materialization-required, oversized, unsupported, and unknown expansion items are classified into human-review quarantine hold categories.
- Held rows block retrieval, external transfer, and output delivery, cannot auto-release, and preserve quarantine/failure status history.
- Hold identity does not use absolute source paths.
- The report performs no backfill execution, extraction retry, source ingest, file content read, source/resource/state mutation, quarantine release, delivery execution, protected action, legal advice, or client-facing output.
- Golden fixture count increased to 182 and `expansion_quarantine_ledger` is included as a regression fixture.
- `npm run resource:expansion-quarantine-ledger -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 281 - Batch Classification Result

Goal: P281 locks batch classification coverage after the Windows baseline stabilization and P280 quarantine posture. It proves Resource Expansion items can be reported with classification, confidence, policy binding, and human-review status without starting Phase 217+ work from an unstable Mac/Windows path basis.

Implementation:
- Added `src/batch-classification-result.mjs`, `scripts/batch-classification-result.mjs`, `schemas/batch-classification-result.schema.json`, and `docs/batch-classification-result.md`.
- Added `resource:batch-classification` npm script.
- The artifact reads Resource Expansion Job, Expansion Cursor Ledger, Expansion Dedup Ledger, Expansion Quarantine Ledger, Data Classification Rule Engine, Backfill Job Contract, package, ledger, roadmap, loop, dashboard, and API sources as read-only inputs and emits classification rule rows, batch classification rows, confidence rows, policy binding rows, checks, validation, and summary artifacts.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Verification:
- Every Resource Expansion item has a classification row with data classification, confidence score, confidence label, policy binding, and pending human-review status.
- Confidence rows pass at the 0.7 floor and no low-confidence or missing-confidence rows remain.
- Policy binding rows pass for all classified resources while client-facing readiness stays at 0.
- The report performs no backfill execution, classification write, source ingest, source file content read, external model use, source/resource/state mutation, delivery execution, protected action, legal advice, or client-facing output.
...
## Phase 283 - Extractor Registry

Goal: P283 locks the extractor registry after P282 matter-tagging separation. It proves document types, extensions, extractor chains, fact schemas, citation rules, confidence policy, fallback policy, and per-resource compatibility rows are cataloged before any extractor execution or OCR work is allowed.

Implementation:
- Added `src/extractor-registry.mjs`, `scripts/extractor-registry.mjs`, and `docs/extractor-registry.md`.
- Added `resource:extractor-registry` npm script.
- The artifact reads Extractor Adapter Contract, Resource Expansion Job, Batch Classification Result, Batch Matter Tagging Result, package, ledger, roadmap, loop, dashboard, and API sources as read-only inputs and emits registry entries, extractor compatibility rows, document-type rows, extension rows, checks, validation, and summary artifacts.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests were wired to the new artifact.

Verification:
- Every registry entry declares document type, classification rules, extractor chain, fact schema, cross-document rules, citation rules, confidence policy, fallback policy, and human review gating.
- Every Resource Expansion item receives a compatibility row linked to P281 classification and P282 pending human-confirmation matter-tagging posture.
- Registry and compatibility rows remain local-only and block external services/network access.
- The report performs no extractor execution, OCR execution, file content read, external model use, source ingest, source/resource/state/matter mutation, delivery execution, protected action, legal advice, or client-facing output.
- Golden fixture count increased to 185 and `extractor_registry` is included as a regression fixture.
- `npm run resource:extractor-registry -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 284 - Extractor Coverage Report

Phase 284 adds `extractor_coverage_report`, a read-only coverage report over Resource Expansion, P280 quarantine decisions, P281 batch classification, and the P283 Extractor Registry. The goal is to make document type, extension, status, failure/hold, and unsupported-type coverage explicit before any extractor execution.

Implementation notes:

- Added `src/extractor-coverage-report.mjs`, `scripts/extractor-coverage-report.mjs`, `schemas/extractor-coverage-report.schema.json`, and `docs/extractor-coverage-report.md`.
- Added `resource:extractor-coverage` npm script.
- The artifact emits `extractor_coverage_item_rows`, `document_type_coverage_rows`, `extension_coverage_rows`, `status_coverage_rows`, `unsupported_type_rows`, `extractor_failure_rows`, and `extractor_coverage_checks`.
- The report keeps extractor execution, OCR execution, source file content reads, external services, network access, mutation, protected action, legal advice, and client-facing output at zero/false.
- Dashboard, API, control-plane loop, goal checkpoint, contract validation, golden fixture, and test harness surfaces now include `extractor_coverage_report`.
...
## Phase 287 - API Route Inventory

Phase 287 adds `api_route_inventory`, a read-only inventory over the live Review API route index. It groups all GET routes into core, review, evidence, policy, runtime, and desktop_companion route groups while preserving the Windows baseline posture after P286.

Changes:

- Added `src/api-route-inventory.mjs`, `scripts/api-route-inventory.mjs`, `schemas/api-route-inventory.schema.json`, and `docs/api-route-inventory.md`.
- Added `api:route-inventory` npm script.
- The inventory emits `api-route-inventory.json`, route group rows, route records, a read-only boundary, checks, validation report, and summary markdown under `artifacts/api-route-inventory/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `api_route_inventory`.
- The inventory is read-only and does not execute routes, start a server, mutate state, execute protected actions, generate legal advice, or produce client-facing output.
- Golden fixture count increased to 189 and `api_route_inventory` is included as a regression fixture.
- `npm run api:route-inventory -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 288 - Review Dashboard Information Architecture

Phase 288 adds `review_dashboard_ia`, a read-only information architecture map over the P287 API Route Inventory. It organizes every Review API route into Overview, Domain Packs, Capabilities, Runs, Approvals, Evidence, Policies, Cost, and Diagnostics navigation sections without building the dashboard or executing routes.

Changes:

- Added `src/review-dashboard-ia.mjs`, `scripts/review-dashboard-ia.mjs`, `schemas/review-dashboard-ia.schema.json`, and `docs/review-dashboard-ia.md`.
- Added `dashboard:ia` npm script.
- The IA emits `review-dashboard-ia.json`, section rows, navigation items, route bindings, boundary, checks, validation report, and summary markdown under `artifacts/review-dashboard-ia/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `dashboard_information_architecture`.
- The IA artifact is read-only and does not build the dashboard, execute routes, start a server, mutate state, execute protected actions, generate legal advice, or produce client-facing output.
- Golden fixture count increased to 190 and `dashboard_information_architecture` is included as a regression fixture.
- `npm run dashboard:ia -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 289 - Approval Queue UI

Phase 289 adds `approval_queue_ui`, a read-only UI projection over Approval Queue, Approval Inbox, Human Gate Receipt Drafts, and Protected Approval Request Pack. It makes pending approvals usable from the dashboard/API surface by exposing required actor, target artifact lookup, receipt draft preview, and protected request preview rows without applying approvals, applying receipts, or executing protected actions.

Changes:

- Added `src/approval-queue-ui.mjs`, `scripts/approval-queue-ui.mjs`, `schemas/approval-queue-ui.schema.json`, and `docs/approval-queue-ui.md`.
- Added `approval:queue-ui` npm script.
- The UI projection emits `approval-queue-ui.json`, panel rows, UI item rows, target artifact lookup rows, receipt preview rows, protected request preview rows, boundary, checks, validation report, and summary markdown under `artifacts/approval-queue-ui/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `approval_queue_ui`.
- The artifact is read-only and preview-only and does not apply approvals, apply receipts, execute protected actions, execute routes, start a server, mutate state, generate legal advice, or produce client-facing output.
- Golden fixture count increased to 191 and `approval_queue_ui` is included as a regression fixture.
- `npm run approval:queue-ui -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 290 - Evidence Viewer UI

Phase 290 adds `evidence_viewer_ui`, a read-only UI projection over Evidence Viewer Data API, Citation Object Store, Evidence Coverage Score, and Evidence Flags. It makes evidence cards inspectable from the dashboard/API surface by joining source span previews, citation rows, coverage dimensions, and review flags without reading source files, ingesting sources, approving citations, mutating evidence, or delivering output.

Changes:

- Added `src/evidence-viewer-ui.mjs`, `scripts/evidence-viewer-ui.mjs`, `schemas/evidence-viewer-ui.schema.json`, and `docs/evidence-viewer-ui.md`.
- Added `evidence:viewer-ui` npm script.
- The UI projection emits `evidence-viewer-ui.json`, panel rows, evidence card rows, source span preview rows, citation rows, coverage rows, flag rows, boundary, checks, validation report, and summary markdown under `artifacts/evidence-viewer-ui/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `evidence_viewer_ui`.
- The artifact is read-only and preview-only and does not read source file content, ingest sources, mutate evidence, approve citations, deliver output, execute routes, start a server, mutate state, execute protected actions, generate legal advice, or produce client-facing output.
- Golden fixture count increased to 192 and `evidence_viewer_ui` is included as a regression fixture.
- `npm run evidence:viewer-ui -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 291 - Source Span Inspector

Phase 291 adds `source_span_inspector`, a read-only inspector over Source Span Store, Normalized Text Contract, Fact Claim Store, and Evidence Viewer UI. It compares source locator/location-unit metadata, normalized text preview metadata, and extracted fact claims without reading source files, reading normalized text objects, ingesting sources, mutating facts, or producing output.

Changes:

- Added `src/source-span-inspector.mjs`, `scripts/source-span-inspector.mjs`, `schemas/source-span-inspector.schema.json`, and `docs/source-span-inspector.md`.
- Added `evidence:source-span-inspector` npm script.
- The inspector emits `source-span-inspector.json`, panel rows, one inspector row per source span, source location comparison rows, normalized text comparison rows, extracted fact comparison rows, boundary, checks, validation report, and summary markdown under `artifacts/source-span-inspector/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `source_span_inspector`.
- The artifact is read-only and preview-only and does not read source file content, read normalized text objects, ingest sources, mutate facts, deliver output, execute routes, start a server, mutate state, execute protected actions, generate legal advice, or produce client-facing output.
- Golden fixture count increased to 193 and `source_span_inspector` is included as a regression fixture.
- `npm run evidence:source-span-inspector -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 292 - Run Ledger Viewer

Phase 292 adds `run_ledger_viewer`, a read-only Desktop session/progress/history projection over Workflow Run Ledger, Agent Run Ledger, Tool Invocation Ledger, Audit Event Ledger, Ledger API Dashboard, and the P291 Source Span Inspector guard. It joins workflow runs, agent runs, tool invocations, event-backed history, and log/artifact references without reading log content, reading artifact content, replaying events, executing routes, starting a server, mutating ledgers, applying approvals, delivering output, generating legal advice, or producing client-facing output.

Changes:

- Added `src/run-ledger-viewer.mjs`, `scripts/run-ledger-viewer.mjs`, `schemas/run-ledger-viewer.schema.json`, and `docs/run-ledger-viewer.md`.
- Added `ledgers:run-viewer` npm script.
- The viewer emits `run-ledger-viewer.json`, panel rows, Desktop session rows, run progress rows, run history rows, agent activity rows, tool activity rows, log/artifact reference rows, boundary, checks, validation report, and summary markdown under `artifacts/run-ledger-viewer/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `run_ledger_viewer`.
- The artifact is read-only and preview-only and does not read log content, read artifact content, ingest sources, replay events, execute routes, start a server, mutate state, execute protected actions, apply approvals, deliver output, generate legal advice, or produce client-facing output.
...
## Phase 294 - Policy Violation Queue

Phase 294 adds `policy_violation_queue`, a read-only Desktop operator queue over Policy Operations Surface, Matter Access Policy, Model Policy Enforcement, Tool/Runtime Policy Enforcement, Output Destination Policy Enforcement, and the P293 Matter Cockpit UI guard. It turns model, tool/runtime, access, and output policy blocks or holds into queue items and actor action rows without reading source content, ingesting sources, mutating policy, applying approvals or receipts, executing protected actions, delivering output, executing routes, starting a server, generating legal advice, or producing client-facing output.

Changes:

- Added `src/policy-violation-queue.mjs`, `scripts/policy-violation-queue.mjs`, `schemas/policy-violation-queue.schema.json`, and `docs/policy-violation-queue.md`.
- Added `policy:violation-queue` npm script.
- The queue emits `policy-violation-queue.json`, panel rows, queue item rows, actor action rows, boundary, checks, validation report, and summary markdown under `artifacts/policy-violation-queue/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `policy_violation_queue`.
- The artifact is read-only and preview-only and does not read source content, ingest sources, mutate policy, apply approvals or receipts, execute protected actions, deliver output, execute routes, start a server, generate legal advice, or produce client-facing output.
...
## Phase 297 - Threat Model Refresh

Phase 297 adds `threat_model_refresh`, a read-only threat model refresh for the Windows-stabilized Desktop/API baseline. It joins the P296 Dashboard/API Freeze guard, Workflow Prompt Injection Boundary, Secrets Broker Contract, Runtime API Dashboard, Runtime Freeze, Capability Registry API, Dev Protected Scan, and Control Plane Loop so Phase 217 and later work can continue on this Windows machine without Mac/Windows completion drift.

Changes:

- Added `src/threat-model-refresh.mjs`, `scripts/threat-model-refresh.mjs`, `schemas/threat-model-refresh.schema.json`, and `docs/threat-model-refresh.md`.
- Added `security:threat-model` npm script.
- The refresh emits `threat-model-refresh.json`, source rows, risk rows, control rows, evidence rows, boundary, checks, validation report, and summary markdown under `artifacts/threat-model-refresh/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `threat_model_refresh`.
- The artifact tracks prompt injection, data leak, over-agency, insecure tool, Desktop installer, Desktop auto-update, Desktop SSH, Desktop cron, Desktop gateway, and provider key risks while preserving no source content reads, no source ingest, no agent invocation, no tool execution, no route execution, no server start, no Desktop mutation, no installer/auto-update/SSH/cron/gateway control, no raw secret/provider key materialization, no protected action execution, no delivery execution, no legal advice, no client-facing output, human-review gates, and Windows baseline stability.
- Golden fixture count increased to 199 and `threat_model_refresh` is included as a regression fixture.
- `npm run security:threat-model -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 298 - Prompt Injection Test Suite

Phase 298 adds `prompt_injection_test_suite`, a deterministic synthetic fixture suite that verifies external document instructions are treated only as evidence content and are never promoted into prompt, tool, policy, or raw execution instructions. It builds on the P297 Threat Model Refresh guard and the P186 Workflow Prompt Injection Boundary without reading source content, ingesting sources, invoking agents, executing tools, executing routes, starting a server, transferring data, executing protected actions, generating legal advice, or producing client-facing output.

Changes:

- Added `src/prompt-injection-test-suite.mjs`, `scripts/prompt-injection-test-suite.mjs`, `schemas/prompt-injection-test-suite.schema.json`, and `docs/prompt-injection-test-suite.md`.
- Added `security:prompt-injection-tests` npm script.
- The suite emits `prompt-injection-test-suite.json`, fixture rows, result rows, instruction promotion check rows, boundary, validation report, and summary markdown under `artifacts/prompt-injection-test-suite/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `prompt_injection_test_suite`.
- The suite includes at least ten synthetic external-document instruction fixtures and confirms zero prompt instruction promotion, tool instruction promotion, policy override, raw instruction execution, client-facing output, external transfer, or protected action execution.
- Golden fixture count increased to 200 and `prompt_injection_test_suite` is included as a regression fixture.
- `npm run security:prompt-injection-tests -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 299 - External Model Policy Audit

Phase 299 adds `external_model_policy_audit`, a read-only audit report that compares classification-level external provider transmission policy and Desktop provider/model settings against policy snapshot evidence. It builds on the P298 Prompt Injection Test Suite guard, Policy Matrix Catalog, Policy Snapshot Ledger, Model Routing Ledger, Model Policy Enforcement, Runtime API Dashboard, and Capability Registry API without reading source content, ingesting sources, executing an external model, sending provider requests, using network access, mutating Desktop settings, materializing provider keys, executing routes, starting a server, executing protected actions, delivering output, generating legal advice, or producing client-facing output.

Changes:

- Added `src/external-model-policy-audit.mjs`, `scripts/external-model-policy-audit.mjs`, `schemas/external-model-policy-audit.schema.json`, and `docs/external-model-policy-audit.md`.
- Added `security:external-model-policy-audit` npm script.
- The audit emits `external-model-policy-audit.json`, classification audit rows, policy snapshot audit rows, route audit rows, Desktop provider/model audit rows, boundary, validation report, and summary markdown under `artifacts/external-model-policy-audit/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `external_model_policy_audit`.
- The audit verifies classification-level provider transmission policy, policy snapshot alignment, route-level external transfer authorization, P3-P5 external provider forbiddance, Desktop provider key invisibility, and Desktop non-execution/non-source-of-truth posture.
...
## Phase 301 - Retention Deletion Policy

Phase 301 adds `retention_deletion_policy`, a read-only retention/deletion hardening report that records resource, artifact, and audit retention periods and keeps every deletion path blocked behind records review, human review, and active deletion holds. It builds on the P300 Secrets Scan Gate, P173 Retention/Archive Ledger, Resource Contract Freeze, Resource Version Ledger, Immutable Object Store Layout, Output Delivery Contract Freeze, Audit Event Ledger, Append-only Event Store, and Evidence Plane Freeze without reading source content, ingesting sources, deleting records, mutating sources, transferring externally, executing routes, starting a server, executing protected actions, generating legal advice, or producing client-facing output.

Changes:

- Added `src/retention-deletion-policy.mjs`, `scripts/retention-deletion-policy.mjs`, `schemas/retention-deletion-policy.schema.json`, and `docs/retention-deletion-policy.md`.
- Added `compliance:retention-deletion-policy` npm script.
- The policy emits `retention-deletion-policy.json`, source status rows, policy rows, deletion hold records, gate result rows, boundary, validation report, and summary markdown under `artifacts/retention-deletion-policy/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `retention_deletion_policy`.
- The policy records resource/artifact/audit retention rows with positive retention periods, active deletion holds, no delete-after dates, no deletion-authorized policies, no deletion execution, records-review and human-review gates, and Windows baseline stability.
...
## Phase 303 - Performance/Cost Budget Report

Phase 303 adds `performance_cost_budget_report`, a read-only batch/workflow/runtime performance, cost, and token budget report. It builds on the P302 Access Review Report, Cost/Observability Dashboard, Cost Budget Ledger, Workflow Run Ledger, Runtime Freeze, and Control Plane Loop without mutating budgets or costs, writing metrics, executing runtime/workflow/batch/route/server actions, transferring externally, reading source content, ingesting sources, generating legal advice, or producing client-facing output.

Changes:

- Added `src/performance-cost-budget-report.mjs`, `scripts/performance-cost-budget-report.mjs`, `schemas/performance-cost-budget-report.schema.json`, and `docs/performance-cost-budget-report.md`.
- Added `compliance:performance-cost-budget-report` npm script.
- The report emits `performance-cost-budget-report.json`, source rows, performance budget rows, cost budget rows, gate result rows, boundary, validation report, and summary markdown under `artifacts/performance-cost-budget/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `performance_cost_budget_report`.
- Control Plane Loop now rebuilds a P287-P295 dashboard surface immediately before `dashboard_api_freeze`, so the P296-P303 hardening chain remains complete after loop regeneration instead of inheriting the reduced pre-health dashboard.
- The report verifies batch, workflow, and runtime scopes have performance and cost/token budget rows; all observed duration, cost, and token values remain within report limits; budget mutation, cost mutation, metric writes, runtime/workflow/batch execution, runtime control, route execution, server start, protected action, external transfer, network access, legal advice, and client-facing output are all blocked; human-review and Windows baseline stability guards are preserved.
...
## Phase 305 - Law Firm E2E Report

Phase 305 adds `law_firm_e2e_report`, a read-only representative law-firm scenario report for the final acceptance envelope. It verifies the matter->resource->evidence->draft->citation->approval->audit chain after the P304 Backup/Restore Drill baseline without recording approval decisions, mutating source artifacts, executing workflows/runtimes/delivery, generating legal advice or legal conclusions, or producing client-facing output.

Changes:

- Added `src/law-firm-e2e-report.mjs`, `scripts/law-firm-e2e-report.mjs`, `schemas/law-firm-e2e-report.schema.json`, and `docs/law-firm-e2e-report.md`.
- Added `law-firm:e2e-report` npm script.
- The report emits `law-firm-e2e-report.json`, source rows, scenario rows, matter/resource/evidence/draft/citation/approval/audit chain stage rows, gate result rows, boundary, validation report, and summary markdown under `artifacts/law-firm-e2e-report/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `law_firm_e2e_report`.
- The report verifies the P304 backup/restore baseline, existing law-firm E2E freeze paths, resource contract freeze, evidence contract freeze, LDD report draft, citation object store, legal approval matrix, and audit event ledger as read-only sources.
- Golden fixture count increased to 207 and `law_firm_e2e_report` is included as a regression fixture.
- `npm run law-firm:e2e-report -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:api-freeze -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.

## Phase 306 - Personal Dev E2E Report

Phase 306 adds `personal_dev_e2e_report`, a read-only representative personal-dev scenario report for the final acceptance envelope. It verifies the issue->plan->worktree->diff->test->PR draft->audit chain after the P305 Law Firm E2E Report Windows baseline without mutating issues, writing task state, executing commands, creating PRs, calling GitHub, pushing branches, merging, releasing, executing rollback, applying patches, invoking external agents, generating legal advice, or producing client-facing output.

Changes:

- Added `src/personal-dev-e2e-report.mjs`, `scripts/personal-dev-e2e-report.mjs`, `schemas/personal-dev-e2e-report.schema.json`, and `docs/personal-dev-e2e-report.md`.
- Added `personal-dev:e2e-report` npm script.
- The report emits `personal-dev-e2e-report.json`, source rows, scenario rows, issue/plan/worktree/diff/test/PR draft/audit chain stage rows, gate result rows, boundary, validation report, and summary markdown under `artifacts/personal-dev-e2e-report/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `personal_dev_e2e_report`.
- The report verifies the P305 Law Firm E2E Report baseline, P230 Personal Dev E2E Freeze, issue intake adapter, plan request contract, plan reconciliation, dev lane ledger, implementation patch capture, diff review gate, canonical test matrix, PR draft artifact, and audit event ledger as read-only sources.
...
## Phase 309 - Deployment Runbook

Phase 309 adds `deployment_runbook`, a deterministic read-only deployment runbook for the final acceptance envelope. It documents local, dev, prod-like dry-run, optional Desktop Companion, and rollback lanes after the P308 Ingestion E2E Report Windows baseline without executing deployments, server starts, API routes, Desktop installers/gateways, rollback, restore, commands, protected actions, legal advice, or client-facing output.

Changes:

- Added `src/deployment-runbook.mjs`, `scripts/deployment-runbook.mjs`, `schemas/deployment-runbook.schema.json`, and `docs/deployment-runbook.md`.
- Added `deployment:runbook` npm script.
- The runbook emits `deployment-runbook.json`, source rows, environment rows, command rows, checklist rows, rollback procedure rows, gate result rows, boundary, validation report, and summary markdown under `artifacts/deployment-runbook/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `deployment_runbook`.
- The runbook verifies the P308 Ingestion E2E Report baseline, Dashboard/API Freeze, Backup/Restore Drill, Runtime Freeze, Control Plane Loop, and Rollback Plan Artifact as read-only sources.
...
## Phase 312 - Hermes Harness v1.0 Freeze

Phase 312 adds `v1_freeze`, a deterministic read-only Hermes Harness v1.0 freeze report for the final acceptance envelope. It records the P311 release candidate, dashboard/API freeze, contract golden fixtures, contract validation suite, control-plane checkpoint/loop, operator handbook, human-review backlog visibility, and tag/release checklist without creating tags, publishing releases, executing commands, tests, routes, servers, deployment, recovery, rollback, restore, protected actions, delivery, legal advice, or client-facing output.

Changes:

- Added `src/v1-freeze.mjs`, `scripts/v1-freeze.mjs`, `schemas/v1-freeze.schema.json`, and `docs/v1-freeze.md`.
- Added `release:freeze` npm script.
- The report emits `v1-freeze.json`, source rows, checklist rows, gate rows, boundary, validation report, and summary markdown under `artifacts/v1-freeze/latest`.
- Review Dashboard stage/summary, Review API routes/filter/smoke, Control Plane Goal Checkpoint/Loop, Contract Golden Fixtures/Validation Suite, and matter harness tests now include `v1_freeze`.
- The freeze preserves pending approval visibility, human/attorney/release gates, Desktop read-only/not-source-of-truth posture, Windows baseline stability, and the Mac/Windows completion instability guard.
- Golden fixture count increased to 214 and `v1_freeze` is included as a regression fixture.
- `npm run release:freeze -- --check`, schema validation, `npm test`, `npm run validate`, `npm run contracts:inventory`, `npm run contracts:dependencies -- --check`, `npm run api:route-inventory`, `npm run dashboard:ia`, `npm run dashboard:api-freeze -- --check`, `npm run release:candidate -- --check`, `npm run contracts:golden-fixtures -- --check`, `npm run contracts:validate -- --check`, `npm run dashboard:build`, `npm run api:smoke`, `npm run control-plane:goal-checkpoint`, `npm run control-plane:loop`, and `git diff --check` passed on the current Windows baseline.
...
| P287-P296 | API, Dashboard, Evidence Viewer, Matter Cockpit | API Route Inventory와 Review Dashboard Information Architecture를 시작점으로 API server, review dashboard, Desktop-ready route group, approval queue, evidence viewer, source span inspector, run ledger viewer, matter cockpit, policy violation queue를 usable UI로 연결한다. |
...
## Phase 375 - Release-Check Receipt Workspace Merge

Phase 375 declares future merge rows for the P374 release-check receipt
workspace. It records actor-workspace expectations for each reviewer row without
reading actor workspace files, materializing merged receipt input, receiving
receipt payloads, validating receipts, marking rows ready for validation,
completing signoff, applying approvals, running release checks, reading or
writing artifacts, publishing releases, or mutating release/trading state.

Changes:

- Added `src/platform-release-check-receipt-workspace-merge.mjs`,
  `scripts/platform-release-check-receipt-workspace-merge.mjs`,
  `schemas/platform-release-check-receipt-workspace-merge.schema.json`, and
  `docs/platform-release-check-receipt-workspace-merge.md`.
- Added `platform:release-check-receipt-workspace-merge` and registered
  `platform:release-check-receipt-workspace-merge -- --check` in the validation
  chain.
- The command consumes the P374 receipt workspace in memory and produces merge
  rows, gate rows, a boundary report, validation output, and a Markdown summary
  when not in `--check` mode.
- Added focused tests for merge rows, missing validation-chain registration,
