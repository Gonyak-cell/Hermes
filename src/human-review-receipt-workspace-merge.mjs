...
export const DEFAULT_HUMAN_REVIEW_RECEIPT_WORKSPACE_MERGE_OUT_DIR = "artifacts/human-review-receipt-workspace-merge/latest";
export const DEFAULT_HUMAN_REVIEW_RECEIPT_WORKSPACE_MERGE_WORKSPACE_PATH = "artifacts/human-review-receipt-workspace/latest/human-review-receipt-workspace.json";
...
export async function runHumanReviewReceiptWorkspaceMerge(options = {}) {
  const result = await buildHumanReviewReceiptWorkspaceMerge(options);
  if (options.write !== false) await writeHumanReviewReceiptWorkspaceMerge(result, result.output_dir);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Human review receipt workspace merge failed with ${result.validation.errors.length} error(s).`);
...
export async function buildHumanReviewReceiptWorkspaceMerge(options = {}) {
  const outputDir = path.resolve(options.outDir ?? DEFAULT_HUMAN_REVIEW_RECEIPT_WORKSPACE_MERGE_OUT_DIR);
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const workspacePath = path.resolve(options.workspacePath ?? DEFAULT_HUMAN_REVIEW_RECEIPT_WORKSPACE_MERGE_WORKSPACE_PATH);
  ...
  const merge = {
    schema_version: "human-review-receipt-workspace-merge.v1",
    generated_at: generatedAt,
    merge_id: `human-review-receipt-workspace-merge.${dateStamp(generatedAt)}`,
    ...
    sources: [
      buildSource("human_review_receipt_workspace", "Human Review Receipt Workspace", workspacePath, workspaceResult),
    ],
...
export async function writeHumanReviewReceiptWorkspaceMerge(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "human-review-receipt-workspace-merge.json"), {
...
export async function runHumanReviewReceiptWorkspaceMergeCli(argv = process.argv.slice(2)) {
  ...
  try {
    const result = await runHumanReviewReceiptWorkspaceMerge(args);
    console.log(`Human review receipt workspace merge ${args.check ? "validated" : "written"} at ${result.output_dir}`);
...
async function readActorInputs(actorWorkspaces) {
  return Promise.all((actorWorkspaces ?? []).map(async (workspace) => {
    ...
    return {
      actor_input_id: `human-review-receipt-workspace-merge.actor.${slugify(workspace.required_actor)}`,
      ...
      errors: result.ok ? [] : [{ path: `actor_inputs.${workspace.required_actor}`, message: `Actor receipt input unavailable: ${result.error}` }],
...
function buildMergeItems(workspace, actorInputs) {
  const expectedEntries = workspace?.workspace_entries ?? [];
  const expectedByReceipt = new Map(expectedEntries.map((entry) => [entry.receipt_id, entry]));
...
function buildMergeItem({ actorInput, receipt, expected, duplicateActors }) {
  ...
  const errors = [];
  if (!expected) {
    errors.push({ path: `actor_inputs.${actorInput.required_actor}.${receipt.receipt_id}.receipt_id`, message: "Actor receipt input contains a receipt outside the current workspace." });
  } else {
    ...
    if (expected.gate_item_id !== receipt.gate_item_id) {
      errors.push({ path: `actor_inputs.${actorInput.required_actor}.${receipt.receipt_id}.gate_item_id`, message: "Receipt gate item does not match the workspace entry." });
    }
    if (expected.source_plan_item_id !== receipt.source_plan_item_id) {
      errors.push({ path: `actor_inputs.${actorInput.required_actor}.${receipt.receipt_id}.source_plan_item_id`, message: "Receipt source plan item does not match the workspace entry." });
    }
  }
  if (duplicateActors.length > 1) {
    errors.push({ path: `actor_inputs.${receipt.receipt_id}`, message: `Receipt appears in multiple actor inputs: ${duplicateActors.join(", ")}.` });
  }
  if (receiptStatus !== "pending" && !APPLY_RECEIPT_STATUSES.has(receiptStatus)) {
    errors.push({ path: `actor_inputs.${actorInput.required_actor}.${receipt.receipt_id}.receipt_status`, message: "Receipt status must be pending or an applyable terminal status before validation." });
  }
  return {
    merge_item_id: `human-review-receipt-workspace-merge.${expected ? slugify(expected.gate_item_id) : `unknown.${slugify(receipt.receipt_id)}`}`,
    ...
    workspace_entry_id: expected?.workspace_entry_id ?? null,
...
function buildMissingMergeItem(expected) {
  const errors = [{ path: `workspace_entries.${expected.workspace_entry_id}.receipt_id`, message: "Expected workspace receipt was not found in any actor input." }];
  return {
    merge_item_id: `human-review-receipt-workspace-merge.missing.${slugify(expected.gate_item_id)}`,
    actor_input_id: null,
    actor_workspace_id: null,
    required_actor: expected.required_actor,
    workspace_entry_id: expected.workspace_entry_id,
...
function validateMerge({ workspaceResult, actorInputs, mergeItems }) {
  ...
  if (!workspaceResult.ok) {
    errors.push({ path: "sources.human_review_receipt_workspace", message: `Human review receipt workspace unavailable: ${workspaceResult.error}` });
  }
  if (workspaceResult.value?.safe_handling?.auto_execute_allowed) {
    errors.push({ path: "human_review_receipt_workspace.safe_handling.auto_execute_allowed", message: "Workspace merge requires auto execution to remain disabled." });
  }
  ...
  for (const item of mergeItems) {
    if (item.safe_handling.auto_execute_allowed || item.safe_handling.protected_actions_executed) {
      errors.push({ path: `merge_items.${item.merge_item_id}.safe_handling`, message: "Workspace merge must not execute protected actions." });
    }
...
function buildMergedReceiptInput(generatedAt, workspace, mergeItems) {
  ...
  const expectedOrder = workspace?.workspace_entries?.map((entry) => entry.receipt_id) ?? [];
  ...
  return {
    ...
    generated_at: generatedAt,
    human_gate_id: workspace?.actor_workspaces?.find((actor) => actor.receipt_input?.human_gate_id)?.receipt_input?.human_gate_id ?? "control-plane-human-gates.unknown",
    instructions: "Merged from actor-specific human review receipt workspaces. Validate this file before applying receipts. It does not execute protected actions.",
...
function summarizeMerge(actorInputs, mergeItems, workspace, validation) {
  return {
    ...
    expected_receipt_count: workspace?.summary?.receipt_row_count ?? workspace?.workspace_entries?.length ?? 0,
...
function renderMergeMarkdown(merge) {
  const lines = [
    ...
    "## Safe Handling",
    "",
    "- This merge only combines actor receipt inputs.",
...
function normalizeOutcome(outcome) {
  return String(outcome ?? "missing").trim() || "missing";
}
...
function printHelp() {
  console.log(`Usage: node scripts/human-review-receipt-workspace-merge.mjs [options]
