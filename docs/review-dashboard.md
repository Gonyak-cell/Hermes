...
Phase 138부터 dashboard는 Source Span Store도 읽는다. whole document/page/paragraph/line/char range source span, locator, location unit, extractor binding, timestamp not-applicable status, validation error count가 summary와 stage status에 노출된다.
...
Phase 142부터 dashboard는 Citation Object Store도 읽는다. review-pending output paragraph, citation object, paragraph-source binding, citation review queue, source span binding, matter/classification/policy preservation, client-facing readiness, validation error count가 summary와 stage status에 노출된다.
...
npm run dashboard:build -- \
  ...
  --human-review-cycle-completion-runbook artifacts/human-review-cycle-receipt-completion-runbook/latest/human-review-cycle-receipt-completion-runbook.json \
...
- Human Review Cycle Receipt Completion Command Receipt Application의 applied command receipt, patched command queue, audit event 상태
- Human Review Cycle Receipt Completion Reconciliation의 pending command receipt, held command, actor follow-up 상태
...
- Lineage Graph Builder의 source/evidence/fact/issue/output node, canonical edge, complete path, citation-bound 상태
...
- Search Index Contract의 source collection, manifest, required filter field, held query plan, non-executable retrieval boundary 상태
