...
사람이 실제 전달, export, merge를 수행한 뒤 `receipt_input_draft.receipts`의 `receipt_status`, `executed_by`, `executed_at`, `delivery_reference`, `notes`를 채운다. 그 다음 `npm run delivery:closeout:validate`로 검증하고 `npm run delivery:closeout:apply`로 검증 완료 receipt만 ledger에 적용한다.
