...
import {
  ...
  runReceiptWorkbenchOperatorQueueStaticShellHandoff,
} from "../src/receipt-workbench-operator-queue-static-shell-handoff.mjs";
import { buildReceiptWorkbenchOperatorQueueLocalUiBindingSmoke } from "../src/receipt-workbench-operator-queue-local-ui-binding-smoke.mjs";

const RUN_AT = "2026-06-07T21:36:15.872Z";

test("P28800 opens P28801 handoff when P28400 source is ready and static shell execution stays closed", async () => {
  const source = await buildP28400Source({ ready: true });
  const result = await buildReceiptWorkbenchOperatorQueueStaticShellHandoff({
    runAt: RUN_AT,
    receiptWorkbenchOperatorQueueLocalUiBindingSmoke: source,
    commitRef: "abc1234",
  });

  assert.equal(result.schema_version, "receipt-workbench-operator-queue-static-shell-handoff.v1");
  assert.equal(result.program_range, "P28401-P28800");
  assert.equal(result.validation.valid, true);
  assert.equal(result.summary.receipt_workbench_operator_queue_static_shell_handoff_status, "ready_for_receipt_workbench_operator_queue_static_shell_handoff");
  assert.equal(result.summary.ready_for_p28801_handoff, true);
  ...
  assert.equal(result.receipt_workbench_operator_queue_static_shell_handoff_boundary.p28800_contract_ready, true);
  assert.equal(result.receipt_workbench_operator_queue_static_shell_handoff_boundary.ready_for_p28801_handoff, true);

  for (const flag of STATIC_SHELL_HANDOFF_FALSE_FLAGS) {
    assert.equal(result.summary[flag] ?? result.receipt_workbench_operator_queue_static_shell_handoff_boundary[flag], false, `${flag} must stay false`);
  }
});

test("P28800 remains a valid visible block when P28400 source handoff is not ready", async () => {
  const source = await buildP28400Source({ ready: false });
  const result = await buildReceiptWorkbenchOperatorQueueStaticShellHandoff({
    runAt: RUN_AT,
    receiptWorkbenchOperatorQueueLocalUiBindingSmoke: source,
    commitRef: "abc1234",
  ...
  assert.equal(result.validation.valid, true);
  assert.equal(result.summary.receipt_workbench_operator_queue_static_shell_handoff_status, "valid_block_receipt_workbench_operator_queue_static_shell_handoff_pending");
  assert.equal(result.summary.source_p28400_ready_for_p28401_handoff, false);
  assert.equal(result.summary.ready_for_p28801_handoff, false);
  assert.equal(result.receipt_workbench_operator_queue_static_shell_handoff_boundary.p28800_contract_ready, true);
  assert.equal(result.receipt_workbench_operator_queue_static_shell_handoff_boundary.ready_for_p28801_handoff, false);
  assert.equal(result.p28800_clean_checkpoint_rows.find((row) => row.row_id === "p28800_checkpoint.p28801_handoff_blocker_visible").current_verdict, "pass");
});
...
test("blocked copy surface is visible but advisory only", async () => {
  ...
  for (const flag of ALL_FALSE_FLAGS) {
    assert.equal(result.receipt_workbench_operator_queue_static_shell_handoff_boundary[flag], false, `${flag} must stay false`);
  }
});

test("check mode validates without writing static shell handoff artifacts", async () => {
  const outDir = await mkdtemp(path.join(os.tmpdir(), "hermes-p28800-"));
  try {
    const source = await buildP28400Source({ ready: true });
    const result = await runReceiptWorkbenchOperatorQueueStaticShellHandoff({
      check: true,
      outDir,
      runAt: RUN_AT,
      receiptWorkbenchOperatorQueueLocalUiBindingSmoke: source,
    ...
    assert.equal(result.validation.valid, true);
    await assert.rejects(readFile(path.join(outDir, "receipt-workbench-operator-queue-static-shell-handoff.json"), "utf8"), /ENOENT/);
  } finally {
    await rm(outDir, { force: true, recursive: true });
  }
});

async function buildP28400Source({ ready }) {
  const source = await buildReceiptWorkbenchOperatorQueueLocalUiBindingSmoke({
    runAt: RUN_AT,
    write: false,
    commitRef: "abc1234",
  });
  source.summary.ready_for_p28401_handoff = ready;
  source.summary.source_p28000_ready_for_p28001_handoff = true;
  source.receipt_workbench_operator_queue_local_ui_binding_smoke_boundary.ready_for_p28401_handoff = ready;
  source.receipt_workbench_operator_queue_local_ui_binding_smoke_boundary.source_p28000_ready_for_p28001_handoff = true;
