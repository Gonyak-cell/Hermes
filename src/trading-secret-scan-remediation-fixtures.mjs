...
import {
  DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS,
  buildTradingSecretScanFailClosedFixtures,
} from "./trading-secret-scan-fail-closed-fixtures.mjs";

export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_OUT_DIR = "artifacts/trading-secret-scan-remediation-fixtures/latest";
export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_INPUTS = {
  ...DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS,
  secretScanFailClosedSchemaPath: DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.schemaPath,
  ...
  schemaPath: "schemas/trading/trading-secret-scan-remediation-fixtures.schema.json",
};

const SCHEMA_VERSION = "trading-secret-scan-remediation-fixtures.v1";
const CAPABILITY_ID = "trading.secret_scan_remediation_fixtures";
const PHASE_SLOT = "P428";
const PREVIOUS_PHASE_SLOT = "P427";
const NEXT_PHASE_SLOT = "P429";
const READY_STATUS = "ready_for_trading_secret_scan_remediation";
const SOURCE_READY_STATUS = "ready_for_trading_secret_scan_fail_closed";
const COMMAND_NAME = "trading:secret-scan-remediation-fixtures";
const REQUIRED_ROW_KEYS = [
  "p427_secret_scan_fail_closed_ready",
...
export async function runTradingSecretScanRemediationFixtures(options = {}) {
  const result = await buildTradingSecretScanRemediationFixtures(options);
  if (options.write !== false) await writeTradingSecretScanRemediationFixtures(result, result.output_dir);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Trading secret scan remediation fixtures failed with ${result.validation.errors.length} error(s).`);
...
export async function buildTradingSecretScanRemediationFixtures(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_OUT_DIR);
  const inputs = normalizeInputs(options);
  const secretScanFailClosed = await buildTradingSecretScanFailClosedFixtures({
    ...options,
    runAt: generatedAt,
    packagePath: inputs.package_path,
    platformOpsLedgerPath: inputs.platform_ops_ledger_path,
    secretScanAttentionSchemaPath: inputs.secret_scan_attention_schema_path,
    secretScanFailClosedSchemaPath: inputs.secret_scan_fail_closed_schema_path,
    schemaPath: inputs.secret_scan_fail_closed_schema_path,
    write: false,
  });
  ...
  const sourceReady = secretScanFailClosed.validation.valid && secretScanFailClosed.summary.trading_secret_scan_fail_closed_fixtures_status === SOURCE_READY_STATUS;
  const coverage = buildRemediationCoverage({ secretScanFailClosed, sources });
  const anchor = buildRemediationAnchor(secretScanFailClosed, coverage);
  const rows = buildRemediationRows({ sourceReady, secretScanFailClosed, coverage });
  const boundary = buildRemediationBoundary({ generatedAt, writeRequested: options.write !== false, sourceReady, secretScanFailClosed, rows, coverage });
  const gateRows = buildRemediationGateRows({ secretScanFailClosed, packageJson, platformOpsLedger, rows, boundary });
  const validationItems = buildValidationItems({ secretScanFailClosed, sources, rows, gateRows, boundary, coverage });
  const preliminaryValidation = summarizeValidation(validationItems);
  const summary = buildSummary({ secretScanFailClosed, rows, gateRows, boundary, coverage, validation: preliminaryValidation });
  const result = {
    schema_version: SCHEMA_VERSION,
    generated_at: generatedAt,
    trading_secret_scan_remediation_fixtures_id: `trading-secret-scan-remediation-fixtures.${dateStamp(generatedAt)}`,
  ...
  const schemaErrors = schema.available ? validateAgainstSchema(result, schema.data, {}, "trading_secret_scan_remediation_fixtures") : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  const schemaValidationItems = schemaErrors.map((error, index) => validationItem(`schema.${index}`, "schema_validation", false, error.message));
  result.validation_items = [...validationItems, ...schemaValidationItems];
  result.validation = summarizeValidation(result.validation_items);
  result.summary = buildSummary({ secretScanFailClosed, rows, gateRows, boundary, coverage, validation: result.validation });
  result.summary.trading_secret_scan_remediation_fixtures_id = result.trading_secret_scan_remediation_fixtures_id;
  return { ...result, markdown: renderMarkdown(result) };
}

export async function writeTradingSecretScanRemediationFixtures(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "trading-secret-scan-remediation-fixtures.json"), serializableResult(result));
  await writeJson(path.join(outDir, "secret-scan-remediation-rows.json"), collectionEnvelope("trading-secret-scan-remediation-rows.v1", "secret_scan_remediation_rows", result.secret_scan_remediation_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-scan-remediation-gate-rows.json"), collectionEnvelope("trading-secret-scan-remediation-gate-rows.v1", "secret_scan_remediation_gate_rows", result.secret_scan_remediation_gate_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "validation-report.json"), {
    schema_version: "trading-secret-scan-remediation-fixtures-validation-report.v1",
...
export async function runTradingSecretScanRemediationFixturesCli(argv = process.argv.slice(2)) {
  ...
  try {
    const result = await runTradingSecretScanRemediationFixtures(args);
    console.log(`Trading secret scan remediation fixtures ${args.check ? "validated" : "written"} at ${result.output_dir}`);
    console.log(`Status: ${result.summary.trading_secret_scan_remediation_fixtures_status}`);
...
function buildRemediationCoverage({ secretScanFailClosed, sources }) {
  ...
  return {
    p427_ready: secretScanFailClosed.validation.valid && secretScanFailClosed.summary.trading_secret_scan_fail_closed_fixtures_status === SOURCE_READY_STATUS,
    ...
    no_secret_material_read: secretScanFailClosed.summary.no_secret_material_read === true
      && secretScanFailClosed.summary.secret_values_read === false
      && secretScanFailClosed.summary.env_file_read === false
      && secretScanFailClosed.summary.desktop_config_content_inspected === false
      && secretScanFailClosed.summary.desktop_config_read === false,
    no_secret_or_trading_mutation: secretScanFailClosed.summary.no_secret_or_trading_mutation === true
      && secretScanFailClosed.summary.credential_lookup_allowed === false
      && secretScanFailClosed.summary.broker_write_allowed === false
      && secretScanFailClosed.summary.exchange_write_allowed === false
      && secretScanFailClosed.summary.artifact_write_performed === false
      && secretScanFailClosed.summary.protected_action_executed === false,
  };
}

function buildRemediationAnchor(secretScanFailClosed, coverage) {
  return {
    schema_version: "trading-secret-scan-remediation-fixtures-anchor.v1",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_fail_closed_fixtures_id: secretScanFailClosed.trading_secret_scan_fail_closed_fixtures_id,
    source_secret_scan_fail_closed_status: secretScanFailClosed.summary.trading_secret_scan_fail_closed_fixtures_status,
    required_row_count: REQUIRED_ROW_KEYS.length,
    required_row_keys: REQUIRED_ROW_KEYS,
    coverage_hash: hashValue(coverage),
    source_hash: hashValue({
      id: secretScanFailClosed.trading_secret_scan_fail_closed_fixtures_id,
      status: secretScanFailClosed.summary.trading_secret_scan_fail_closed_fixtures_status,
      row_count: secretScanFailClosed.summary.secret_scan_fail_closed_row_count,
      gate_count: secretScanFailClosed.summary.secret_scan_fail_closed_gate_count,
    }),
  };
}

function buildRemediationRows({ sourceReady, secretScanFailClosed, coverage }) {
  const rowInputs = [
    remediationRowInput("p427_secret_scan_fail_closed_ready", "trading_secret_scan_fail_closed", "summary", [
      condition("source.trading_secret_scan_fail_closed_fixtures_status", secretScanFailClosed.summary.trading_secret_scan_fail_closed_fixtures_status, SOURCE_READY_STATUS),
      condition("source.ready_secret_scan_fail_closed_row_count", secretScanFailClosed.summary.ready_secret_scan_fail_closed_row_count, 8),
      condition("source.all_gate_rows_fail_closed", secretScanFailClosed.summary.all_gate_rows_fail_closed, true),
    ]),
...
function buildRemediationRow(input, sourceReady, index) {
  ...
  const row = {
    schema_version: "trading-secret-scan-remediation-row.v1",
    secret_scan_remediation_row_id: `trading-secret-scan-remediation.row.${input.rowKey}`,
    phase_slot: PHASE_SLOT,
    ...
    source_secret_scan_fail_closed_ready: sourceReady,
...
function buildRemediationBoundary({ generatedAt, writeRequested, sourceReady, secretScanFailClosed, rows, coverage }) {
  return {
    schema_version: "trading-secret-scan-remediation-fixtures-boundary.v1",
    generated_at: generatedAt,
...
function buildRemediationGateRows({ secretScanFailClosed, packageJson, platformOpsLedger, boundary }) {
  const scripts = packageJson.data?.scripts ?? {};
  const validateScript = scripts.validate ?? "";
  const ledgerText = platformOpsLedger.text ?? "";
  const gateInputs = [
    ["p427_secret_scan_fail_closed_ready", "P427 secret scan fail-closed source is ready.", secretScanFailClosed.validation.valid && secretScanFailClosed.summary.trading_secret_scan_fail_closed_fixtures_status === SOURCE_READY_STATUS],
    ["platform_package_script_registered", "package.json registers the P428 trading secret scan remediation fixtures command.", typeof scripts[COMMAND_NAME] === "string" && scripts[COMMAND_NAME].length > 0],
    ["platform_validation_chain_registered", "Validation chain includes the P428 trading secret scan remediation fixtures command.", validateScript.includes(`npm run ${COMMAND_NAME} -- --check`)],
    ["p428_ledger_acceptance_declared", "P428 acceptance row is declared in the platform operations ledger.", ledgerText.includes("P428: `trading:secret-scan-remediation-fixtures`")],
    ...
    ["no_secret_or_trading_mutation", "P428 performs no secret reads, credential lookup, trading writes, artifact mutation, release, git, or protected action.", boundary.no_secret_material_read && boundary.no_secret_or_trading_mutation && !boundary.artifact_write_performed && !boundary.protected_action_executed],
  ];
...
function gateRow(rowKey, description, passed) {
  return {
    schema_version: "trading-secret-scan-remediation-gate-row.v1",
    secret_scan_remediation_gate_row_id: `trading-secret-scan-remediation-fixtures.gate.${rowKey}`,
    phase_slot: PHASE_SLOT,
    row_key: rowKey,
    description,
...
function buildValidationItems({ secretScanFailClosed, sources, rows, gateRows, boundary, coverage }) {
  return [
    validationItem("source.secret_scan_fail_closed", "p427_secret_scan_fail_closed_ready", secretScanFailClosed.validation.valid && secretScanFailClosed.summary.trading_secret_scan_fail_closed_fixtures_status === SOURCE_READY_STATUS, "P427 secret scan fail-closed fixtures must be ready."),
    validationItem("source.package_json", "package_json_available", sources.packageJson.available, "package.json is readable for P428 secret scan remediation fixtures."),
    ...
    validationItem("secret_scan_remediation_gate_rows", "secret_scan_remediation_gates_ready", gateRows.length >= 10 && gateRows.every((row) => row.gate_status === "ready" && !row.protected_action_executed_by_gate), "P428 secret scan remediation gates are ready."),
    ...
    validationItem("boundary.no_secret_material_read", "no_secret_material_read", boundary.no_secret_material_read && !boundary.secret_values_read && !boundary.env_file_read && !boundary.desktop_config_content_inspected && !boundary.desktop_config_read, "P428 reads no secret values, env files, or Desktop config content."),
    validationItem("boundary.no_mutation", "no_secret_or_trading_mutation", boundary.no_secret_or_trading_mutation && !boundary.credential_lookup_allowed && !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.artifact_write_performed && !boundary.protected_action_executed, "P428 performs no credential lookup, trading mutation, artifact mutation, or protected action."),
  ];
}

function buildSummary({ secretScanFailClosed, rows, gateRows, boundary, coverage, validation }) {
  return {
    trading_secret_scan_remediation_fixtures_status: validation.valid ? READY_STATUS : "blocked",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_fail_closed_status: secretScanFailClosed.summary.trading_secret_scan_fail_closed_fixtures_status,
    source_secret_scan_fail_closed_ready: boundary.source_secret_scan_fail_closed_ready,
    required_row_count: REQUIRED_ROW_KEYS.length,
    secret_scan_remediation_row_count: rows.length,
...
function renderMarkdown(result) {
  const lines = [
    ...
    "",
    `Status: ${result.summary.trading_secret_scan_remediation_fixtures_status}`,
    `Phase: ${result.summary.phase_slot}`,
    `Source secret scan fail-closed: ${result.summary.source_secret_scan_fail_closed_status}`,
...
function parseArgs(argv) {
  const parsed = { outDir: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_OUT_DIR };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--secret-scan-fail-closed-schema") parsed.secretScanFailClosedSchemaPath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/trading-secret-scan-remediation-fixtures.mjs [options]

Options:
  --out-dir <folder>                           Output directory. Default: ${DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_OUT_DIR}
  --run-at <iso>                               Deterministic generated_at timestamp.
  --package <path>                             package.json path.
  --platform-ops-ledger <path>                 P341-P500 platform operations ledger path.
  --secret-scan-fail-closed-schema <path>      P427 secret scan fail-closed fixtures schema path.
...
function normalizeInputs(options = {}) {
  return {
    package_path: path.resolve(options.packagePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_INPUTS.packagePath),
    platform_ops_ledger_path: path.resolve(options.platformOpsLedgerPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_INPUTS.platformOpsLedgerPath),
    secret_scan_attention_schema_path: path.resolve(options.secretScanAttentionSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_INPUTS.secretScanAttentionSchemaPath),
    secret_scan_fail_closed_schema_path: path.resolve(options.secretScanFailClosedSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_INPUTS.secretScanFailClosedSchemaPath),
    review_dashboard_path: path.resolve(options.reviewDashboardPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_INPUTS.reviewDashboardPath),
    control_plane_action_plan_path: path.resolve(options.controlPlaneActionPlanPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_INPUTS.controlPlaneActionPlanPath),
    control_plane_human_gate_receipts_path: path.resolve(options.controlPlaneHumanGateReceiptsPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_INPUTS.controlPlaneHumanGateReceiptsPath),
    schema_path: path.resolve(options.schemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_FIXTURES_INPUTS.schemaPath),
  };
}
