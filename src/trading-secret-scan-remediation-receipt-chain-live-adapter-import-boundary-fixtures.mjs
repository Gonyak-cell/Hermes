...
import {
  DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS,
  buildTradingSecretScanRemediationReceiptChainBrokerAdapterSeparationFixtures,
} from "./trading-secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures.mjs";

export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_OUT_DIR = "artifacts/trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures/latest";
export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS = {
  ...DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS,
  brokerAdapterSeparationFixturesSchemaPath: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_BROKER_ADAPTER_SEPARATION_FIXTURES_INPUTS.schemaPath,
  ...
  schemaPath: "schemas/trading/trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures.schema.json",
};

const SCHEMA_VERSION = "trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures.v1";
const CAPABILITY_ID = "trading.secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures";
const PHASE_SLOT = "P448";
const PREVIOUS_PHASE_SLOT = "P447";
const NEXT_PHASE_SLOT = "P449";
const READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_live_adapter_import_boundary";
const SOURCE_READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_broker_adapter_separation";
const COMMAND_NAME = "trading:secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures";
const WORKFLOW_ID = "workflow.trading.secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures.v1";
const REQUIRED_ROW_KEYS = [
  "p447_secret_scan_remediation_receipt_chain_broker_adapter_separation_ready",
...
export async function runTradingSecretScanRemediationReceiptChainLiveAdapterImportBoundaryFixtures(options = {}) {
  const result = await buildTradingSecretScanRemediationReceiptChainLiveAdapterImportBoundaryFixtures(options);
  if (options.write !== false) await writeTradingSecretScanRemediationReceiptChainLiveAdapterImportBoundaryFixtures(result, result.output_dir);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Trading secret scan remediation receipt chain live adapter import boundary fixtures failed with ${result.validation.errors.length} error(s).`);
...
export async function buildTradingSecretScanRemediationReceiptChainLiveAdapterImportBoundaryFixtures(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_OUT_DIR);
  const inputs = normalizeInputs(options);
  const brokerAdapterSeparation = await buildTradingSecretScanRemediationReceiptChainBrokerAdapterSeparationFixtures({
    ...options,
    runAt: generatedAt,
  ...
  const sourceReady = brokerAdapterSeparation.validation.valid && brokerAdapterSeparation.summary.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_status === SOURCE_READY_STATUS;
  ...
  const result = {
    ...
    trading_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_id: `trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures.${dateStamp(generatedAt)}`,
  ...
  const schemaErrors = schema.available ? validateAgainstSchema(result, schema.data, {}, "trading_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures") : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  ...
  result.summary.trading_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_id = result.trading_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_id;
  return { ...result, markdown: renderMarkdown(result) };
}

export async function writeTradingSecretScanRemediationReceiptChainLiveAdapterImportBoundaryFixtures(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures.json"), serializableResult(result));
  await writeJson(path.join(outDir, "live-adapter-import-boundary-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-rows.v1", "live_adapter_import_boundary_rows", result.live_adapter_import_boundary_rows, result.generated_at));
  await writeJson(path.join(outDir, "live-adapter-import-boundary-gate-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-gate-rows.v1", "live_adapter_import_boundary_gate_rows", result.live_adapter_import_boundary_gate_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "validation-report.json"), {
    schema_version: "trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures-validation-report.v1",
...
export async function runTradingSecretScanRemediationReceiptChainLiveAdapterImportBoundaryFixturesCli(argv = process.argv.slice(2)) {
  ...
  try {
    const result = await runTradingSecretScanRemediationReceiptChainLiveAdapterImportBoundaryFixtures(args);
    console.log(`Trading secret scan remediation receipt chain live adapter import boundary fixtures ${args.check ? "validated" : "written"} at ${result.output_dir}`);
    console.log(`Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_status}`);
...
function buildLiveAdapterImportBoundaryAnchor(brokerAdapterSeparation, importRefs) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures-anchor.v1",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_id: brokerAdapterSeparation.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_id,
    source_secret_scan_remediation_receipt_chain_broker_adapter_separation_status: brokerAdapterSeparation.summary.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_status,
    ...
    source_hash: hashValue({
      id: brokerAdapterSeparation.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_id,
      status: brokerAdapterSeparation.summary.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_status,
...
function buildLiveAdapterImportBoundaryRows({ sourceReady, brokerAdapterSeparation, packageJson, packManifest, capability, executionEngine, paperShadow, limitedLive, fullAuto, researchBacktestPaper, futureLiveAdapterPath, importRefs }) {
  ...
  const schemaRegistered = Array.isArray(packManifest?.schemas) && packManifest.schemas.includes(DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.schemaPath);
  const entrypointRegistered = Array.isArray(capability?.entrypoints) && capability.entrypoints.some((entrypoint) => entrypoint?.name === COMMAND_NAME && entrypoint?.path === "scripts/trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures.mjs");
  ...
  const rowInputs = [
    liveImportRowInput("p447_secret_scan_remediation_receipt_chain_broker_adapter_separation_ready", "secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures", "summary", [
      condition("source.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_status", brokerAdapterSeparation.summary.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_status, SOURCE_READY_STATUS),
    ...
    liveImportRowInput("package_scripts_no_future_live_adapter_import", "package_json", "scripts", [
      ...
      condition("package.scripts.validate.contains_p448_check", validateScript.includes(`npm run ${COMMAND_NAME} -- --check`), true),
    ...
    liveImportRowInput("validate_chain_no_future_live_adapter_import", "package_json", "scripts.validate", [
      ...
      condition("package.scripts.validate.contains_p447_check", validateScript.includes("npm run trading:secret-scan-remediation-receipt-chain-broker-adapter-separation-fixtures -- --check"), true),
      condition("package.scripts.validate.contains_p448_check", validateScript.includes(`npm run ${COMMAND_NAME} -- --check`), true),
...
function buildLiveAdapterImportBoundaryRow(input, sourceReady, index) {
  ...
  const row = {
    schema_version: "trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-row.v1",
    live_adapter_import_boundary_row_id: `trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary.row.${input.rowKey}`,
    phase_slot: PHASE_SLOT,
    ...
    source_secret_scan_remediation_receipt_chain_broker_adapter_separation_ready: sourceReady,
    future_live_adapter_path: input.overrides.futureLiveAdapterPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.futureLiveAdapterPath,
...
function buildLiveAdapterImportBoundary({ generatedAt, writeRequested, brokerAdapterSeparation, rows, importRefs, futureLiveAdapterPath }) {
  ...
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures-boundary.v1",
    generated_at: generatedAt,
...
function buildLiveAdapterImportBoundaryGateRows({ brokerAdapterSeparation, packageJson, platformOpsLedger, packManifest, capability, sources, rows, boundary }) {
  ...
  const gateInputs = [
    ["p447_secret_scan_remediation_receipt_chain_broker_adapter_separation_ready", "P447 secret scan remediation receipt chain broker adapter separation source is ready.", brokerAdapterSeparation.validation.valid && brokerAdapterSeparation.summary.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_status === SOURCE_READY_STATUS],
    ["platform_package_script_registered", "package.json registers the P448 trading live adapter import boundary fixtures command.", typeof scripts[COMMAND_NAME] === "string" && scripts[COMMAND_NAME].length > 0],
    ["platform_validation_chain_registered", "Validation chain includes the P448 trading live adapter import boundary fixtures command.", validateScript.includes(`npm run ${COMMAND_NAME} -- --check`)],
    ["p448_ledger_acceptance_declared", "P448 acceptance row is declared in the platform operations ledger.", ledgerText.includes("P448: `trading:secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures`")],
    ...
    ["no_secret_or_trading_mutation", "P448 keeps credentials reference-only and performs no secret scan remediation action, trading, artifact, release, git, or protected mutation.", boundary.credential_reference_only && !boundary.credential_lookup_allowed && !boundary.plaintext_secret_allowed && !boundary.provider_key_material_present && !boundary.environment_dump_present && !boundary.secret_material_exposed && boundary.no_secret_material_read && boundary.no_secret_or_trading_mutation && !boundary.secret_scan_remediation_action_allowed && !boundary.secret_values_read && !boundary.env_file_read && !boundary.desktop_config_content_inspected && !boundary.desktop_config_read && !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.command_execution_performed && !boundary.artifact_write_performed && !boundary.release_published && !boundary.git_operation_performed && !boundary.protected_action_executed],
  ];
...
function gateRow(rowKey, description, passed) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-gate-row.v1",
    live_adapter_import_boundary_gate_row_id: `trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures.gate.${rowKey}`,
    phase_slot: PHASE_SLOT,
    row_key: rowKey,
    description,
...
function buildValidationItems({ brokerAdapterSeparation, packageJson, platformOpsLedger, packManifest, capability, sources, rows, gateRows, boundary }) {
  return [
    validationItem("source.secret_scan_remediation_receipt_chain_broker_adapter_separation", "p447_secret_scan_remediation_receipt_chain_broker_adapter_separation_ready", brokerAdapterSeparation.validation.valid && brokerAdapterSeparation.summary.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_status === SOURCE_READY_STATUS, "P447 secret scan remediation receipt chain broker adapter separation fixtures must be ready."),
    validationItem("source.package_json", "package_json_available", packageJson.available, "package.json is readable for P448 live adapter import boundary fixtures."),
    validationItem("source.platform_ops_ledger", "platform_ops_ledger_available", platformOpsLedger.available, "Platform operations stability ledger is readable."),
    validationItem("source.pack_manifest", "pack_manifest_available", packManifest.available, "Trading pack manifest is readable for P448 live adapter import boundary fixtures."),
    validationItem("source.capability", "capability_available", capability.available, "Trading capability manifest is readable for P448 live adapter import boundary fixtures."),
    ...
    validationItem("live_adapter_import_boundary_gate_rows", "import_boundary_gates_ready", gateRows.length >= 10 && gateRows.every((row) => row.gate_status === "ready" && !row.protected_action_executed_by_gate), "P448 live adapter import boundary gates are ready."),
    ...
    validationItem("boundary.secret_boundary", "reference_only_credentials", boundary.credential_reference_only && !boundary.credential_lookup_allowed && !boundary.plaintext_secret_allowed && !boundary.provider_key_material_present && !boundary.environment_dump_present && !boundary.secret_material_exposed && boundary.no_secret_material_read && !boundary.secret_values_read && !boundary.env_file_read && !boundary.desktop_config_content_inspected && !boundary.desktop_config_read, "Credential handling must remain reference-only with no plaintext, environment dump, provider key, model-context secret, .env, Desktop config, or secret-value exposure."),
    validationItem("boundary.no_mutation", "no_secret_or_trading_mutation", boundary.no_secret_or_trading_mutation && !boundary.secret_scan_remediation_action_allowed && !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.command_execution_performed && !boundary.package_command_execution_performed && !boundary.release_check_execution_performed && !boundary.artifact_read_performed && !boundary.artifact_write_performed && !boundary.release_published && !boundary.git_operation_performed && !boundary.protected_action_executed, "P448 performs no secret scan remediation action, trading, artifact, release, git, or protected mutation."),
  ];
}

function buildSummary({ brokerAdapterSeparation, rows, gateRows, boundary, validation }) {
  return {
    trading_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_status: validation.valid ? READY_STATUS : "blocked",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_broker_adapter_separation_status: brokerAdapterSeparation.summary.trading_secret_scan_remediation_receipt_chain_broker_adapter_separation_fixtures_status,
    source_secret_scan_remediation_receipt_chain_broker_adapter_separation_ready: boundary.source_secret_scan_remediation_receipt_chain_broker_adapter_separation_ready,
    required_row_count: REQUIRED_ROW_KEYS.length,
    live_adapter_import_boundary_row_count: rows.length,
...
function renderMarkdown(result) {
  const lines = [
    ...
    "",
    `Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_live_adapter_import_boundary_fixtures_status}`,
    `Phase: ${result.summary.phase_slot}`,
    `Source secret scan remediation receipt chain broker adapter separation: ${result.summary.source_secret_scan_remediation_receipt_chain_broker_adapter_separation_status}`,
...
function parseArgs(argv) {
  const parsed = { outDir: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_OUT_DIR };
...
function printHelp() {
  console.log(`Usage: node scripts/trading-secret-scan-remediation-receipt-chain-live-adapter-import-boundary-fixtures.mjs [options]

Options:
  --out-dir <folder>                         Output directory. Default: ${DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_OUT_DIR}
  ...
  --broker-adapter-separation-fixtures-schema <path>
                                             P447 secret scan remediation receipt chain broker adapter separation fixtures schema path.
...
function normalizeInputs(options = {}) {
  return {
    package_path: path.resolve(options.packagePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.packagePath),
    platform_ops_ledger_path: path.resolve(options.platformOpsLedgerPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.platformOpsLedgerPath),
    broker_adapter_separation_fixtures_schema_path: path.resolve(options.brokerAdapterSeparationFixturesSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.brokerAdapterSeparationFixturesSchemaPath),
    execution_engine_path: path.resolve(options.executionEnginePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.executionEnginePath),
    paper_shadow_path: path.resolve(options.paperShadowPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.paperShadowPath),
    limited_live_path: path.resolve(options.limitedLivePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.limitedLivePath),
    full_auto_path: path.resolve(options.fullAutoPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.fullAutoPath),
    research_backtest_paper_path: path.resolve(options.researchBacktestPaperPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.researchBacktestPaperPath),
    pack_manifest_path: path.resolve(options.packManifestPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.packManifestPath),
    capability_path: path.resolve(options.capabilityPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.capabilityPath),
    future_live_adapter_path: options.futureLiveAdapterPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.futureLiveAdapterPath,
    schema_path: path.resolve(options.schemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_LIVE_ADAPTER_IMPORT_BOUNDARY_FIXTURES_INPUTS.schemaPath),
  };
}
