...
import {
  ALL_FALSE_FLAGS as P28000_FALSE_FLAGS,
  buildReceiptWorkbenchOperatorQueueUiHandoffBundle,
} from "./receipt-workbench-operator-queue-ui-handoff-bundle.mjs";

export const DEFAULT_RECEIPT_WORKBENCH_OPERATOR_QUEUE_LOCAL_UI_BINDING_SMOKE_OUT_DIR = "artifacts/receipt-workbench-operator-queue-local-ui-binding-smoke/latest";
export const DEFAULT_RECEIPT_WORKBENCH_OPERATOR_QUEUE_LOCAL_UI_BINDING_SMOKE_INPUTS = {
  schemaPath: "schemas/receipt-workbench-operator-queue-local-ui-binding-smoke.schema.json",
  packagePath: "package.json",
  roadmapDocPath: "docs/hermes-roadmap-p28001-p28400.md",
  architectureDocPath: "docs/architecture.md",
  sourceReceiptWorkbenchOperatorQueueUiHandoffBundlePath: "artifacts/receipt-workbench-operator-queue-ui-handoff-bundle/latest/receipt-workbench-operator-queue-ui-handoff-bundle.json",
};

const COMMAND_NAME = "platform:receipt-workbench-operator-queue-local-ui-binding-smoke";
const SCHEMA_VERSION = "receipt-workbench-operator-queue-local-ui-binding-smoke.v1";
const CAPABILITY_ID = "platform.receipt_workbench_operator_queue_local_ui_binding_smoke";
const PROGRAM_RANGE = "P28001-P28400";
const SOURCE_PROGRAM_RANGE = "P27601-P28000";
const READY_STATUS = "ready_for_receipt_workbench_operator_queue_local_ui_binding_smoke";
const BLOCK_PENDING_STATUS = "valid_block_receipt_workbench_operator_queue_local_ui_binding_smoke_pending";
const BLOCKED_STATUS = "blocked_receipt_workbench_operator_queue_local_ui_binding_smoke";
...
const BINDING_SMOKE_SLOTS = [
  ["queue_summary", "Queue summary shell slot"],
  ["receipt_candidate_list", "Receipt candidate list shell slot"],
  ["receipt_candidate_detail", "Receipt candidate detail shell slot"],
  ["validation_evidence_panel", "Validation evidence panel shell slot"],
  ["review_status_panel", "Review status panel shell slot"],
  ["blocker_notice_panel", "Blocker notice panel shell slot"],
  ["next_action_panel", "Next action panel shell slot"],
  ["redaction_notice_panel", "Redaction notice panel shell slot"],
];
...
export const ALL_FALSE_FLAGS = [...new Set([...LOCAL_UI_BINDING_FALSE_FLAGS, ...P28000_FALSE_FLAGS])];

export async function runReceiptWorkbenchOperatorQueueLocalUiBindingSmoke(options = {}) {
  const result = await buildReceiptWorkbenchOperatorQueueLocalUiBindingSmoke(options);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Receipt Workbench Operator Queue Local UI Binding Smoke failed with ${result.validation.errors.length} error(s).`);
    error.validation = result.validation;
    throw error;
  }
  if (!options.check && options.write !== false) await writeReceiptWorkbenchOperatorQueueLocalUiBindingSmoke(result, result.output_dir);
  return result;
}

export async function buildReceiptWorkbenchOperatorQueueLocalUiBindingSmoke(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_RECEIPT_WORKBENCH_OPERATOR_QUEUE_LOCAL_UI_BINDING_SMOKE_OUT_DIR);
  const inputs = normalizeInputs(options);
  const schema = await readJsonSource(inputs.schema_path);
  const packageJson = await readJsonSource(inputs.package_path);
  const roadmapDoc = await readTextSource(inputs.roadmap_doc_path);
  const architectureDoc = await readTextSource(inputs.architecture_doc_path);
  const source = Object.prototype.hasOwnProperty.call(options, "receiptWorkbenchOperatorQueueUiHandoffBundle")
    ? normalizeInlineJsonSource("inline.receipt_workbench_operator_queue_ui_handoff_bundle", options.receiptWorkbenchOperatorQueueUiHandoffBundle)
    : await readJsonOrBuildP28000(inputs.source_receipt_workbench_operator_queue_ui_handoff_bundle_path, generatedAt);
  ...
  const result = {
    ...
    inputs,
    source_refs: {
      receipt_workbench_operator_queue_ui_handoff_bundle_path: source.path,
      source_commit_ref: commitRef || null,
    },
    source_receipt_workbench_operator_queue_ui_handoff_bundle_summary: source.data?.summary ?? null,
    receipt_workbench_operator_queue_local_ui_binding_smoke_contract: buildContract(generatedAt),
    receipt_workbench_operator_queue_local_ui_binding_smoke_phase_rows: phaseRows,
    p28000_source_binding_rows: sourceRows,
    ...
    p28400_clean_checkpoint_rows: checkpointRows,
    receipt_workbench_operator_queue_local_ui_binding_smoke_boundary: boundary,
    receipt_workbench_operator_queue_local_ui_binding_smoke_validation_items: validationItems,
    validation: preliminaryValidation,
    summary: buildSummary({ boundary, validation: preliminaryValidation }),
  };

  const schemaErrors = schema.available
    ? validateAgainstSchema(result, schema.data, {}, "receipt_workbench_operator_queue_local_ui_binding_smoke")
    : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  const schemaValidationItems = schemaErrors.map((error, index) => validationItem(`schema.${index}`, "schema_validation", false, error.message));
  result.receipt_workbench_operator_queue_local_ui_binding_smoke_validation_items = [...validationItems, ...schemaValidationItems];
  result.validation = summarizeValidation(result.receipt_workbench_operator_queue_local_ui_binding_smoke_validation_items);
  result.summary = buildSummary({ boundary, validation: result.validation });
  return { ...result, markdown: renderMarkdown(result), html: renderHtml(result) };
}

export async function writeReceiptWorkbenchOperatorQueueLocalUiBindingSmoke(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "receipt-workbench-operator-queue-local-ui-binding-smoke.json"), serializableResult(result));
  await writeJson(path.join(outDir, "p28000-source-binding-rows.json"), collectionEnvelope("p28000-source-binding-rows.v1", "p28000_source_binding_rows", result.p28000_source_binding_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "p28400-clean-checkpoint-rows.json"), collectionEnvelope("p28400-clean-checkpoint-rows.v1", "p28400_clean_checkpoint_rows", result.p28400_clean_checkpoint_rows, result.generated_at));
  await writeJson(path.join(outDir, "receipt-workbench-operator-queue-local-ui-binding-smoke-boundary.json"), result.receipt_workbench_operator_queue_local_ui_binding_smoke_boundary);
  await writeFile(path.join(outDir, "summary.md"), result.markdown, "utf8");
  await writeFile(path.join(outDir, "index.html"), result.html, "utf8");
}

export async function runReceiptWorkbenchOperatorQueueLocalUiBindingSmokeCli(argv = process.argv.slice(2)) {
  const args = parseArgs(argv);
  if (args.help) {
    printHelp();
    return null;
  }
  const result = await runReceiptWorkbenchOperatorQueueLocalUiBindingSmoke(args);
  console.log(`Receipt Workbench Operator Queue Local UI Binding Smoke ${args.check ? "validated" : "written"} at ${result.output_dir}`);
  console.log(`Status: ${result.summary.receipt_workbench_operator_queue_local_ui_binding_smoke_status}`);
  console.log(`Program: ${result.program_range}`);
  console.log(`P28000 ready for P28001 handoff: ${result.summary.source_p28000_ready_for_p28001_handoff}`);
  ...
  console.log(`Ready for P28401 handoff: ${result.summary.ready_for_p28401_handoff}`);
...
function buildSourceState(source) {
  ...
  const boundary = source.data?.receipt_workbench_operator_queue_ui_handoff_bundle_boundary ?? {};
  return {
    available: source.available === true,
    programRangeOk: source.data?.program_range === SOURCE_PROGRAM_RANGE,
    validationValid: source.data?.validation?.valid === true,
    sourceReady: summary.ready_for_p28001_handoff === true,
    status: summary.receipt_workbench_operator_queue_ui_handoff_bundle_status ?? "missing",
    p28000ContractReady: boundary.p28000_contract_ready === true,
    uiHandoffBundleVisible: boundary.ui_handoff_bundle_manifest_visible_now === true,
    adapterManifestVisible: boundary.read_only_adapter_manifest_visible_now === true,
    handoffViewVisible: boundary.operator_queue_handoff_view_visible_now === true,
    reviewAffordanceVisible: boundary.review_affordance_visibility_visible_now === true,
    noServeNoRenderClosed: boundary.no_serve_no_render_boundary_closed_now === true,
    boundaryClosed: P28000_FALSE_FLAGS.every((flag) => boundary[flag] === false),
  };
}
...
function buildPhaseRows(roadmapText, generatedAt) {
  return PHASE_SPECS.map(([range, label, outputRef]) => row({
    ...
    evidence_ref: "docs/hermes-roadmap-p28001-p28400.md",
...
function buildSourceRows({ source, sourceState, commitRef, generatedAt }) {
  return [
    ["artifact_available", "P28000 operator queue UI handoff bundle source is available", sourceState.available],
    ["program_range", "P28000 source program range is P27601-P28000", sourceState.programRangeOk],
    ["validation_valid", "P28000 source validation is valid", sourceState.validationValid],
    ["p28001_handoff_open", "P28000 source opened P28001 handoff", sourceState.sourceReady],
    ["p28000_contract_ready", "P28000 source contract is ready", sourceState.p28000ContractReady],
    ["ui_handoff_bundle_visible", "P28000 UI handoff bundle rows are visible", sourceState.uiHandoffBundleVisible],
    ["adapter_manifest_visible", "P28000 read-only adapter manifest rows are visible", sourceState.adapterManifestVisible],
    ["handoff_view_visible", "P28000 handoff view rows are visible", sourceState.handoffViewVisible],
    ["review_affordance_visible", "P28000 review affordance rows are visible", sourceState.reviewAffordanceVisible],
    ["no_serve_no_render_closed", "P28000 no-serve/no-render boundary is closed", sourceState.noServeNoRenderClosed],
    ["commit_ref_present", "Current commit ref is present for local UI binding smoke", Boolean(commitRef)],
    ["source_blocker_visible", "P28000 source blocker is visible when local UI binding is closed", sourceState.available],
  ].map(([id, label, observed]) => row({
    row_id: `source_binding.${id}`,
    category: "p28000_source_binding",
    label,
    observed,
...
function buildLocalUiBindingSmokeRows({ source, generatedAt }) {
  const bundleRows = Array.isArray(source.data?.ui_handoff_bundle_manifest_rows) ? source.data.ui_handoff_bundle_manifest_rows : [];
  return BINDING_SMOKE_SLOTS.map(([slotId, label], index) => {
    ...
    return row({
      ...
      label,
      observed: Boolean(bundle) && bundle.current_verdict === "pass" && bundle.read_only === true,
      ...
      local_fixture_ref: `fixtures.operator_queue.${slotId}`,
      visible_now: true,
...
function buildStaticShellBindingMapRows({ source, bindingRows, generatedAt }) {
  const adapterRows = Array.isArray(source.data?.read_only_adapter_manifest_rows) ? source.data.read_only_adapter_manifest_rows : [];
  return bindingRows.map((binding, index) => {
    ...
    return row({
      ...
      shell_slot_id: `shell.operator_queue.${binding.binding_slot_id}`,
      binding_slot_ref: binding.row_id,
      adapter_manifest_ref: adapter?.row_id ?? null,
      route_hint: adapter?.route_hint ?? `/work-os/operator-queue/${binding.binding_slot_id}`,
...
function buildGetOnlyFixtureFetchContractRows({ source, bindingRows, shellRows, generatedAt }) {
  const viewRows = Array.isArray(source.data?.operator_queue_handoff_view_rows) ? source.data.operator_queue_handoff_view_rows : [];
  return bindingRows.map((binding, index) => {
    ...
    return row({
      ...
      fixture_fetch_id: `fixture_fetch.operator_queue.${binding.binding_slot_id}`,
      binding_slot_ref: binding.row_id,
...
function buildVisibleBlockerNoActionRows({ source, bindingRows, shellRows, fetchRows, generatedAt }) {
  const reviewRows = Array.isArray(source.data?.review_affordance_visibility_rows) ? source.data.review_affordance_visibility_rows : [];
  return BINDING_SMOKE_SLOTS.map(([slotId, label], index) => {
    ...
    return row({
      ...
      evidence_ref: review?.row_id ?? fetch?.row_id ?? binding?.row_id ?? "review_affordance_visibility_rows",
...
function buildNoServerNoBrowserBoundaryRows(generatedAt) {
  return ALL_FALSE_FLAGS.map((flag) => row({
    ...
    observed: true,
    evidence_ref: "receipt_workbench_operator_queue_local_ui_binding_smoke_boundary",
    boundary_flag: flag,
    allowed_now: false,
    generated_at: generatedAt,
  }));
...
function buildCheckpointRows(context) {
  ...
  return [
    ["source_ready", "P28000 source is ready for P28001", context.sourceState.sourceReady],
    ...
    ["p28401_handoff_gate", "P28401 handoff opens only when local UI binding smoke conditions pass", handoffReady],
    ["p28401_handoff_blocker_visible", "P28401 handoff blocker is visible when the gate is closed", true],
  ].map(([id, label, observed]) => row({
    row_id: `p28400_checkpoint.${id}`,
    category: "p28400_clean_checkpoint",
    label,
    observed,
    evidence_ref: "p28400_clean_checkpoint_rows",
    generated_at: context.generatedAt,
  }));
}

function buildBoundary(context) {
  const p28400ContractReady = allPass(context.phaseRows)
    ...
    && allPass(context.blockerRows)
    && allPass(context.boundaryRows)
    && visibleOrPassed(context.checkpointRows, "p28400_checkpoint.p28401_handoff_blocker_visible");
  const handoffReady = context.sourceState.sourceReady
    && context.sourceState.validationValid
    && context.sourceState.boundaryClosed
    && allPass(context.bindingRows)
    ...
    && allPass(context.boundaryRows);
  return {
    p28400_contract_ready: p28400ContractReady,
    ready_for_p28401_handoff: handoffReady,
    source_p28000_ready_for_p28001_handoff: context.sourceState.sourceReady,
...
function buildValidationItems(context) {
  return [
    ...
    validationItem("roadmap.phase_coverage", "roadmap", allPass(context.phaseRows), "P28001-P28400 phase rows are incomplete"),
    validationItem("architecture.reference", "architecture", context.architectureDoc.text.includes("P28001-P28400 Receipt Workbench Operator Queue Local UI Binding Smoke"), "Architecture doc missing P28001-P28400 reference"),
    validationItem("source.visible", "source", visibleOrPassed(context.sourceRows, "source_binding.source_blocker_visible"), "P28000 source state is not visible"),
    ...
    validationItem("checkpoint.visible", "checkpoint", visibleOrPassed(context.checkpointRows, "p28400_checkpoint.p28401_handoff_blocker_visible"), "P28400 checkpoint blocker is not visible"),
  ];
}

function buildContract(generatedAt) {
  return {
    contract_id: "receipt_workbench_operator_queue_local_ui_binding_smoke.contract.v1",
    generated_at: generatedAt,
    source_p28000_required_or_rebuilt: true,
    ...
    p28401_handoff_is_not_server_route_dom_browser_action_write_approval_closeout_or_enterprise_trust: true,
    protected_authority: "closed",
  };
}

function buildSummary({ boundary, validation }) {
  const status = validation.valid && boundary.ready_for_p28401_handoff
    ? READY_STATUS
    : validation.valid && boundary.p28400_contract_ready
      ? BLOCK_PENDING_STATUS
      : BLOCKED_STATUS;
  return {
    receipt_workbench_operator_queue_local_ui_binding_smoke_status: status,
    source_p28000_ready_for_p28001_handoff: boundary.source_p28000_ready_for_p28001_handoff,
    ...
    ready_for_p28401_handoff: validation.valid && boundary.ready_for_p28401_handoff,
...
function renderMarkdown(result) {
  return [
    "# Receipt Workbench Operator Queue Local UI Binding Smoke",
    "",
    `Status: ${result.summary.receipt_workbench_operator_queue_local_ui_binding_smoke_status}`,
    `Program: ${result.program_range}`,
    `P28000 ready for P28001 handoff: ${result.summary.source_p28000_ready_for_p28001_handoff}`,
    ...
    `Ready for P28401 handoff: ${result.summary.ready_for_p28401_handoff}`,
...
<head>
  ...
  <title>Hermes Receipt Workbench Operator Queue Local UI Binding Smoke</title>
  <style>
...
<body>
  <main>
    <h1>Hermes Receipt Workbench Operator Queue Local UI Binding Smoke</h1>
...
async function readJsonOrBuildP28000(filePath, generatedAt) {
  const source = await readJsonSource(filePath);
  if (source.available) return source;
  const built = await buildReceiptWorkbenchOperatorQueueUiHandoffBundle({ runAt: generatedAt, write: false });
  return normalizeInlineJsonSource("built.receipt_workbench_operator_queue_ui_handoff_bundle", built);
}
...
function normalizeInputs(options) {
  const defaults = DEFAULT_RECEIPT_WORKBENCH_OPERATOR_QUEUE_LOCAL_UI_BINDING_SMOKE_INPUTS;
  ...
  return {
    ...
    source_receipt_workbench_operator_queue_ui_handoff_bundle_path: path.resolve(repoRoot, options.sourceReceiptWorkbenchOperatorQueueUiHandoffBundlePath ?? defaults.sourceReceiptWorkbenchOperatorQueueUiHandoffBundlePath),
  };
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--check") {
      ...
      args.sourceReceiptWorkbenchOperatorQueueUiHandoffBundlePath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/receipt-workbench-operator-queue-local-ui-binding-smoke.mjs [--check] [--out-dir DIR] [--source FILE] [--commit-ref SHA]

Validates or writes the Hermes P28001-P28400 Receipt Workbench Operator Queue Local UI Binding Smoke.
`);
}
