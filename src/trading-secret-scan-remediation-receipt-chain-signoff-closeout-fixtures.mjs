...
import {
  DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_APPROVAL_CLOSEOUT_FIXTURES_INPUTS,
  buildTradingSecretScanRemediationReceiptChainSignoffApprovalCloseoutFixtures,
} from "./trading-secret-scan-remediation-receipt-chain-signoff-approval-closeout-fixtures.mjs";

export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_OUT_DIR = "artifacts/trading-secret-scan-remediation-receipt-chain-signoff-closeout-fixtures/latest";
export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS = {
  ...DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_APPROVAL_CLOSEOUT_FIXTURES_INPUTS,
  secretScanRemediationReceiptChainSignoffApprovalCloseoutSchemaPath: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_APPROVAL_CLOSEOUT_FIXTURES_INPUTS.schemaPath,
  schemaPath: "schemas/trading/trading-secret-scan-remediation-receipt-chain-signoff-closeout-fixtures.schema.json",
};

const SCHEMA_VERSION = "trading-secret-scan-remediation-receipt-chain-signoff-closeout-fixtures.v1";
const CAPABILITY_ID = "trading.secret_scan_remediation_receipt_chain_signoff_closeout_fixtures";
const PHASE_SLOT = "P446";
const PREVIOUS_PHASE_SLOT = "P445";
const NEXT_PHASE_SLOT = "P447";
const READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_signoff_closeout";
const SOURCE_READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout";
const SOURCE_APPROVAL_CLOSEOUT_READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout";
const SIGNOFF_CLOSEOUT_READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_signoff_closeout";
const SIGNOFF_CLOSEOUT_CHECKS = [
  "p445_signoff_approval_closeout_ready",
...
export async function runTradingSecretScanRemediationReceiptChainSignoffCloseoutFixtures(options = {}) {
  const result = await buildTradingSecretScanRemediationReceiptChainSignoffCloseoutFixtures(options);
  if (options.write !== false) await writeTradingSecretScanRemediationReceiptChainSignoffCloseoutFixtures(result, result.output_dir);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Trading secret scan remediation receipt chain signoff closeout fixtures failed with ${result.validation.errors.length} error(s).`);
...
export async function buildTradingSecretScanRemediationReceiptChainSignoffCloseoutFixtures(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_OUT_DIR);
  const inputs = normalizeInputs(options);
  const signoffApprovalCloseout = await buildTradingSecretScanRemediationReceiptChainSignoffApprovalCloseoutFixtures({
    ...options,
    runAt: generatedAt,
    ...
    secretScanRemediationReceiptChainSignoffValidationRulesSchemaPath: inputs.secret_scan_remediation_receipt_chain_signoff_validation_rules_schema_path,
    schemaPath: inputs.secret_scan_remediation_receipt_chain_signoff_approval_closeout_schema_path,
    write: false,
  });
  ...
  const result = {
    ...
    trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_id: `trading-secret-scan-remediation-receipt-chain-signoff-closeout-fixtures.${dateStamp(generatedAt)}`,
    capability_id: CAPABILITY_ID,
    output_dir: outputDir,
    inputs,
    secret_scan_remediation_receipt_chain_signoff_closeout_anchor: intakeAnchor,
    secret_scan_remediation_receipt_chain_signoff_closeout_rows: intakeRows,
    secret_scan_remediation_receipt_chain_signoff_closeout_gate_rows: intakeGateRows,
    secret_scan_remediation_receipt_chain_signoff_closeout_boundary: intakeBoundary,
    validation_items: validationItems,
    validation: preliminaryValidation,
    summary,
  };
  const schema = await readJsonSource(inputs.schema_path);
  const schemaErrors = schema.available ? validateAgainstSchema(result, schema.data, {}, "trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures") : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  ...
  result.summary.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_id = result.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_id;
  return { ...result, markdown: renderMarkdown(result) };
}

export async function writeTradingSecretScanRemediationReceiptChainSignoffCloseoutFixtures(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "trading-secret-scan-remediation-receipt-chain-signoff-closeout-fixtures.json"), serializableResult(result));
  await writeJson(path.join(outDir, "secret-scan-remediation-receipt-chain-signoff-closeout-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-signoff-closeout-rows.v1", "secret_scan_remediation_receipt_chain_signoff_closeout_rows", result.secret_scan_remediation_receipt_chain_signoff_closeout_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-scan-remediation-receipt-chain-signoff-closeout-gate-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-signoff-closeout-gate-rows.v1", "secret_scan_remediation_receipt_chain_signoff_closeout_gate_rows", result.secret_scan_remediation_receipt_chain_signoff_closeout_gate_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-scan-remediation-receipt-chain-signoff-closeout-boundary.json"), result.secret_scan_remediation_receipt_chain_signoff_closeout_boundary);
  await writeJson(path.join(outDir, "validation-report.json"), {
    schema_version: "trading-secret-scan-remediation-receipt-chain-signoff-closeout-fixtures-validation-report.v1",
...
export async function runTradingSecretScanRemediationReceiptChainSignoffCloseoutFixturesCli(argv = process.argv.slice(2)) {
  ...
  try {
    const result = await runTradingSecretScanRemediationReceiptChainSignoffCloseoutFixtures(args);
    console.log(`Trading secret scan remediation receipt chain signoff closeout fixtures ${args.check ? "validated" : "written"} at ${result.output_dir}`);
    console.log(`Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_status}`);
...
function buildIntakeAnchor(signoffApprovalCloseout) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-signoff-closeout-fixtures-anchor.v1",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_signoff_approval_closeout_fixtures_id: signoffApprovalCloseout.trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout_fixtures_id,
    source_secret_scan_remediation_receipt_chain_signoff_approval_closeout_status: signoffApprovalCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout_fixtures_status,
    source_hash: hashValue({
      id: signoffApprovalCloseout.trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout_fixtures_id,
      status: signoffApprovalCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout_fixtures_status,
...
function buildIntakeRows(signoffApprovalCloseout) {
  const sourceReady = signoffApprovalCloseout.validation.valid && signoffApprovalCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout_fixtures_status === SOURCE_READY_STATUS;
  return signoffApprovalCloseout.secret_scan_remediation_receipt_chain_signoff_approval_closeout_rows.map((sourceRow, index) => {
    const closeoutReady = sourceReady && sourceRow.chain_signoff_approval_closeout_status === SOURCE_APPROVAL_CLOSEOUT_READY_STATUS;
    const row = {
      schema_version: "trading-secret-scan-remediation-receipt-chain-signoff-closeout-row.v1",
      secret_scan_remediation_receipt_chain_signoff_closeout_row_id: `trading-secret-scan-remediation-receipt-chain-signoff-closeout.row.${sourceRow.source_phase_slot.toLowerCase()}`,
      phase_slot: PHASE_SLOT,
      source_chain_signoff_approval_closeout_row_id: sourceRow.secret_scan_remediation_receipt_chain_signoff_approval_closeout_row_id,
      ...
      source_secret_scan_remediation_receipt_chain_signoff_approval_closeout_status: signoffApprovalCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout_fixtures_status,
      source_secret_scan_remediation_receipt_chain_signoff_validation_rules_status: sourceRow.source_secret_scan_remediation_receipt_chain_signoff_validation_rules_status,
      ...
      secret_scan_remediation_receipt_chain_signoff_approval_closeout_declared: sourceRow.secret_scan_remediation_receipt_chain_signoff_approval_closeout_declared,
      secret_scan_remediation_receipt_chain_signoff_closeout_declared: true,
    ...
    return withOrdinalAndHash(row, index, "secret_scan_remediation_receipt_chain_signoff_closeout_hash");
  });
}
...
function buildIntakeBoundary({ generatedAt, writeRequested, signoffApprovalCloseout, intakeRows }) {
  ...
  const sourceReady = signoffApprovalCloseout.validation.valid && sourceSummary.trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout_fixtures_status === SOURCE_READY_STATUS;
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-signoff-closeout-fixtures-boundary.v1",
    generated_at: generatedAt,
    boundary_status: "enforced",
    phase_slot: PHASE_SLOT,
    read_only: true,
    report_only: true,
    secret_scan_remediation_receipt_chain_signoff_closeout_artifact_write_requested: writeRequested,
    source_secret_scan_remediation_receipt_chain_signoff_approval_closeout_status: sourceSummary.trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout_fixtures_status,
    source_secret_scan_remediation_receipt_chain_signoff_approval_closeout_ready: sourceReady,
    source_secret_scan_remediation_receipt_chain_signoff_validation_rules_status: sourceSummary.source_secret_scan_remediation_receipt_chain_signoff_validation_rules_status,
    source_secret_scan_remediation_receipt_chain_signoff_validation_rules_ready: sourceSummary.source_secret_scan_remediation_receipt_chain_signoff_validation_rules_ready,
...
function buildIntakeGateRows({ signoffApprovalCloseout, packageJson, platformOpsLedger, intakeRows, intakeBoundary }) {
  ...
  const sourceRowsReady = signoffApprovalCloseout.secret_scan_remediation_receipt_chain_signoff_approval_closeout_rows.length === 10 && signoffApprovalCloseout.secret_scan_remediation_receipt_chain_signoff_approval_closeout_rows.every((row) => row.chain_signoff_approval_closeout_status === SOURCE_APPROVAL_CLOSEOUT_READY_STATUS && row.secret_scan_remediation_receipt_chain_signoff_approval_closeout_declared && row.secret_scan_remediation_receipt_chain_signoff_validation_rules_declared && !row.signoff_validation_performed && !row.signoff_receipt_received && !row.approval_closeout_completed && !row.signoff_completed && !row.signoff_approval_applied && row.no_secret_material_read && row.no_secret_or_trading_mutation);
  const rows = [
    gateRow("p445_signoff_approval_closeout_ready", "P445 secret scan remediation receipt chain signoff approval closeout source is ready.", signoffApprovalCloseout.validation.valid && signoffApprovalCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout_fixtures_status === SOURCE_READY_STATUS),
    gateRow("platform_package_script_registered", "package.json registers the P446 trading secret scan remediation receipt chain signoff closeout fixtures command.", typeof scripts["trading:secret-scan-remediation-receipt-chain-signoff-closeout-fixtures"] === "string" && scripts["trading:secret-scan-remediation-receipt-chain-signoff-closeout-fixtures"].length > 0),
    gateRow("platform_validation_chain_registered", "Validation chain includes the P446 trading secret scan remediation receipt chain signoff closeout fixtures command.", validateScript.includes("npm run trading:secret-scan-remediation-receipt-chain-signoff-closeout-fixtures -- --check")),
    gateRow("p446_ledger_acceptance_declared", "P446 acceptance row is declared in the platform operations ledger.", ledgerText.includes("P446: `trading:secret-scan-remediation-receipt-chain-signoff-closeout-fixtures`")),
    gateRow("p445_approval_closeout_rows_ready", "Every P445 secret scan remediation receipt chain signoff approval closeout row is declared and still has no receipt.", sourceRowsReady),
    gateRow("chain_signoff_closeout_ready", "Every secret scan remediation receipt chain signoff closeout row declares closeout readiness while no receipt is received.", intakeRows.length === 10 && intakeRows.every((row) => row.chain_signoff_closeout_status === SIGNOFF_CLOSEOUT_READY_STATUS && row.secret_scan_remediation_receipt_chain_signoff_approval_closeout_declared && row.secret_scan_remediation_receipt_chain_signoff_closeout_declared && !row.signoff_validation_performed && !row.signoff_receipt_received && !row.approval_closeout_completed && !row.signoff_closeout_completed && !row.signoff_completed && !row.signoff_approval_applied && row.human_receipts_pending && row.no_secret_material_read && row.no_secret_or_trading_mutation && row.required_validation_rules.length >= 5)),
    gateRow("closeout_pending_no_receipt_received", "P446 declares signoff closeout without receiving receipts, validating signoffs, completing signoff, or applying approvals.", !intakeBoundary.signoff_receipt_received && !intakeBoundary.signoff_validation_performed && !intakeBoundary.approval_closeout_completed && !intakeBoundary.signoff_closeout_completed && !intakeBoundary.receipt_received && !intakeBoundary.signoff_completed && !intakeBoundary.signoff_approval_applied && intakeBoundary.human_review_required && !intakeBoundary.approval_applied),
    gateRow("no_secret_material_read", "P446 reads no secret values, env files, Desktop config content, or credentials.", intakeBoundary.no_secret_material_read && !intakeBoundary.secret_values_read && !intakeBoundary.env_file_read && !intakeBoundary.desktop_config_content_inspected && !intakeBoundary.desktop_config_read && !intakeBoundary.credential_lookup_allowed),
    gateRow("no_receipt_or_trading_mutation", "P446 does not receive, validate, apply, enable, execute, or mutate trading state.", intakeBoundary.no_secret_or_trading_mutation && !intakeBoundary.actor_workspace_input_present && !intakeBoundary.receipt_payload_present && !intakeBoundary.ready_for_validation && !intakeBoundary.ready_for_approval_application && !intakeBoundary.receipt_received && !intakeBoundary.receipt_validated && !intakeBoundary.receipt_application_performed && !intakeBoundary.approval_applied && !intakeBoundary.shadow_live_enabled && !intakeBoundary.limited_live_enabled && !intakeBoundary.full_auto_enabled && !intakeBoundary.automatic_order_submission_allowed && !intakeBoundary.live_order_submission_allowed && !intakeBoundary.secret_scan_remediation_action_allowed && !intakeBoundary.live_execution_allowed && !intakeBoundary.broker_write_allowed && !intakeBoundary.exchange_write_allowed),
    gateRow("no_command_or_artifact_mutation", "P446 signoff closeout does not execute commands, read/write artifacts, publish releases, run git, or execute protected actions.", !intakeBoundary.command_execution_performed && !intakeBoundary.package_command_execution_performed && !intakeBoundary.release_check_execution_performed && !intakeBoundary.artifact_read_performed && !intakeBoundary.artifact_write_performed && !intakeBoundary.release_published && !intakeBoundary.git_operation_performed && !intakeBoundary.protected_action_executed),
  ];
  return rows.map((row, index) => withOrdinalAndHash(row, index, "secret_scan_remediation_receipt_chain_signoff_closeout_gate_hash"));
}

function gateRow(rowKey, description, passed) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-signoff-closeout-gate-row.v1",
    secret_scan_remediation_receipt_chain_signoff_closeout_gate_row_id: `trading-secret-scan-remediation-receipt-chain-signoff-closeout-fixtures.gate.${rowKey}`,
    phase_slot: PHASE_SLOT,
    row_key: rowKey,
    description,
...
function buildValidationItems({ signoffApprovalCloseout, packageJson, platformOpsLedger, intakeRows, intakeGateRows, intakeBoundary }) {
  return [
    validationItem("source.secret_scan_remediation_receipt_chain_signoff_approval_closeout", "p445_signoff_approval_closeout_ready", signoffApprovalCloseout.validation.valid && signoffApprovalCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout_fixtures_status === SOURCE_READY_STATUS, "P445 secret scan remediation receipt chain signoff approval closeout fixtures must be ready."),
    validationItem("source.package_json", "package_json_available", packageJson.available, "package.json is readable for P446 secret scan remediation receipt chain signoff closeout fixtures."),
    validationItem("source.platform_ops_ledger", "platform_ops_ledger_available", platformOpsLedger.available, "Platform operations stability ledger is readable."),
    validationItem("secret_scan_remediation_receipt_chain_signoff_closeout_rows", "chain_signoff_closeout_ready", intakeRows.length === 10 && intakeRows.every((row) => row.chain_signoff_closeout_status === SIGNOFF_CLOSEOUT_READY_STATUS && row.secret_scan_remediation_receipt_chain_signoff_template_declared && row.secret_scan_remediation_receipt_chain_signoff_intake_queued && row.secret_scan_remediation_receipt_chain_signoff_validation_rules_declared && row.secret_scan_remediation_receipt_chain_signoff_approval_closeout_declared && row.secret_scan_remediation_receipt_chain_signoff_closeout_declared && !row.signoff_validation_performed && !row.signoff_receipt_received && !row.approval_closeout_completed && !row.signoff_closeout_completed && !row.signoff_completed && !row.signoff_approval_applied && !row.receipt_payload_present && !row.approval_applied_by_closeout && !row.secret_values_read_by_closeout && !row.credential_lookup_allowed_by_closeout && row.required_validation_rules.length >= 5 && row.p429_p438_chain_ready && row.human_receipts_pending && row.no_secret_material_read && row.no_secret_or_trading_mutation), "P429-P438 secret scan remediation receipt chain signoff closeout must be declared while receipts remain absent."),
    validationItem("secret_scan_remediation_receipt_chain_signoff_closeout_gate_rows", "chain_signoff_closeout_gates_ready", intakeGateRows.length === 10 && intakeGateRows.every((row) => row.gate_status === "ready" && !row.signoff_receipt_received_by_closeout && !row.signoff_validation_performed_by_closeout && !row.signoff_completed_by_closeout && !row.approval_applied_by_closeout && !row.secret_values_read_by_closeout && !row.credential_lookup_allowed_by_closeout && !row.protected_action_executed_by_closeout), "P446 secret scan remediation receipt chain signoff closeout gates are ready."),
    validationItem("boundary.closeout_pending", "closeout_pending_no_receipt_received", intakeBoundary.read_only && intakeBoundary.report_only && intakeBoundary.signoff_approval_closeout_consumed_in_memory && !intakeBoundary.signoff_approval_closeout_artifact_read_performed && intakeBoundary.secret_scan_remediation_receipt_chain_signoff_closeout_declared && !intakeBoundary.signoff_validation_performed && !intakeBoundary.signoff_receipt_received && !intakeBoundary.approval_closeout_completed && !intakeBoundary.signoff_closeout_completed && !intakeBoundary.review_completed && !intakeBoundary.review_approval_applied && !intakeBoundary.signoff_completed && !intakeBoundary.signoff_approval_applied && intakeBoundary.p429_p438_chain_ready && intakeBoundary.human_receipts_pending && intakeBoundary.human_review_required, "Secret scan remediation receipt chain signoff closeout rows are declared and no receipt is received."),
    validationItem("boundary.no_secret_material_read", "no_secret_material_read", intakeBoundary.no_secret_material_read && !intakeBoundary.secret_values_read && !intakeBoundary.env_file_read && !intakeBoundary.desktop_config_content_inspected && !intakeBoundary.desktop_config_read && !intakeBoundary.credential_lookup_allowed, "P446 reads no secret material or credentials."),
    validationItem("boundary.no_receipt_or_trading_mutation", "no_receipt_or_trading_mutation", intakeBoundary.no_secret_or_trading_mutation && !intakeBoundary.actor_workspace_input_present && !intakeBoundary.receipt_input_file_materialized && !intakeBoundary.merged_receipt_input_materialized && !intakeBoundary.receipt_payload_present && !intakeBoundary.ready_for_validation && !intakeBoundary.ready_for_approval_application && !intakeBoundary.receipt_received && !intakeBoundary.receipt_validated && !intakeBoundary.receipt_application_performed && !intakeBoundary.approval_applied && !intakeBoundary.shadow_live_enabled && !intakeBoundary.limited_live_enabled && !intakeBoundary.full_auto_enabled && !intakeBoundary.automatic_order_submission_allowed && !intakeBoundary.live_order_submission_allowed && !intakeBoundary.secret_scan_remediation_action_allowed && !intakeBoundary.live_execution_allowed && !intakeBoundary.broker_write_allowed && !intakeBoundary.exchange_write_allowed, "P446 does not receive/apply receipts or enable trading mutation."),
    validationItem("boundary.no_execution_or_artifacts", "no_execution_or_artifacts", !intakeBoundary.command_execution_performed && !intakeBoundary.package_command_execution_performed && !intakeBoundary.release_check_execution_performed && !intakeBoundary.artifact_read_performed && !intakeBoundary.artifact_write_performed && !intakeBoundary.release_published && !intakeBoundary.git_operation_performed && !intakeBoundary.protected_action_executed, "P446 does not execute commands or read/write artifacts."),
    validationItem("boundary.no_source_receipt_or_approval", "no_source_receipt_or_approval", !intakeBoundary.source_receipt_present && !intakeBoundary.source_approval_applied, "P446 must not inherit pre-applied secret scan remediation receipts or approvals."),
  ];
}

function buildSummary({ signoffApprovalCloseout, intakeRows, intakeGateRows, intakeBoundary, validation }) {
  return {
    trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_status: validation.valid ? READY_STATUS : "blocked",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_signoff_approval_closeout_status: signoffApprovalCloseout.summary.trading_secret_scan_remediation_receipt_chain_signoff_approval_closeout_fixtures_status,
    source_secret_scan_remediation_receipt_chain_signoff_approval_closeout_ready: intakeBoundary.source_secret_scan_remediation_receipt_chain_signoff_approval_closeout_ready,
    source_secret_scan_remediation_receipt_chain_signoff_validation_rules_status: intakeBoundary.source_secret_scan_remediation_receipt_chain_signoff_validation_rules_status,
    ...
    secret_scan_remediation_receipt_chain_signoff_closeout_declared: intakeBoundary.secret_scan_remediation_receipt_chain_signoff_closeout_declared,
...
function renderMarkdown(result) {
  const lines = [
    ...
    "",
    `Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_signoff_closeout_fixtures_status}`,
    `Phase: ${result.summary.phase_slot}`,
    `Source signoff approval closeout: ${result.summary.source_secret_scan_remediation_receipt_chain_signoff_approval_closeout_status}`,
    ...
    "## Signoff Closeout",
    "",
    ...result.secret_scan_remediation_receipt_chain_signoff_closeout_rows.map((row) => `- ${row.source_phase_slot} ${row.package_script_name}: ${row.chain_signoff_closeout_status}`),
    "",
    "## Gates",
    "",
    ...result.secret_scan_remediation_receipt_chain_signoff_closeout_gate_rows.map((row) => `- ${row.row_key}: ${row.gate_status}`),
  ];
  return `${lines.join("\n")}\n`;
}

function parseArgs(argv) {
  const parsed = { outDir: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_OUT_DIR };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--secret-scan-remediation-receipt-chain-signoff-approval-closeout-schema") parsed.secretScanRemediationReceiptChainSignoffApprovalCloseoutSchemaPath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/trading-secret-scan-remediation-receipt-chain-signoff-closeout-fixtures.mjs [options]

Options:
  --out-dir <folder>                         Output directory. Default: ${DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_OUT_DIR}
  ...
  --secret-scan-remediation-receipt-chain-signoff-approval-closeout-schema <path>
                                             P445 secret scan remediation receipt chain signoff approval closeout fixtures schema path.
...
function normalizeInputs(options = {}) {
  return {
    package_path: path.resolve(options.packagePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.packagePath),
    platform_ops_ledger_path: path.resolve(options.platformOpsLedgerPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.platformOpsLedgerPath),
    secret_scan_remediation_receipt_workspace_schema_path: path.resolve(options.secretScanRemediationReceiptWorkspaceSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptWorkspaceSchemaPath),
    secret_scan_remediation_receipt_workspace_merge_schema_path: path.resolve(options.secretScanRemediationReceiptWorkspaceMergeSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptWorkspaceMergeSchemaPath),
    secret_scan_remediation_receipt_merge_preflight_schema_path: path.resolve(options.secretScanRemediationReceiptMergePreflightSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptMergePreflightSchemaPath),
    secret_scan_remediation_receipt_validation_packet_schema_path: path.resolve(options.secretScanRemediationReceiptValidationPacketSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptValidationPacketSchemaPath),
    secret_scan_remediation_receipt_approval_plan_schema_path: path.resolve(options.secretScanRemediationReceiptApprovalPlanSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptApprovalPlanSchemaPath),
    secret_scan_remediation_receipt_approval_closeout_schema_path: path.resolve(options.secretScanRemediationReceiptApprovalCloseoutSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptApprovalCloseoutSchemaPath),
    secret_scan_remediation_receipt_closeout_schema_path: path.resolve(options.secretScanRemediationReceiptCloseoutSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptCloseoutSchemaPath),
    secret_scan_remediation_receipt_chain_regression_schema_path: path.resolve(options.secretScanRemediationReceiptChainRegressionSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptChainRegressionSchemaPath),
    secret_scan_remediation_receipt_chain_review_schema_path: path.resolve(options.secretScanRemediationReceiptChainReviewSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptChainReviewSchemaPath),
    secret_scan_remediation_receipt_chain_signoff_ledger_schema_path: path.resolve(options.secretScanRemediationReceiptChainSignoffLedgerSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptChainSignoffLedgerSchemaPath),
    secret_scan_remediation_receipt_chain_signoff_template_schema_path: path.resolve(options.secretScanRemediationReceiptChainSignoffTemplateSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptChainSignoffTemplateSchemaPath),
    secret_scan_remediation_receipt_chain_signoff_intake_schema_path: path.resolve(options.secretScanRemediationReceiptChainSignoffIntakeSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptChainSignoffIntakeSchemaPath),
    secret_scan_remediation_receipt_chain_signoff_validation_rules_schema_path: path.resolve(options.secretScanRemediationReceiptChainSignoffValidationRulesSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptChainSignoffValidationRulesSchemaPath),
    secret_scan_remediation_receipt_chain_signoff_approval_closeout_schema_path: path.resolve(options.secretScanRemediationReceiptChainSignoffApprovalCloseoutSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.secretScanRemediationReceiptChainSignoffApprovalCloseoutSchemaPath),
    schema_path: path.resolve(options.schemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SIGNOFF_CLOSEOUT_FIXTURES_INPUTS.schemaPath),
  };
}
