...
import {
  ALL_FALSE_FLAGS as P42800_FALSE_FLAGS,
  buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyPlan,
} from "./work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-plan.mjs";

export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_ASSEMBLY_HANDOFF_OUT_DIR = "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-handoff/latest";
export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_ASSEMBLY_HANDOFF_INPUTS = {
  schemaPath: "schemas/work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-handoff.schema.json",
  packagePath: "package.json",
  roadmapDocPath: "docs/hermes-roadmap-p42801-p43200.md",
  architectureDocPath: "docs/architecture.md",
  sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyPlanPath: "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-plan/latest/work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-plan.json",
};

const COMMAND_NAME = "platform:work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-handoff";
const SCHEMA_VERSION = "work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-handoff.v1";
const CAPABILITY_ID = "platform.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff";
const PROGRAM_RANGE = "P42801-P43200";
const SOURCE_PROGRAM_RANGE = "P42401-P42800";
const READY_STATUS = "ready_for_work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff";
const BLOCK_PENDING_STATUS = "valid_block_work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_pending";
const BLOCKED_STATUS = "blocked_work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff";
...
export const ALL_FALSE_FLAGS = [...new Set([...STATIC_SHELL_ASSEMBLY_HANDOFF_FALSE_FLAGS, ...P42800_FALSE_FLAGS])];

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoff(options = {}) {
  const result = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoff(options);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Work OS Static Bundle Review UI Implementation Review Static Shell Assembly Handoff failed with ${result.validation.errors.length} error(s).`);
    error.validation = result.validation;
    throw error;
  }
  if (!options.check && options.write !== false) await writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoff(result, result.output_dir);
  return result;
}

export async function buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoff(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_ASSEMBLY_HANDOFF_OUT_DIR);
  const inputs = normalizeInputs(options);
  const schema = await readJsonSource(inputs.schema_path);
  const packageJson = await readJsonSource(inputs.package_path);
  const roadmapDoc = await readTextSource(inputs.roadmap_doc_path);
  const architectureDoc = await readTextSource(inputs.architecture_doc_path);
  const source = Object.prototype.hasOwnProperty.call(options, "workOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyPlan")
    ? normalizeInlineJsonSource("inline.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_plan", options.workOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyPlan)
    : await readJsonOrBuildP42800(inputs.source_work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_plan_path, generatedAt);
  ...
  const result = {
    ...
    inputs,
    source_refs: {
      work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_plan_path: source.path,
      source_commit_ref: commitRef || null,
    },
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_plan_summary: source.data?.summary ?? null,
    work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_contract: buildContract(generatedAt),
    work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_phase_rows: phaseRows,
    p42800_source_binding_rows: sourceRows,
    ...
    p43200_clean_checkpoint_rows: checkpointRows,
    work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_boundary: boundary,
    work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_validation_items: validationItems,
    validation: preliminaryValidation,
    summary: buildSummary({ boundary, validation: preliminaryValidation }),
  };

  const schemaErrors = schema.available
    ? validateAgainstSchema(result, schema.data, {}, "work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff")
    : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  const schemaValidationItems = schemaErrors.map((error, index) => validationItem(`schema.${index}`, "schema_validation", false, error.message));
  result.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_validation_items = [...validationItems, ...schemaValidationItems];
  result.validation = summarizeValidation(result.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_validation_items);
  result.summary = buildSummary({ boundary, validation: result.validation });
  return { ...result, markdown: renderMarkdown(result), html: renderHtml(result) };
}

export async function writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoff(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-handoff.json"), serializableResult(result));
  await writeJson(path.join(outDir, "p42800-source-binding-rows.json"), collectionEnvelope("p42800-source-binding-rows.v1", "p42800_source_binding_rows", result.p42800_source_binding_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "p43200-clean-checkpoint-rows.json"), collectionEnvelope("p43200-clean-checkpoint-rows.v1", "p43200_clean_checkpoint_rows", result.p43200_clean_checkpoint_rows, result.generated_at));
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-handoff-boundary.json"), result.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_boundary);
  await writeFile(path.join(outDir, "summary.md"), result.markdown, "utf8");
  await writeFile(path.join(outDir, "index.html"), result.html, "utf8");
}

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoffCli(argv = process.argv.slice(2)) {
  const args = parseArgs(argv);
  if (args.help) {
    printHelp();
    return null;
  }
  const result = await runWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoff(args);
  console.log(`Work OS Static Bundle Review UI Implementation Review Static Shell Assembly Handoff ${args.check ? "validated" : "written"} at ${result.output_dir}`);
  console.log(`Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_status}`);
  console.log(`Program: ${result.program_range}`);
  console.log(`P42800 ready for P42801 handoff: ${result.summary.source_p42800_ready_for_p42801_handoff}`);
  ...
  console.log(`Ready for P43201 handoff: ${result.summary.ready_for_p43201_handoff}`);
...
function buildSourceState(source) {
  ...
  const boundary = source.data?.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_plan_boundary ?? {};
  return {
    available: source.available === true,
    programRangeOk: source.data?.program_range === SOURCE_PROGRAM_RANGE,
    validationValid: source.data?.validation?.valid === true,
    sourceReady: summary.ready_for_p42801_handoff === true,
    status: summary.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_plan_status ?? "missing",
    p42800ContractReady: boundary.p42800_contract_ready === true,
    ...
    boundaryClosed: P42800_FALSE_FLAGS.every((flag) => boundary[flag] === false),
  };
}
...
function buildPhaseRows(roadmapText, generatedAt) {
  return PHASE_SPECS.map(([range, label, outputRef]) => row({
    ...
    evidence_ref: "docs/hermes-roadmap-p42801-p43200.md",
...
function buildSourceRows({ source, sourceState, commitRef, generatedAt }) {
  return [
    ["artifact_available", "P42800 implementation review static shell assembly plan source is available", sourceState.available],
    ["program_range", "P42800 source program range is P42401-P42800", sourceState.programRangeOk],
    ["validation_valid", "P42800 source validation is valid", sourceState.validationValid],
    ["p42801_handoff_open", "P42800 source opened P42801 handoff", sourceState.sourceReady],
    ["p42800_contract_ready", "P42800 source contract is ready", sourceState.p42800ContractReady],
    ["assembly_plan_visible", "P42800 static shell assembly plan rows are visible", sourceState.assemblyPlanVisible],
    ["template_composition_visible", "P42800 template composition rows are visible", sourceState.templateCompositionVisible],
    ["state_slot_visible", "P42800 read-only state slot rows are visible", sourceState.readOnlyStateSlotVisible],
    ["blocked_copy_guard_visible", "P42800 accessibility and blocked copy guard rows are visible", sourceState.accessibilityBlockedCopyVisible],
    ["no_build_no_render_closed", "P42800 no-build/no-render boundary is closed", sourceState.noBuildNoRenderClosed],
    ["commit_ref_present", "Current commit ref is present for static shell assembly handoff", Boolean(commitRef)],
    ["source_blocker_visible", "P42800 source blocker is visible when assembly handoff is closed", sourceState.available],
  ].map(([id, label, observed]) => row({
    row_id: `source_binding.${id}`,
    category: "p42800_source_binding",
    label,
    observed,
...
function buildNoApplyNoBuildBoundaryRows(generatedAt) {
  return ALL_FALSE_FLAGS.map((flag) => row({
    ...
    observed: true,
    evidence_ref: "work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_boundary",
    boundary_flag: flag,
    allowed_now: false,
    generated_at: generatedAt,
  }));
...
function buildCheckpointRows(context) {
  ...
  return [
    ["source_ready", "P42800 source is ready for P42801", context.sourceState.sourceReady],
    ...
    ["p43201_handoff_gate", "P43201 handoff opens only when static shell assembly handoff conditions pass", handoffReady],
    ["p43201_handoff_blocker_visible", "P43201 handoff blocker is visible when the gate is closed", true],
  ].map(([id, label, observed]) => row({
    row_id: `p43200_checkpoint.${id}`,
    category: "p43200_clean_checkpoint",
    label,
    observed,
    evidence_ref: "p43200_clean_checkpoint_rows",
    generated_at: context.generatedAt,
  }));
}

function buildBoundary(context) {
  const p43200ContractReady = allPass(context.phaseRows)
    ...
    && allPass(context.assetRows)
    && allPass(context.boundaryRows)
    && visibleOrPassed(context.checkpointRows, "p43200_checkpoint.p43201_handoff_blocker_visible");
  const handoffReady = context.sourceState.sourceReady
    && context.sourceState.validationValid
    && context.sourceState.boundaryClosed
    && allPass(context.handoffRows)
    ...
    && allPass(context.boundaryRows);
  return {
    p43200_contract_ready: p43200ContractReady,
    ready_for_p43201_handoff: handoffReady,
    source_p42800_ready_for_p42801_handoff: context.sourceState.sourceReady,
...
function buildValidationItems(context) {
  return [
    ...
    validationItem("roadmap.phase_coverage", "roadmap", allPass(context.phaseRows), "P42801-P43200 phase rows are incomplete"),
    validationItem("architecture.reference", "architecture", context.architectureDoc.text.includes("P42801-P43200 Work OS Static Bundle Review UI Implementation Review Static Shell Assembly Handoff"), "Architecture doc missing P42801-P43200 reference"),
    validationItem("source.visible", "source", visibleOrPassed(context.sourceRows, "source_binding.source_blocker_visible"), "P42800 source state is not visible"),
    ...
    validationItem("checkpoint.visible", "checkpoint", visibleOrPassed(context.checkpointRows, "p43200_checkpoint.p43201_handoff_blocker_visible"), "P43200 checkpoint blocker is not visible"),
  ];
}

function buildContract(generatedAt) {
  return {
    contract_id: "work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff.contract.v1",
    generated_at: generatedAt,
    source_p42800_required_or_rebuilt: true,
    ...
    p43201_handoff_is_not_apply_build_route_dom_browser_action_write_approval_closeout_or_enterprise_trust: true,
    protected_authority: "closed",
  };
}

function buildSummary({ boundary, validation }) {
  const status = validation.valid && boundary.ready_for_p43201_handoff
    ? READY_STATUS
    : validation.valid && boundary.p43200_contract_ready
      ? BLOCK_PENDING_STATUS
      : BLOCKED_STATUS;
  return {
    work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_status: status,
    source_p42800_ready_for_p42801_handoff: boundary.source_p42800_ready_for_p42801_handoff,
    ...
    ready_for_p43201_handoff: validation.valid && boundary.ready_for_p43201_handoff,
...
function renderMarkdown(result) {
  return [
    "# Work OS Static Bundle Review UI Implementation Review Static Shell Assembly Handoff",
    "",
    `Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_status}`,
    `Program: ${result.program_range}`,
    `P42800 ready for P42801 handoff: ${result.summary.source_p42800_ready_for_p42801_handoff}`,
    ...
    `Ready for P43201 handoff: ${result.summary.ready_for_p43201_handoff}`,
...
<head>
  ...
  <title>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Assembly Handoff</title>
  <style>
...
<body>
  <main>
    <h1>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Assembly Handoff</h1>
...
async function readJsonOrBuildP42800(filePath, generatedAt) {
  const source = await readJsonSource(filePath);
  if (source.available) return source;
  const built = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyPlan({ runAt: generatedAt, write: false });
  return normalizeInlineJsonSource("built.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_plan", built);
}
...
function normalizeInputs(options) {
  const defaults = DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_ASSEMBLY_HANDOFF_INPUTS;
  ...
  return {
    ...
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_plan_path: path.resolve(repoRoot, options.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyPlanPath ?? defaults.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyPlanPath),
  };
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--check") {
      ...
      args.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyPlanPath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-handoff.mjs [--check] [--out-dir DIR] [--source FILE] [--commit-ref SHA]

Validates or writes the Hermes P42801-P43200 Work OS Static Bundle Review UI Implementation Review Static Shell Assembly Handoff.
`);
}
