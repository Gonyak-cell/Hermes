...
const SOURCE_DEFINITIONS = [
  ...
  {
    option: "policySnapshotEventBindingPath",
    source_id: "policy_snapshot_event_binding",
    label: "Policy Snapshot Event Binding",
  },
  {
...
function summarizeSource(sourceId, data) {
  ...
  if (sourceId === "policy_snapshot_event_binding") return data.summary ?? {};
...
function buildStageStatuses(artifacts, sources) {
  ...
  return [
    ...
    buildPolicySnapshotEventBindingStage(artifacts.policy_snapshot_event_binding, sourceById.get("policy_snapshot_event_binding")),
...
function buildPolicySnapshotEventBindingStage(ledger, source) {
  if (!ledger) return missingStage("policy_snapshot_event_binding", "Policy Snapshot Event Binding", source);
  const summary = ledger.summary ?? {};
  const errorCount = summary.validation_error_count ?? ledger.validation?.errors?.length ?? 0;
  const bindingCount = summary.event_run_gate_policy_binding_count ?? 0;
  const storedCheckedCount = summary.stored_event_policy_snapshot_checked_count ?? 0;
  const status = summary.policy_snapshot_event_binding_status === "complete"
    && errorCount === 0
    && bindingCount > 0
    && (summary.source_policy_snapshot_present_count ?? 0) === bindingCount
    && (summary.resolved_policy_snapshot_known_count ?? 0) === bindingCount
    && (summary.stored_event_policy_snapshot_matched_count ?? 0) === storedCheckedCount
    ? "passed"
    : "blocked";
  return {
    stage_id: "policy_snapshot_event_binding",
    label: "Policy Snapshot Event Binding",
    status,
    message: status === "passed"
      ? `${bindingCount} event/run/gate binding(s), ${storedCheckedCount} stored event snapshot(s) preserved.`
      : `${errorCount} validation error(s), ${summary.source_policy_snapshot_present_count ?? 0}/${bindingCount} source snapshot(s), ${summary.resolved_policy_snapshot_known_count ?? 0}/${bindingCount} known resolved snapshot(s).`,
    source_path: source?.path ?? null,
    metrics: {
      policy_snapshot_event_binding_status: summary.policy_snapshot_event_binding_status ?? "unknown",
      event_run_gate_policy_binding_count: bindingCount,
      event_policy_snapshot_binding_count: summary.event_policy_snapshot_binding_count ?? 0,
      run_policy_snapshot_binding_count: summary.run_policy_snapshot_binding_count ?? 0,
      event_run_policy_snapshot_binding_count: summary.event_run_policy_snapshot_binding_count ?? 0,
      gate_policy_snapshot_binding_count: summary.gate_policy_snapshot_binding_count ?? 0,
      source_policy_snapshot_present_count: summary.source_policy_snapshot_present_count ?? 0,
      resolved_policy_snapshot_known_count: summary.resolved_policy_snapshot_known_count ?? 0,
      bound_policy_snapshot_binding_count: summary.bound_policy_snapshot_binding_count ?? 0,
      source_snapshot_matched_count: summary.source_snapshot_matched_count ?? 0,
      source_unresolved_resolved_count: summary.source_unresolved_resolved_count ?? 0,
      source_snapshot_mismatch_count: summary.source_snapshot_mismatch_count ?? 0,
      execution_time_recorded_count: summary.execution_time_recorded_count ?? 0,
      stored_event_policy_snapshot_checked_count: storedCheckedCount,
      stored_event_policy_snapshot_matched_count: summary.stored_event_policy_snapshot_matched_count ?? 0,
      stored_event_policy_snapshot_missing_count: summary.stored_event_policy_snapshot_missing_count ?? 0,
      stored_event_policy_snapshot_mismatch_count: summary.stored_event_policy_snapshot_mismatch_count ?? 0,
      gate_event_linked_count: summary.gate_event_linked_count ?? 0,
      gate_event_missing_count: summary.gate_event_missing_count ?? 0,
      validation_error_count: errorCount,
    },
  };
}
...
function buildCostRecordProjectionStage(ledger, source) {
  if (!ledger) return missingStage("cost_record_projection", "Cost Record Projection", source);
  const summary = ledger.summary ?? {};
  const errorCount = summary.validation_error_count ?? ledger.validation?.errors?.length ?? 0;
  const missingRuns = summary.missing_run_cost_rollup_count ?? 0;
  const missingRecords = summary.run_missing_record_count ?? 0;
  const status = summary.cost_record_projection_status === "complete" && errorCount === 0 && missingRuns === 0 && missingRecords === 0 ? "passed" : "blocked";
  return {
    stage_id: "cost_record_projection",
    label: "Cost Record Projection",
    status,
    message: status === "passed"
      ? `${summary.projected_cost_record_count ?? 0} projected cost record(s), ${summary.run_cost_rollup_count ?? 0} run rollup(s), provider/runtime/storage/API ${summary.provider_cost_record_count ?? 0}/${summary.runtime_cost_record_count ?? 0}/${summary.storage_cost_record_count ?? 0}/${summary.api_cost_record_count ?? 0}.`
      : `${missingRuns} missing run rollup(s), ${missingRecords} unattributed record(s), ${errorCount} validation error(s).`,
    source_path: source?.path ?? null,
    metrics: {
      cost_record_projection_status: summary.cost_record_projection_status ?? "unknown",
      projected_cost_record_count: summary.projected_cost_record_count ?? 0,
      raw_cost_record_count: summary.raw_cost_record_count ?? 0,
      provider_cost_record_count: summary.provider_cost_record_count ?? 0,
      runtime_cost_record_count: summary.runtime_cost_record_count ?? 0,
      storage_cost_record_count: summary.storage_cost_record_count ?? 0,
      api_cost_record_count: summary.api_cost_record_count ?? 0,
      run_cost_rollup_count: summary.run_cost_rollup_count ?? 0,
      attributed_run_cost_rollup_count: summary.attributed_run_cost_rollup_count ?? 0,
      missing_run_cost_rollup_count: missingRuns,
      run_missing_record_count: missingRecords,
      priced_record_count: summary.priced_record_count ?? 0,
      estimated_record_count: summary.estimated_record_count ?? 0,
      unpriced_record_count: summary.unpriced_record_count ?? 0,
      total_observed_usd: summary.total_observed_usd ?? 0,
      total_estimated_usd: summary.total_estimated_usd ?? 0,
      total_projected_usd: summary.total_projected_usd ?? 0,
      total_token_count: summary.total_token_count ?? 0,
      total_runtime_seconds: summary.total_runtime_seconds ?? 0,
...
function buildWorkflowDslStateModelStage(model, source) {
  if (!model) return missingStage("workflow_dsl_state_model", "Workflow DSL State Model", source);
  const summary = model.summary ?? {};
  const blockers = (summary.validation_error_count ?? model.validation?.errors?.length ?? 0)
    + (summary.unknown_source_state_count ?? 0)
    + (summary.blocked_projection_count ?? 0);
  const status = summary.workflow_dsl_state_model_status === "complete" && blockers === 0 ? "passed" : "blocked";
  return {
    stage_id: "workflow_dsl_state_model",
    label: "Workflow DSL State Model",
    status,
    message: status === "passed"
      ? `${summary.dsl_state_count ?? 0} DSL states, ${summary.workflow_run_projection_count ?? 0} run projection(s), ${summary.human_review_waiting_count ?? 0} waiting review run(s).`
      : `${blockers} workflow DSL state blocker(s).`,
    source_path: source?.path ?? null,
    metrics: {
      workflow_dsl_state_model_status: summary.workflow_dsl_state_model_status ?? "unknown",
      state_model_version: summary.state_model_version ?? null,
      dsl_state_count: summary.dsl_state_count ?? 0,
      required_state_count: summary.required_state_count ?? 0,
      terminal_state_count: summary.terminal_state_count ?? 0,
      transition_rule_count: summary.transition_rule_count ?? 0,
      workflow_blueprint_count: summary.workflow_blueprint_count ?? summary.workflow_state_blueprint_count ?? 0,
      workflow_run_projection_count: summary.workflow_run_projection_count ?? summary.workflow_run_state_projection_count ?? 0,
      clear_projection_count: summary.clear_projection_count ?? 0,
      blocked_projection_count: summary.blocked_projection_count ?? 0,
      waiting_run_count: summary.waiting_run_count ?? summary.waiting_projection_count ?? 0,
      gated_run_count: summary.gated_run_count ?? 0,
      approved_run_count: summary.approved_run_count ?? 0,
      completed_run_count: summary.completed_run_count ?? 0,
      failed_run_count: summary.failed_run_count ?? 0,
      unknown_source_state_count: summary.unknown_source_state_count ?? 0,
...
function buildDashboardSummary(artifacts, stageStatuses, actionItems) {
  ...
  return {
    ...
    workflow_dsl_state_model_status: artifacts.workflow_dsl_state_model?.summary?.workflow_dsl_state_model_status ?? "unknown",
    workflow_dsl_state_model_state_model_version: artifacts.workflow_dsl_state_model?.summary?.state_model_version ?? null,
    workflow_dsl_state_model_dsl_state_count: artifacts.workflow_dsl_state_model?.summary?.dsl_state_count ?? 0,
    workflow_dsl_state_model_required_state_count: artifacts.workflow_dsl_state_model?.summary?.required_state_count ?? 0,
    workflow_dsl_state_model_terminal_state_count: artifacts.workflow_dsl_state_model?.summary?.terminal_state_count ?? 0,
    workflow_dsl_state_model_transition_rule_count: artifacts.workflow_dsl_state_model?.summary?.transition_rule_count ?? 0,
    workflow_dsl_state_model_workflow_blueprint_count: artifacts.workflow_dsl_state_model?.summary?.workflow_blueprint_count ?? artifacts.workflow_dsl_state_model?.summary?.workflow_state_blueprint_count ?? 0,
    workflow_dsl_state_model_workflow_run_projection_count: artifacts.workflow_dsl_state_model?.summary?.workflow_run_projection_count ?? artifacts.workflow_dsl_state_model?.summary?.workflow_run_state_projection_count ?? 0,
    workflow_dsl_state_model_clear_projection_count: artifacts.workflow_dsl_state_model?.summary?.clear_projection_count ?? 0,
    workflow_dsl_state_model_blocked_projection_count: artifacts.workflow_dsl_state_model?.summary?.blocked_projection_count ?? 0,
    workflow_dsl_state_model_waiting_run_count: artifacts.workflow_dsl_state_model?.summary?.waiting_run_count ?? artifacts.workflow_dsl_state_model?.summary?.waiting_projection_count ?? 0,
    workflow_dsl_state_model_gated_run_count: artifacts.workflow_dsl_state_model?.summary?.gated_run_count ?? 0,
    workflow_dsl_state_model_approved_run_count: artifacts.workflow_dsl_state_model?.summary?.approved_run_count ?? 0,
    workflow_dsl_state_model_completed_run_count: artifacts.workflow_dsl_state_model?.summary?.completed_run_count ?? 0,
    workflow_dsl_state_model_failed_run_count: artifacts.workflow_dsl_state_model?.summary?.failed_run_count ?? 0,
    workflow_dsl_state_model_unknown_source_state_count: artifacts.workflow_dsl_state_model?.summary?.unknown_source_state_count ?? 0,
    ...
    policy_snapshot_event_binding_status: artifacts.policy_snapshot_event_binding?.summary?.policy_snapshot_event_binding_status ?? "unknown",
    policy_snapshot_event_binding_count: artifacts.policy_snapshot_event_binding?.summary?.event_run_gate_policy_binding_count ?? 0,
    policy_snapshot_event_binding_event_count: artifacts.policy_snapshot_event_binding?.summary?.event_policy_snapshot_binding_count ?? 0,
    policy_snapshot_event_binding_run_count: artifacts.policy_snapshot_event_binding?.summary?.run_policy_snapshot_binding_count ?? 0,
    policy_snapshot_event_binding_event_run_count: artifacts.policy_snapshot_event_binding?.summary?.event_run_policy_snapshot_binding_count ?? 0,
    policy_snapshot_event_binding_gate_count: artifacts.policy_snapshot_event_binding?.summary?.gate_policy_snapshot_binding_count ?? 0,
    policy_snapshot_event_binding_source_present_count: artifacts.policy_snapshot_event_binding?.summary?.source_policy_snapshot_present_count ?? 0,
    policy_snapshot_event_binding_known_count: artifacts.policy_snapshot_event_binding?.summary?.resolved_policy_snapshot_known_count ?? 0,
    policy_snapshot_event_binding_stored_checked_count: artifacts.policy_snapshot_event_binding?.summary?.stored_event_policy_snapshot_checked_count ?? 0,
    policy_snapshot_event_binding_stored_matched_count: artifacts.policy_snapshot_event_binding?.summary?.stored_event_policy_snapshot_matched_count ?? 0,
    policy_snapshot_event_binding_gate_event_linked_count: artifacts.policy_snapshot_event_binding?.summary?.gate_event_linked_count ?? 0,
    policy_snapshot_event_binding_validation_error_count: artifacts.policy_snapshot_event_binding?.summary?.validation_error_count ?? artifacts.policy_snapshot_event_binding?.validation?.errors?.length ?? 0,
    ...
    cost_record_projection_status: artifacts.cost_record_projection?.summary?.cost_record_projection_status ?? "unknown",
    cost_record_projection_count: artifacts.cost_record_projection?.summary?.projected_cost_record_count ?? 0,
    cost_record_projection_raw_count: artifacts.cost_record_projection?.summary?.raw_cost_record_count ?? 0,
    cost_record_projection_provider_count: artifacts.cost_record_projection?.summary?.provider_cost_record_count ?? 0,
    cost_record_projection_runtime_count: artifacts.cost_record_projection?.summary?.runtime_cost_record_count ?? 0,
    cost_record_projection_storage_count: artifacts.cost_record_projection?.summary?.storage_cost_record_count ?? 0,
    cost_record_projection_api_count: artifacts.cost_record_projection?.summary?.api_cost_record_count ?? 0,
    cost_record_projection_run_rollup_count: artifacts.cost_record_projection?.summary?.run_cost_rollup_count ?? 0,
    cost_record_projection_attributed_run_count: artifacts.cost_record_projection?.summary?.attributed_run_cost_rollup_count ?? 0,
    cost_record_projection_missing_run_count: artifacts.cost_record_projection?.summary?.missing_run_cost_rollup_count ?? 0,
    cost_record_projection_run_missing_record_count: artifacts.cost_record_projection?.summary?.run_missing_record_count ?? 0,
    cost_record_projection_priced_count: artifacts.cost_record_projection?.summary?.priced_record_count ?? 0,
    cost_record_projection_estimated_count: artifacts.cost_record_projection?.summary?.estimated_record_count ?? 0,
    cost_record_projection_unpriced_count: artifacts.cost_record_projection?.summary?.unpriced_record_count ?? 0,
    cost_record_projection_total_estimated_usd: artifacts.cost_record_projection?.summary?.total_estimated_usd ?? 0,
    cost_record_projection_total_projected_usd: artifacts.cost_record_projection?.summary?.total_projected_usd ?? 0,
    cost_record_projection_total_tokens: artifacts.cost_record_projection?.summary?.total_token_count ?? 0,
    cost_record_projection_total_runtime_seconds: artifacts.cost_record_projection?.summary?.total_runtime_seconds ?? 0,
...
export function renderReviewDashboardMarkdown(dashboard) {
  ...
  lines.push(`- Cost record projections: ${dashboard.summary.cost_record_projection_count ?? 0}`);
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--no-policy-snapshot-event-binding") parsed.policySnapshotEventBindingPath = false;
...
Options:
  ...
  --policy-snapshot-event-binding <path>
                                  policy-snapshot-event-binding.json path.
  --no-policy-snapshot-event-binding
                                  Do not include Policy Snapshot Event Binding status.
