...
test("renderer navigation maps to read-model sections deterministically", () => {
  ...
  assert.equal(sectionForNav("agents"), "agents");
...
test("renderer queue starts with blockers then falls back to active operating rows", () => {
  const readModel = {
    source_rows: [
      ...
      { source_id: "agents", section_id: "agents", status: "ready" },
  ...
  assert.deepEqual(rowsForNav("sources", readModel).map((row) => row.source_id), ["release", "factory", "review", "agents", "governance"]);
  assert.deepEqual(rowsForNav("agents", readModel).map((row) => row.source_id), ["agents"]);
  ...
  assert.deepEqual(rowsForNav("queue", allReady).map((row) => row.source_id), ["release", "factory", "review", "agents"]);
});
