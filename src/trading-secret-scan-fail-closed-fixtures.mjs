...
import {
  DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_INPUTS,
  buildTradingSecretScanAttentionFixtures,
} from "./trading-secret-scan-attention-fixtures.mjs";

export const DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_OUT_DIR = "artifacts/trading-secret-scan-fail-closed-fixtures/latest";
export const DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS = {
  ...DEFAULT_SECRETS_SCAN_GATE_INPUTS,
  ...DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_INPUTS,
  secretScanAttentionSchemaPath: DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_INPUTS.schemaPath,
  schemaPath: "schemas/trading/trading-secret-scan-fail-closed-fixtures.schema.json",
};

const SCHEMA_VERSION = "trading-secret-scan-fail-closed-fixtures.v1";
const CAPABILITY_ID = "trading.secret_scan_fail_closed_fixtures";
const PHASE_SLOT = "P427";
const PREVIOUS_PHASE_SLOT = "P426";
const NEXT_PHASE_SLOT = "P428";
const READY_STATUS = "ready_for_trading_secret_scan_fail_closed";
const SOURCE_READY_STATUS = "ready_for_trading_secret_scan_attention";
const SECRETS_SCAN_GATE_READY_STATUS = "complete";
const COMMAND_NAME = "trading:secret-scan-fail-closed-fixtures";
...
const REQUIRED_ROW_KEYS = [
  "p426_secret_scan_attention_ready",
...
export async function runTradingSecretScanFailClosedFixtures(options = {}) {
  const result = await buildTradingSecretScanFailClosedFixtures(options);
  if (options.write !== false) await writeTradingSecretScanFailClosedFixtures(result, result.output_dir);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Trading secret scan fail-closed fixtures failed with ${result.validation.errors.length} error(s).`);
...
export async function buildTradingSecretScanFailClosedFixtures(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_OUT_DIR);
  const inputs = normalizeInputs(options);
  const secretScanAttention = await buildTradingSecretScanAttentionFixtures({
    ...options,
    runAt: generatedAt,
    packagePath: inputs.package_path,
    platformOpsLedgerPath: inputs.platform_ops_ledger_path,
    secretScanGateFixturesSchemaPath: inputs.secret_scan_gate_fixtures_schema_path,
    ...
    schemaPath: inputs.secret_scan_attention_schema_path,
    write: false,
  });
  ...
  const sourceReady = secretScanAttention.validation.valid && secretScanAttention.summary.trading_secret_scan_attention_fixtures_status === SOURCE_READY_STATUS;
  ...
  const result = {
    ...
    trading_secret_scan_fail_closed_fixtures_id: `trading-secret-scan-fail-closed-fixtures.${dateStamp(generatedAt)}`,
  ...
  const schemaErrors = schema.available ? validateAgainstSchema(result, schema.data, {}, "trading_secret_scan_fail_closed_fixtures") : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  ...
  result.summary.trading_secret_scan_fail_closed_fixtures_id = result.trading_secret_scan_fail_closed_fixtures_id;
  return { ...result, markdown: renderMarkdown(result) };
}

export async function writeTradingSecretScanFailClosedFixtures(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "trading-secret-scan-fail-closed-fixtures.json"), serializableResult(result));
  await writeJson(path.join(outDir, "secret-scan-fail-closed-rows.json"), collectionEnvelope("trading-secret-scan-fail-closed-rows.v1", "secret_scan_fail_closed_rows", result.secret_scan_fail_closed_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-scan-fail-closed-gate-rows.json"), collectionEnvelope("trading-secret-scan-fail-closed-gate-rows.v1", "secret_scan_fail_closed_gate_rows", result.secret_scan_fail_closed_gate_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "validation-report.json"), {
    schema_version: "trading-secret-scan-fail-closed-fixtures-validation-report.v1",
...
export async function runTradingSecretScanFailClosedFixturesCli(argv = process.argv.slice(2)) {
  ...
  try {
    const result = await runTradingSecretScanFailClosedFixtures(args);
    console.log(`Trading secret scan fail-closed fixtures ${args.check ? "validated" : "written"} at ${result.output_dir}`);
    console.log(`Status: ${result.summary.trading_secret_scan_fail_closed_fixtures_status}`);
...
function buildFailClosedCoverage({ secretScanAttention, secretsScanGate }) {
  ...
  return {
    p426_ready: secretScanAttention.validation.valid && secretScanAttention.summary.trading_secret_scan_attention_fixtures_status === SOURCE_READY_STATUS,
...
function buildFailClosedAnchor({ secretScanAttention, secretsScanGate, coverage }) {
  return {
    schema_version: "trading-secret-scan-fail-closed-fixtures-anchor.v1",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_attention_fixtures_id: secretScanAttention.trading_secret_scan_attention_fixtures_id,
    source_secret_scan_attention_status: secretScanAttention.summary.trading_secret_scan_attention_fixtures_status,
    ...
    source_hash: hashValue({
      p426_id: secretScanAttention.trading_secret_scan_attention_fixtures_id,
      p426_status: secretScanAttention.summary.trading_secret_scan_attention_fixtures_status,
...
function buildFailClosedRows({ sourceReady, secretsScanGateComplete, coverage, secretScanAttention, secretsScanGate }) {
  const rowInputs = [
    failClosedRowInput("p426_secret_scan_attention_ready", "trading_secret_scan_attention", "summary", [], [
      condition("source.trading_secret_scan_attention_fixtures_status", secretScanAttention.summary.trading_secret_scan_attention_fixtures_status, SOURCE_READY_STATUS),
...
function buildFailClosedRow(input, sourceReady, secretsScanGateComplete, coverage, index) {
  ...
  const row = {
    schema_version: "trading-secret-scan-fail-closed-row.v1",
    secret_scan_fail_closed_row_id: `trading-secret-scan-fail-closed.row.${input.rowKey}`,
    phase_slot: PHASE_SLOT,
    ...
    source_secret_scan_attention_ready: sourceReady,
...
function buildFailClosedBoundary({ generatedAt, writeRequested, sourceReady, secretsScanGateComplete, coverage, secretScanAttention, secretsScanGate, rows }) {
  ...
  const p300Boundary = secretsScanGate.secrets_scan_boundary ?? {};
  return {
    schema_version: "trading-secret-scan-fail-closed-fixtures-boundary.v1",
    generated_at: generatedAt,
    ...
    desktop_config_read: p300Summary.desktop_config_read === true || p300Boundary.desktop_config_read === true,
...
function buildFailClosedGateRows({ packageJson, platformOpsLedger, rows, boundary, sourceReady, secretsScanGateComplete, coverage }) {
  ...
  const gateInputs = [
    ["p426_secret_scan_attention_ready", "P426 secret scan attention source is ready.", sourceReady],
    ["p300_secrets_scan_gate_complete", "P300 Secrets Scan Gate is complete.", secretsScanGateComplete],
    ["platform_package_script_registered", "package.json registers the P427 trading secret scan fail-closed fixtures command.", typeof scripts[COMMAND_NAME] === "string" && scripts[COMMAND_NAME].length > 0],
    ["platform_validation_chain_registered", "Validation chain includes the P427 trading secret scan fail-closed fixtures command.", validateScript.includes(`npm run ${COMMAND_NAME} -- --check`)],
    ["p427_ledger_acceptance_declared", "P427 acceptance row is declared in the platform operations ledger.", ledgerText.includes("P427: `trading:secret-scan-fail-closed-fixtures`")],
    ...
    ["p300_boundary_no_secret_material_read", "P300 and P426 boundaries read no secret values, env files, or Desktop config content.", coverage.no_secret_material_read],
    ["no_secret_or_trading_mutation", "P427 performs no credential lookup, trading writes, artifact mutation, release, git, or protected action.", boundary.no_secret_or_trading_mutation && !boundary.artifact_write_performed && !boundary.protected_action_executed],
  ];
...
function gateRow(rowKey, description, passed) {
  return {
    schema_version: "trading-secret-scan-fail-closed-gate-row.v1",
    secret_scan_fail_closed_gate_row_id: `trading-secret-scan-fail-closed-fixtures.gate.${rowKey}`,
    phase_slot: PHASE_SLOT,
    row_key: rowKey,
    description,
...
function buildValidationItems({ secretScanAttention, secretsScanGate, sources, rows, gateRows, boundary, coverage }) {
  return [
    validationItem("source.secret_scan_attention", "p426_secret_scan_attention_ready", secretScanAttention.validation.valid && secretScanAttention.summary.trading_secret_scan_attention_fixtures_status === SOURCE_READY_STATUS, "P426 secret scan attention fixtures must be ready."),
    ...
    validationItem("source.package_json", "package_json_available", sources.packageJson.available, "package.json is readable for P427 secret scan fail-closed fixtures."),
    ...
    validationItem("secret_scan_fail_closed_gate_rows", "secret_scan_fail_closed_gates_ready", gateRows.length >= 10 && gateRows.every((row) => row.gate_status === "ready" && !row.protected_action_executed_by_gate), "P427 secret scan fail-closed gates are ready."),
    ...
    validationItem("boundary.no_secret_material_read", "no_secret_material_read", boundary.no_secret_material_read && !boundary.secret_values_read && !boundary.env_file_read && !boundary.desktop_config_content_inspected && !boundary.desktop_config_read, "P427 reads no secret values, env files, or Desktop config content."),
    validationItem("boundary.no_mutation", "no_secret_or_trading_mutation", boundary.no_secret_or_trading_mutation && !boundary.credential_lookup_allowed && !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.artifact_write_performed && !boundary.protected_action_executed, "P427 performs no credential lookup, trading mutation, artifact mutation, or protected action."),
  ];
}

function buildSummary({ secretScanAttention, secretsScanGate, rows, gateRows, boundary, coverage, validation }) {
  return {
    trading_secret_scan_fail_closed_fixtures_status: validation.valid ? READY_STATUS : "blocked",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_attention_status: secretScanAttention.summary.trading_secret_scan_attention_fixtures_status,
    source_secret_scan_attention_ready: boundary.source_secret_scan_attention_ready,
    source_secrets_scan_gate_status: secretsScanGate.summary.secrets_scan_gate_status,
...
function renderMarkdown(result) {
  const lines = [
    ...
    "",
    `Status: ${result.summary.trading_secret_scan_fail_closed_fixtures_status}`,
    `Phase: ${result.summary.phase_slot}`,
    `Source secret scan attention: ${result.summary.source_secret_scan_attention_status}`,
...
function parseArgs(argv) {
  const parsed = { outDir: DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_OUT_DIR };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--secret-scan-attention-schema") parsed.secretScanAttentionSchemaPath = argv[++index];
    else if (arg === "--secret-scan-gate-fixtures-schema") parsed.secretScanGateFixturesSchemaPath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/trading-secret-scan-fail-closed-fixtures.mjs [options]

Options:
  --out-dir <folder>                         Output directory. Default: ${DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_OUT_DIR}
  --run-at <iso>                             Deterministic generated_at timestamp.
  --package <path>                           package.json path.
  --platform-ops-ledger <path>               P341-P500 platform operations ledger path.
  --secret-scan-attention-schema <path>      P426 secret scan attention fixtures schema path.
  --secret-scan-gate-fixtures-schema <path>  P425 secret scan gate fixtures schema path.
...
function normalizeInputs(options = {}) {
  return {
    package_path: path.resolve(options.packagePath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.packagePath),
    platform_ops_ledger_path: path.resolve(options.platformOpsLedgerPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.platformOpsLedgerPath),
    secret_scan_gate_fixtures_schema_path: path.resolve(options.secretScanGateFixturesSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.secretScanGateFixturesSchemaPath),
    secret_scan_attention_schema_path: path.resolve(options.secretScanAttentionSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.secretScanAttentionSchemaPath),
    external_model_policy_audit_path: path.resolve(options.externalModelPolicyAuditPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.externalModelPolicyAuditPath),
    secrets_broker_contract_path: path.resolve(options.secretsBrokerContractPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.secretsBrokerContractPath),
    protected_file_gate_path: path.resolve(options.protectedFileGatePath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.protectedFileGatePath),
    dev_protected_scan_path: path.resolve(options.devProtectedScanPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.devProtectedScanPath),
    connector_freeze_path: path.resolve(options.connectorFreezePath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.connectorFreezePath),
    connector_contract_v2_path: path.resolve(options.connectorContractV2Path ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.connectorContractV2Path),
    expansion_quarantine_ledger_path: path.resolve(options.expansionQuarantineLedgerPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.expansionQuarantineLedgerPath),
    runtime_api_dashboard_path: path.resolve(options.runtimeApiDashboardPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.runtimeApiDashboardPath),
    capability_registry_api_path: path.resolve(options.capabilityRegistryApiPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.capabilityRegistryApiPath),
    roadmap_path: path.resolve(options.roadmapPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.roadmapPath),
    implementation_roadmap_path: path.resolve(options.implementationRoadmapPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.implementationRoadmapPath),
    review_dashboard_path: path.resolve(options.reviewDashboardPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.reviewDashboardPath),
    review_dashboard_source_path: path.resolve(options.reviewDashboardSourcePath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.reviewDashboardSourcePath),
    review_api_path: path.resolve(options.reviewApiPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.reviewApiPath),
    review_api_source_path: path.resolve(options.reviewApiSourcePath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.reviewApiSourcePath),
    review_api_doc_path: path.resolve(options.reviewApiDocPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.reviewApiDocPath),
    secrets_scan_gate_doc_path: path.resolve(options.secretsScanGateDocPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.secretsScanGateDocPath),
    schema_path: path.resolve(options.schemaPath ?? DEFAULT_TRADING_SECRET_SCAN_FAIL_CLOSED_FIXTURES_INPUTS.schemaPath),
  };
}
