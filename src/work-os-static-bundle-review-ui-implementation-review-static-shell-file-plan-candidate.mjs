...
import {
  ALL_FALSE_FLAGS as P43200_FALSE_FLAGS,
  buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoff,
} from "./work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-handoff.mjs";

export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_FILE_PLAN_CANDIDATE_OUT_DIR = "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-file-plan-candidate/latest";
export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_FILE_PLAN_CANDIDATE_INPUTS = {
  schemaPath: "schemas/work-os-static-bundle-review-ui-implementation-review-static-shell-file-plan-candidate.schema.json",
  packagePath: "package.json",
  roadmapDocPath: "docs/hermes-roadmap-p43201-p43600.md",
  architectureDocPath: "docs/architecture.md",
  sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoffPath: "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-handoff/latest/work-os-static-bundle-review-ui-implementation-review-static-shell-assembly-handoff.json",
};

const COMMAND_NAME = "platform:work-os-static-bundle-review-ui-implementation-review-static-shell-file-plan-candidate";
const SCHEMA_VERSION = "work-os-static-bundle-review-ui-implementation-review-static-shell-file-plan-candidate.v1";
const CAPABILITY_ID = "platform.work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate";
const PROGRAM_RANGE = "P43201-P43600";
const SOURCE_PROGRAM_RANGE = "P42801-P43200";
const READY_STATUS = "ready_for_work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate";
const BLOCK_PENDING_STATUS = "valid_block_work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate_pending";
const BLOCKED_STATUS = "blocked_work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate";
...
export const ALL_FALSE_FLAGS = [...new Set([...STATIC_SHELL_FILE_PLAN_CANDIDATE_FALSE_FLAGS, ...P43200_FALSE_FLAGS])];

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellFilePlanCandidate(options = {}) {
  const result = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellFilePlanCandidate(options);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Work OS Static Bundle Review UI Implementation Review Static Shell File Plan Candidate failed with ${result.validation.errors.length} error(s).`);
    error.validation = result.validation;
    throw error;
  }
  if (!options.check && options.write !== false) await writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellFilePlanCandidate(result, result.output_dir);
  return result;
}

export async function buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellFilePlanCandidate(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_FILE_PLAN_CANDIDATE_OUT_DIR);
  const inputs = normalizeInputs(options);
  const schema = await readJsonSource(inputs.schema_path);
  const packageJson = await readJsonSource(inputs.package_path);
  const roadmapDoc = await readTextSource(inputs.roadmap_doc_path);
  const architectureDoc = await readTextSource(inputs.architecture_doc_path);
  const source = Object.prototype.hasOwnProperty.call(options, "workOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoff")
    ? normalizeInlineJsonSource("inline.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff", options.workOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoff)
    : await readJsonOrBuildP43200(inputs.source_work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_path, generatedAt);
  ...
  const result = {
    ...
    inputs,
    source_refs: {
      work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_path: source.path,
      source_commit_ref: commitRef || null,
    },
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_summary: source.data?.summary ?? null,
    work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate_contract: buildContract(generatedAt),
    work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate_phase_rows: phaseRows,
    p43200_source_binding_rows: sourceRows,
    ...
    p43600_clean_checkpoint_rows: checkpointRows,
    work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate_boundary: boundary,
    work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate_validation_items: validationItems,
    validation: preliminaryValidation,
    summary: buildSummary({ boundary, validation: preliminaryValidation }),
  };

  const schemaErrors = schema.available
    ? validateAgainstSchema(result, schema.data, {}, "work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate")
    : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  const schemaValidationItems = schemaErrors.map((error, index) => validationItem(`schema.${index}`, "schema_validation", false, error.message));
  result.work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate_validation_items = [...validationItems, ...schemaValidationItems];
  result.validation = summarizeValidation(result.work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate_validation_items);
  result.summary = buildSummary({ boundary, validation: result.validation });
  return { ...result, markdown: renderMarkdown(result), html: renderHtml(result) };
}

export async function writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellFilePlanCandidate(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-file-plan-candidate.json"), serializableResult(result));
  await writeJson(path.join(outDir, "p43200-source-binding-rows.json"), collectionEnvelope("p43200-source-binding-rows.v1", "p43200_source_binding_rows", result.p43200_source_binding_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "p43600-clean-checkpoint-rows.json"), collectionEnvelope("p43600-clean-checkpoint-rows.v1", "p43600_clean_checkpoint_rows", result.p43600_clean_checkpoint_rows, result.generated_at));
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-file-plan-candidate-boundary.json"), result.work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate_boundary);
  await writeFile(path.join(outDir, "summary.md"), result.markdown, "utf8");
  await writeFile(path.join(outDir, "index.html"), result.html, "utf8");
}

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellFilePlanCandidateCli(argv = process.argv.slice(2)) {
  const args = parseArgs(argv);
  if (args.help) {
    printHelp();
    return null;
  }
  const result = await runWorkOsStaticBundleReviewUiImplementationReviewStaticShellFilePlanCandidate(args);
  console.log(`Work OS Static Bundle Review UI Implementation Review Static Shell File Plan Candidate ${args.check ? "validated" : "written"} at ${result.output_dir}`);
  console.log(`Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate_status}`);
  console.log(`Program: ${result.program_range}`);
  console.log(`P43200 ready for P43201 handoff: ${result.summary.source_p43200_ready_for_p43201_handoff}`);
  ...
  console.log(`Ready for P43601 handoff: ${result.summary.ready_for_p43601_handoff}`);
...
function buildSourceState(source) {
  ...
  const boundary = source.data?.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_boundary ?? {};
  return {
    available: source.available === true,
    programRangeOk: source.data?.program_range === SOURCE_PROGRAM_RANGE,
    validationValid: source.data?.validation?.valid === true,
    sourceReady: summary.ready_for_p43201_handoff === true,
    status: summary.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_status ?? "missing",
    p43200ContractReady: boundary.p43200_contract_ready === true,
    ...
    boundaryClosed: P43200_FALSE_FLAGS.every((flag) => boundary[flag] === false),
  };
}
...
function buildPhaseRows(roadmapText, generatedAt) {
  return PHASE_SPECS.map(([range, label, outputRef]) => row({
    ...
    evidence_ref: "docs/hermes-roadmap-p43201-p43600.md",
...
function buildSourceRows({ source, sourceState, commitRef, generatedAt }) {
  return [
    ["artifact_available", "P43200 implementation review static shell assembly handoff source is available", sourceState.available],
    ["program_range", "P43200 source program range is P42801-P43200", sourceState.programRangeOk],
    ["validation_valid", "P43200 source validation is valid", sourceState.validationValid],
    ["p43201_handoff_open", "P43200 source opened P43201 handoff", sourceState.sourceReady],
    ["p43200_contract_ready", "P43200 source contract is ready", sourceState.p43200ContractReady],
    ["handoff_packet_visible", "P43200 assembly handoff packet rows are visible", sourceState.handoffPacketVisible],
    ["template_target_visible", "P43200 template target map rows are visible", sourceState.templateTargetVisible],
    ["state_copy_binding_visible", "P43200 state/copy binding matrix rows are visible", sourceState.stateCopyBindingVisible],
    ["asset_hook_visible", "P43200 static asset hook guard rows are visible", sourceState.assetHookVisible],
    ["no_apply_no_build_closed", "P43200 no-apply/no-build boundary is closed", sourceState.noApplyNoBuildClosed],
    ["commit_ref_present", "Current commit ref is present for static shell file plan candidate", Boolean(commitRef)],
    ["source_blocker_visible", "P43200 source blocker is visible when file plan candidate is closed", sourceState.available],
  ].map(([id, label, observed]) => row({
    row_id: `source_binding.${id}`,
    category: "p43200_source_binding",
    label,
    observed,
...
function buildNoWriteNoBuildBoundaryRows(generatedAt) {
  return ALL_FALSE_FLAGS.map((flag) => row({
    ...
    observed: true,
    evidence_ref: "work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate_boundary",
    boundary_flag: flag,
    allowed_now: false,
    generated_at: generatedAt,
  }));
...
function buildCheckpointRows(context) {
  ...
  return [
    ["source_ready", "P43200 source is ready for P43201", context.sourceState.sourceReady],
    ...
    ["p43601_handoff_gate", "P43601 handoff opens only when static shell file plan candidate conditions pass", handoffReady],
    ["p43601_handoff_blocker_visible", "P43601 handoff blocker is visible when the gate is closed", true],
  ].map(([id, label, observed]) => row({
    row_id: `p43600_checkpoint.${id}`,
    category: "p43600_clean_checkpoint",
    label,
    observed,
    evidence_ref: "p43600_clean_checkpoint_rows",
    generated_at: context.generatedAt,
  }));
}

function buildBoundary(context) {
  const p43600ContractReady = allPass(context.phaseRows)
    ...
    && allPass(context.assetRows)
    && allPass(context.boundaryRows)
    && visibleOrPassed(context.checkpointRows, "p43600_checkpoint.p43601_handoff_blocker_visible");
  const handoffReady = context.sourceState.sourceReady
    && context.sourceState.validationValid
    && context.sourceState.boundaryClosed
    && allPass(context.filePlanRows)
    ...
    && allPass(context.boundaryRows);
  return {
    p43600_contract_ready: p43600ContractReady,
    ready_for_p43601_handoff: handoffReady,
    source_p43200_ready_for_p43201_handoff: context.sourceState.sourceReady,
...
function buildValidationItems(context) {
  return [
    ...
    validationItem("roadmap.phase_coverage", "roadmap", allPass(context.phaseRows), "P43201-P43600 phase rows are incomplete"),
    validationItem("architecture.reference", "architecture", context.architectureDoc.text.includes("P43201-P43600 Work OS Static Bundle Review UI Implementation Review Static Shell File Plan Candidate"), "Architecture doc missing P43201-P43600 reference"),
    validationItem("source.visible", "source", visibleOrPassed(context.sourceRows, "source_binding.source_blocker_visible"), "P43200 source state is not visible"),
    ...
    validationItem("checkpoint.visible", "checkpoint", visibleOrPassed(context.checkpointRows, "p43600_checkpoint.p43601_handoff_blocker_visible"), "P43600 checkpoint blocker is not visible"),
  ];
}

function buildContract(generatedAt) {
  return {
    contract_id: "work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate.contract.v1",
    generated_at: generatedAt,
    source_p43200_required_or_rebuilt: true,
    ...
    p43601_handoff_is_not_file_create_write_apply_build_route_dom_browser_action_approval_closeout_or_enterprise_trust: true,
    protected_authority: "closed",
  };
}

function buildSummary({ boundary, validation }) {
  const status = validation.valid && boundary.ready_for_p43601_handoff
    ? READY_STATUS
    : validation.valid && boundary.p43600_contract_ready
      ? BLOCK_PENDING_STATUS
      : BLOCKED_STATUS;
  return {
    work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate_status: status,
    source_p43200_ready_for_p43201_handoff: boundary.source_p43200_ready_for_p43201_handoff,
    ...
    ready_for_p43601_handoff: validation.valid && boundary.ready_for_p43601_handoff,
...
function renderMarkdown(result) {
  return [
    "# Work OS Static Bundle Review UI Implementation Review Static Shell File Plan Candidate",
    "",
    `Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_file_plan_candidate_status}`,
    `Program: ${result.program_range}`,
    `P43200 ready for P43201 handoff: ${result.summary.source_p43200_ready_for_p43201_handoff}`,
    ...
    `Ready for P43601 handoff: ${result.summary.ready_for_p43601_handoff}`,
...
<head>
  ...
  <title>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell File Plan Candidate</title>
  <style>
...
<body>
  <main>
    <h1>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell File Plan Candidate</h1>
...
async function readJsonOrBuildP43200(filePath, generatedAt) {
  const source = await readJsonSource(filePath);
  if (source.available) return source;
  const built = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoff({ runAt: generatedAt, write: false });
  return normalizeInlineJsonSource("built.work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff", built);
}
...
function normalizeInputs(options) {
  const defaults = DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_FILE_PLAN_CANDIDATE_INPUTS;
  ...
  return {
    ...
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_assembly_handoff_path: path.resolve(repoRoot, options.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoffPath ?? defaults.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoffPath),
  };
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--check") {
      ...
      args.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellAssemblyHandoffPath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/work-os-static-bundle-review-ui-implementation-review-static-shell-file-plan-candidate.mjs [--check] [--out-dir DIR] [--source FILE] [--commit-ref SHA]

Validates or writes the Hermes P43201-P43600 Work OS Static Bundle Review UI Implementation Review Static Shell File Plan Candidate.
`);
}
