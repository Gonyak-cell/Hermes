...
import {
  DEFAULT_TRADING_SECRET_SCAN_GATE_FIXTURES_INPUTS,
  buildTradingSecretScanGateFixtures,
} from "./trading-secret-scan-gate-fixtures.mjs";

export const DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_OUT_DIR = "artifacts/trading-secret-scan-attention-fixtures/latest";
export const DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_INPUTS = {
  ...DEFAULT_TRADING_SECRET_SCAN_GATE_FIXTURES_INPUTS,
  secretScanGateFixturesSchemaPath: DEFAULT_TRADING_SECRET_SCAN_GATE_FIXTURES_INPUTS.schemaPath,
  reviewDashboardPath: "src/review-dashboard.mjs",
  reviewApiDocPath: "docs/review-api.md",
  schemaPath: "schemas/trading/trading-secret-scan-attention-fixtures.schema.json",
};

const SCHEMA_VERSION = "trading-secret-scan-attention-fixtures.v1";
const CAPABILITY_ID = "trading.secret_scan_attention_fixtures";
const PHASE_SLOT = "P426";
const PREVIOUS_PHASE_SLOT = "P425";
const NEXT_PHASE_SLOT = "P427";
const READY_STATUS = "ready_for_trading_secret_scan_attention";
const SOURCE_READY_STATUS = "ready_for_trading_secret_scan_gate";
const COMMAND_NAME = "trading:secret-scan-attention-fixtures";
const REQUIRED_ROW_KEYS = [
  "p425_secret_scan_gate_ready",
...
export async function runTradingSecretScanAttentionFixtures(options = {}) {
  const result = await buildTradingSecretScanAttentionFixtures(options);
  if (options.write !== false) await writeTradingSecretScanAttentionFixtures(result, result.output_dir);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Trading secret scan attention fixtures failed with ${result.validation.errors.length} error(s).`);
...
export async function buildTradingSecretScanAttentionFixtures(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_OUT_DIR);
  const inputs = normalizeInputs(options);
  const secretScanGate = await buildTradingSecretScanGateFixtures({
    ...options,
    runAt: generatedAt,
    packagePath: inputs.package_path,
    platformOpsLedgerPath: inputs.platform_ops_ledger_path,
    schemaPath: inputs.secret_scan_gate_fixtures_schema_path,
    write: false,
  });
  ...
  const sourceReady = secretScanGate.validation.valid && secretScanGate.summary.trading_secret_scan_gate_fixtures_status === SOURCE_READY_STATUS;
  ...
  const result = {
    ...
    trading_secret_scan_attention_fixtures_id: `trading-secret-scan-attention-fixtures.${dateStamp(generatedAt)}`,
  ...
  const schemaErrors = schema.available ? validateAgainstSchema(result, schema.data, {}, "trading_secret_scan_attention_fixtures") : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  ...
  result.summary.trading_secret_scan_attention_fixtures_id = result.trading_secret_scan_attention_fixtures_id;
  return { ...result, markdown: renderMarkdown(result) };
}

export async function writeTradingSecretScanAttentionFixtures(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "trading-secret-scan-attention-fixtures.json"), serializableResult(result));
  await writeJson(path.join(outDir, "secret-scan-attention-rows.json"), collectionEnvelope("trading-secret-scan-attention-rows.v1", "secret_scan_attention_rows", result.secret_scan_attention_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-scan-attention-gate-rows.json"), collectionEnvelope("trading-secret-scan-attention-gate-rows.v1", "secret_scan_attention_gate_rows", result.secret_scan_attention_gate_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-scan-attention-boundary.json"), result.secret_scan_attention_boundary);
  await writeJson(path.join(outDir, "validation-report.json"), {
    schema_version: "trading-secret-scan-attention-fixtures-validation-report.v1",
...
export async function runTradingSecretScanAttentionFixturesCli(argv = process.argv.slice(2)) {
  ...
  try {
    const result = await runTradingSecretScanAttentionFixtures(args);
    console.log(`Trading secret scan attention fixtures ${args.check ? "validated" : "written"} at ${result.output_dir}`);
    console.log(`Status: ${result.summary.trading_secret_scan_attention_fixtures_status}`);
...
function buildSecretScanAttentionCoverage({ secretScanGate, sources }) {
  ...
  return {
    p425_ready: secretScanGate.validation.valid && secretScanGate.summary.trading_secret_scan_gate_fixtures_status === SOURCE_READY_STATUS,
    ...
    no_secret_exposure: !secretScanGate.summary.secret_values_read && !secretScanGate.summary.env_file_read && !secretScanGate.summary.desktop_config_content_inspected && !secretScanGate.summary.raw_secret_material_exposed && !secretScanGate.summary.provider_key_material_present,
    no_secret_or_trading_mutation: secretScanGate.summary.no_secret_or_trading_mutation && !secretScanGate.summary.credential_lookup_allowed && !secretScanGate.summary.broker_write_allowed && !secretScanGate.summary.exchange_write_allowed && !secretScanGate.summary.artifact_write_performed && !secretScanGate.summary.protected_action_executed,
  };
}

function buildSecretScanAttentionAnchor(secretScanGate, coverage) {
  return {
    schema_version: "trading-secret-scan-attention-fixtures-anchor.v1",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_gate_fixtures_id: secretScanGate.trading_secret_scan_gate_fixtures_id,
    source_secret_scan_gate_status: secretScanGate.summary.trading_secret_scan_gate_fixtures_status,
    required_row_count: REQUIRED_ROW_KEYS.length,
    required_row_keys: REQUIRED_ROW_KEYS,
    coverage_hash: hashValue(coverage),
    source_hash: hashValue({
      id: secretScanGate.trading_secret_scan_gate_fixtures_id,
      status: secretScanGate.summary.trading_secret_scan_gate_fixtures_status,
...
function buildSecretScanAttentionRows({ sourceReady, secretScanGate, coverage }) {
  const rowInputs = [
    attentionRowInput("p425_secret_scan_gate_ready", "trading_secret_scan_gate", "summary", [
      condition("source.trading_secret_scan_gate_fixtures_status", secretScanGate.summary.trading_secret_scan_gate_fixtures_status, SOURCE_READY_STATUS),
...
function buildSecretScanAttentionRow(input, sourceReady, index) {
  ...
  const row = {
    schema_version: "trading-secret-scan-attention-row.v1",
    secret_scan_attention_row_id: `trading-secret-scan-attention.row.${input.rowKey}`,
    phase_slot: PHASE_SLOT,
    ...
    source_secret_scan_gate_ready: sourceReady,
...
function buildSecretScanAttentionBoundary({ generatedAt, writeRequested, secretScanGate, rows, coverage }) {
  const sourceReady = secretScanGate.validation.valid && secretScanGate.summary.trading_secret_scan_gate_fixtures_status === SOURCE_READY_STATUS;
  return {
    schema_version: "trading-secret-scan-attention-fixtures-boundary.v1",
    generated_at: generatedAt,
...
function buildSecretScanAttentionGateRows({ secretScanGate, packageJson, platformOpsLedger, rows, boundary }) {
  ...
  const gateInputs = [
    ["p425_secret_scan_gate_ready", "P425 secret scan gate source is ready.", secretScanGate.validation.valid && secretScanGate.summary.trading_secret_scan_gate_fixtures_status === SOURCE_READY_STATUS],
    ["platform_package_script_registered", "package.json registers the P426 trading secret scan attention fixtures command.", typeof scripts[COMMAND_NAME] === "string" && scripts[COMMAND_NAME].length > 0],
    ["platform_validation_chain_registered", "Validation chain includes the P426 trading secret scan attention fixtures command.", validateScript.includes(`npm run ${COMMAND_NAME} -- --check`)],
    ["p426_ledger_acceptance_declared", "P426 acceptance row is declared in the platform operations ledger.", ledgerText.includes("P426: `trading:secret-scan-attention-fixtures`")],
    ...
    ["no_secret_or_trading_mutation", "P426 performs no secret reads, credential lookup, trading writes, artifact mutation, release, git, or protected action.", boundary.no_secret_exposure && boundary.no_secret_or_trading_mutation && !boundary.secret_values_read && !boundary.artifact_write_performed && !boundary.protected_action_executed],
  ];
...
function gateRow(rowKey, description, passed) {
  return {
    schema_version: "trading-secret-scan-attention-gate-row.v1",
    secret_scan_attention_gate_row_id: `trading-secret-scan-attention-fixtures.gate.${rowKey}`,
    phase_slot: PHASE_SLOT,
    row_key: rowKey,
    description,
...
function buildValidationItems({ secretScanGate, sources, rows, gateRows, boundary }) {
  return [
    validationItem("source.secret_scan_gate", "p425_secret_scan_gate_ready", secretScanGate.validation.valid && secretScanGate.summary.trading_secret_scan_gate_fixtures_status === SOURCE_READY_STATUS, "P425 secret scan gate fixtures must be ready."),
    validationItem("source.package_json", "package_json_available", sources.packageJson.available, "package.json is readable for P426 secret scan attention fixtures."),
    ...
    validationItem("secret_scan_attention_gate_rows", "secret_scan_attention_gates_ready", gateRows.length >= 10 && gateRows.every((row) => row.gate_status === "ready" && !row.protected_action_executed_by_gate), "P426 secret scan attention gates are ready."),
    ...
    validationItem("boundary.no_mutation", "no_secret_or_trading_mutation", boundary.no_secret_exposure && boundary.no_secret_or_trading_mutation && !boundary.secret_values_read && !boundary.env_file_read && !boundary.desktop_config_content_inspected && !boundary.credential_lookup_allowed && !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.artifact_write_performed && !boundary.protected_action_executed, "P426 performs no secret reads, trading mutation, artifact mutation, or protected action."),
  ];
}

function buildSummary({ secretScanGate, rows, gateRows, boundary, validation }) {
  return {
    trading_secret_scan_attention_fixtures_status: validation.valid ? READY_STATUS : "blocked",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_gate_status: secretScanGate.summary.trading_secret_scan_gate_fixtures_status,
    source_secret_scan_gate_ready: boundary.source_secret_scan_gate_ready,
    required_row_count: REQUIRED_ROW_KEYS.length,
    secret_scan_attention_row_count: rows.length,
...
function renderMarkdown(result) {
  const lines = [
    ...
    "",
    `Status: ${result.summary.trading_secret_scan_attention_fixtures_status}`,
    `Phase: ${result.summary.phase_slot}`,
    `Source secret scan gate: ${result.summary.source_secret_scan_gate_status}`,
...
function parseArgs(argv) {
  const parsed = { outDir: DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_OUT_DIR };
...
function printHelp() {
  console.log(`Usage: node scripts/trading-secret-scan-attention-fixtures.mjs [options]

Options:
  --out-dir <folder>                         Output directory. Default: ${DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_OUT_DIR}
...
function normalizeInputs(options = {}) {
  return {
    package_path: path.resolve(options.packagePath ?? DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_INPUTS.packagePath),
    platform_ops_ledger_path: path.resolve(options.platformOpsLedgerPath ?? DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_INPUTS.platformOpsLedgerPath),
    secret_scan_gate_fixtures_schema_path: path.resolve(options.secretScanGateFixturesSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_INPUTS.secretScanGateFixturesSchemaPath),
    review_dashboard_path: path.resolve(options.reviewDashboardPath ?? DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_INPUTS.reviewDashboardPath),
    review_api_path: path.resolve(options.reviewApiPath ?? DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_INPUTS.reviewApiPath),
    review_api_doc_path: path.resolve(options.reviewApiDocPath ?? DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_INPUTS.reviewApiDocPath),
    secrets_scan_gate_doc_path: path.resolve(options.secretsScanGateDocPath ?? DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_INPUTS.secretsScanGateDocPath),
    schema_path: path.resolve(options.schemaPath ?? DEFAULT_TRADING_SECRET_SCAN_ATTENTION_FIXTURES_INPUTS.schemaPath),
  };
}
