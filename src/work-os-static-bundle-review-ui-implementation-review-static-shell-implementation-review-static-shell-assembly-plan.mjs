...
import {
  ALL_FALSE_FLAGS as P47200_FALSE_FLAGS,
  buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellCandidate,
} from "./work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-static-shell-candidate.mjs";

export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_REVIEW_STATIC_SHELL_ASSEMBLY_PLAN_OUT_DIR = "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-static-shell-assembly-plan/latest";
export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_REVIEW_STATIC_SHELL_ASSEMBLY_PLAN_INPUTS = {
  schemaPath: "schemas/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-static-shell-assembly-plan.schema.json",
  packagePath: "package.json",
  roadmapDocPath: "docs/hermes-roadmap-p47201-p47600.md",
  architectureDocPath: "docs/architecture.md",
  sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellCandidatePath: "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-static-shell-candidate/latest/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-static-shell-candidate.json",
};

const COMMAND_NAME = "platform:work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-static-shell-assembly-plan";
const SCHEMA_VERSION = "work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-static-shell-assembly-plan.v1";
const CAPABILITY_ID = "platform.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan";
const PROGRAM_RANGE = "P47201-P47600";
const SOURCE_PROGRAM_RANGE = "P46801-P47200";
const READY_STATUS = "ready_for_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan";
const BLOCK_PENDING_STATUS = "valid_block_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan_pending";
const BLOCKED_STATUS = "blocked_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan";

const PHASE_SPECS = [
  ["P47201-P47240", "P47200 Source Binding", "p47200_source_binding_rows"],
  ["P47241-P47320", "Static Shell Assembly Plan Contract", "static_shell_assembly_plan_contract_rows"],
  ["P47321-P47400", "Template Composition Manifest", "template_composition_manifest_rows"],
  ["P47401-P47480", "Read-Only State Slot Map", "read_only_state_slot_map_rows"],
  ["P47481-P47540", "Accessibility And Blocked Copy Guard", "accessibility_blocked_copy_guard_rows"],
  ["P47541-P47580", "No-Build/No-Render Boundary", "no_build_no_render_boundary_rows"],
  ["P47581-P47600", "P47600 Clean Checkpoint", "p47600_clean_checkpoint_rows"],
];
...
export const ALL_FALSE_FLAGS = [...new Set([...STATIC_SHELL_ASSEMBLY_PLAN_FALSE_FLAGS, ...P47200_FALSE_FLAGS])];

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellAssemblyPlan(options = {}) {
  const result = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellAssemblyPlan(options);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Static Shell Assembly Plan failed with ${result.validation.errors.length} error(s).`);
    error.validation = result.validation;
    throw error;
  }
  if (!options.check && options.write !== false) await writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellAssemblyPlan(result, result.output_dir);
  return result;
}

export async function buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellAssemblyPlan(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_REVIEW_STATIC_SHELL_ASSEMBLY_PLAN_OUT_DIR);
  const inputs = normalizeInputs(options);
  const schema = await readJsonSource(inputs.schema_path);
  const packageJson = await readJsonSource(inputs.package_path);
  const roadmapDoc = await readTextSource(inputs.roadmap_doc_path);
  const architectureDoc = await readTextSource(inputs.architecture_doc_path);
  const source = Object.prototype.hasOwnProperty.call(options, "workOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellCandidate")
    ? normalizeInlineJsonSource("inline.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_candidate", options.workOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellCandidate)
    : await readJsonOrBuildP47200(inputs.source_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_candidate_path, generatedAt);
  ...
  const result = {
    ...
    inputs,
    source_refs: {
      work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_candidate_path: source.path,
      source_commit_ref: commitRef || null,
    },
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_candidate_summary: source.data?.summary ?? null,
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan_contract: buildContract(generatedAt),
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan_phase_rows: phaseRows,
    p47200_source_binding_rows: sourceRows,
    ...
    p47600_clean_checkpoint_rows: checkpointRows,
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan_boundary: boundary,
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan_validation_items: validationItems,
    validation: preliminaryValidation,
    summary: buildSummary({ boundary, validation: preliminaryValidation }),
  };

  const schemaErrors = schema.available
    ? validateAgainstSchema(result, schema.data, {}, "work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan")
    : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  const schemaValidationItems = schemaErrors.map((error, index) => validationItem(`schema.${index}`, "schema_validation", false, error.message));
  result.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan_validation_items = [...validationItems, ...schemaValidationItems];
  result.validation = summarizeValidation(result.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan_validation_items);
  result.summary = buildSummary({ boundary, validation: result.validation });
  return { ...result, markdown: renderMarkdown(result), html: renderHtml(result) };
}

export async function writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellAssemblyPlan(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-static-shell-assembly-plan.json"), serializableResult(result));
  await writeJson(path.join(outDir, "p47200-source-binding-rows.json"), collectionEnvelope("p47200-source-binding-rows.v1", "p47200_source_binding_rows", result.p47200_source_binding_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "p47600-clean-checkpoint-rows.json"), collectionEnvelope("p47600-clean-checkpoint-rows.v1", "p47600_clean_checkpoint_rows", result.p47600_clean_checkpoint_rows, result.generated_at));
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-static-shell-assembly-plan-boundary.json"), result.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan_boundary);
  await writeFile(path.join(outDir, "summary.md"), result.markdown, "utf8");
  await writeFile(path.join(outDir, "index.html"), result.html, "utf8");
}

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellAssemblyPlanCli(argv = process.argv.slice(2)) {
  const args = parseArgs(argv);
  if (args.help) {
    printHelp();
    return null;
  }
  const result = await runWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellAssemblyPlan(args);
  console.log(`Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Static Shell Assembly Plan ${args.check ? "validated" : "written"} at ${result.output_dir}`);
  console.log(`Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan_status}`);
  console.log(`Program: ${result.program_range}`);
  console.log(`P47200 ready for P47201 handoff: ${result.summary.source_p47200_ready_for_p47201_handoff}`);
  ...
  console.log(`Ready for P47601 handoff: ${result.summary.ready_for_p47601_handoff}`);
...
function buildSourceState(source) {
  ...
  const boundary = source.data?.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_candidate_boundary ?? {};
  return {
    available: source.available === true,
    programRangeOk: source.data?.program_range === SOURCE_PROGRAM_RANGE,
    validationValid: source.data?.validation?.valid === true,
    sourceReady: summary.ready_for_p47201_handoff === true,
    status: summary.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_candidate_status ?? "missing",
    p47200ContractReady: boundary.p47200_contract_ready === true,
    ...
    boundaryClosed: P47200_FALSE_FLAGS.every((flag) => boundary[flag] === false),
  };
}
...
function buildPhaseRows(roadmapText, generatedAt) {
  return PHASE_SPECS.map(([range, label, outputRef]) => row({
    ...
    evidence_ref: "docs/hermes-roadmap-p47201-p47600.md",
...
function buildSourceRows({ source, sourceState, commitRef, generatedAt }) {
  return [
    ["artifact_available", "P47200 implementation review static shell candidate source is available", sourceState.available],
    ["program_range", "P47200 source program range is P46801-P47200", sourceState.programRangeOk],
    ["validation_valid", "P47200 source validation is valid", sourceState.validationValid],
    ["p47201_handoff_open", "P47200 source opened P47201 handoff", sourceState.sourceReady],
    ["p47200_contract_ready", "P47200 source contract is ready", sourceState.p47200ContractReady],
    ["candidate_visible", "P47200 static shell candidate rows are visible", sourceState.candidateVisible],
    ["template_manifest_visible", "P47200 section template manifest rows are visible", sourceState.templateManifestVisible],
    ["hydration_stub_visible", "P47200 fixture hydration stub rows are visible", sourceState.hydrationStubVisible],
    ["blocked_copy_visible", "P47200 blocked control copy rows are visible", sourceState.blockedCopyVisible],
    ["no_serve_no_dom_closed", "P47200 no-serve/no-DOM boundary is closed", sourceState.noServeNoDomClosed],
    ["commit_ref_present", "Current commit ref is present for static shell assembly plan", Boolean(commitRef)],
    ["source_blocker_visible", "P47200 source blocker is visible when assembly plan is closed", sourceState.available],
  ].map(([id, label, observed]) => row({
    row_id: `source_binding.${id}`,
    category: "p47200_source_binding",
    label,
    observed,
...
function buildNoBuildNoRenderBoundaryRows(generatedAt) {
  return ALL_FALSE_FLAGS.map((flag) => row({
    ...
    observed: true,
    evidence_ref: "work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan_boundary",
    boundary_flag: flag,
    allowed_now: false,
    generated_at: generatedAt,
  }));
...
function buildCheckpointRows(context) {
  ...
  return [
    ["source_ready", "P47200 source is ready for P47201", context.sourceState.sourceReady],
    ...
    ["p47601_handoff_gate", "P47601 handoff opens only when static shell assembly plan conditions pass", handoffReady],
    ["p47601_handoff_blocker_visible", "P47601 handoff blocker is visible when the gate is closed", true],
  ].map(([id, label, observed]) => row({
    row_id: `p47600_checkpoint.${id}`,
    category: "p47600_clean_checkpoint",
    label,
    observed,
    evidence_ref: "p47600_clean_checkpoint_rows",
    generated_at: context.generatedAt,
  }));
}

function buildBoundary(context) {
  const p47600ContractReady = allPass(context.phaseRows)
    ...
    && allPass(context.guardRows)
    && allPass(context.boundaryRows)
    && visibleOrPassed(context.checkpointRows, "p47600_checkpoint.p47601_handoff_blocker_visible");
  const handoffReady = context.sourceState.sourceReady
    && context.sourceState.validationValid
    && context.sourceState.boundaryClosed
    && allPass(context.assemblyRows)
    ...
    && allPass(context.boundaryRows);
  return {
    p47600_contract_ready: p47600ContractReady,
    ready_for_p47601_handoff: handoffReady,
    source_p47200_ready_for_p47201_handoff: context.sourceState.sourceReady,
...
function buildValidationItems(context) {
  return [
    ...
    validationItem("roadmap.phase_coverage", "roadmap", allPass(context.phaseRows), "P47201-P47600 phase rows are incomplete"),
    validationItem("architecture.reference", "architecture", context.architectureDoc.text.includes("P47201-P47600 Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Static Shell Assembly Plan"), "Architecture doc missing P47201-P47600 reference"),
    validationItem("source.visible", "source", visibleOrPassed(context.sourceRows, "source_binding.source_blocker_visible"), "P47200 source state is not visible"),
    ...
    validationItem("checkpoint.visible", "checkpoint", visibleOrPassed(context.checkpointRows, "p47600_checkpoint.p47601_handoff_blocker_visible"), "P47600 checkpoint blocker is not visible"),
  ];
}

function buildContract(generatedAt) {
  return {
    contract_id: "work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan.contract.v1",
    generated_at: generatedAt,
    source_p47200_required_or_rebuilt: true,
    ...
    p47601_handoff_is_not_build_route_dom_browser_action_write_approval_closeout_or_enterprise_trust: true,
    protected_authority: "closed",
  };
}

function buildSummary({ boundary, validation }) {
  const status = validation.valid && boundary.ready_for_p47601_handoff
    ? READY_STATUS
    : validation.valid && boundary.p47600_contract_ready
      ? BLOCK_PENDING_STATUS
      : BLOCKED_STATUS;
  return {
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan_status: status,
    source_p47200_ready_for_p47201_handoff: boundary.source_p47200_ready_for_p47201_handoff,
    ...
    ready_for_p47601_handoff: validation.valid && boundary.ready_for_p47601_handoff,
...
function renderMarkdown(result) {
  return [
    "# Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Static Shell Assembly Plan",
    "",
    `Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_assembly_plan_status}`,
    `Program: ${result.program_range}`,
    `P47200 ready for P47201 handoff: ${result.summary.source_p47200_ready_for_p47201_handoff}`,
    ...
    `Ready for P47601 handoff: ${result.summary.ready_for_p47601_handoff}`,
...
<head>
  ...
  <title>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Static Shell Assembly Plan</title>
  <style>
...
<body>
  <main>
    <h1>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Static Shell Assembly Plan</h1>
...
async function readJsonOrBuildP47200(filePath, generatedAt) {
  const source = await readJsonSource(filePath);
  if (source.available) return source;
  const built = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellCandidate({ runAt: generatedAt, write: false });
  return normalizeInlineJsonSource("built.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_candidate", built);
}
...
function normalizeInputs(options) {
  const defaults = DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_REVIEW_STATIC_SHELL_ASSEMBLY_PLAN_INPUTS;
  ...
  return {
    ...
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_static_shell_candidate_path: path.resolve(repoRoot, options.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellCandidatePath ?? defaults.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellCandidatePath),
  };
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--check") {
      ...
      args.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewStaticShellCandidatePath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-static-shell-assembly-plan.mjs [--check] [--out-dir DIR] [--source FILE] [--commit-ref SHA]

Validates or writes the Hermes P47201-P47600 Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Static Shell Assembly Plan.
`);
}
