...
test("Factory G1a Source Literal Preflight recognizes the source-bound G1a opening commit", async () => {
  ...
  assert.equal(result.summary.factory_g1a_source_literal_preflight_status, "source_literal_opening_commit_already_applied");
  ...
  assert.equal(result.summary.preflight_pass_count, 12);
...
test("Factory G1a Source Literal Preflight blocks wrong gate and existing source literal", async () => {
  ...
  try {
    ...
    assert.ok(sourceResult.validation.errors.some((error) => error.item_id === "source.g1a_literal_state"));
  } finally {
...
test("Factory G1a Source Literal Preflight writes artifacts and check mode does not overwrite", async () => {
  ...
  try {
    const result = await withPreApplySource((gateOpeningSourcePath) => runFactoryG1aSourceLiteralPreflight({
      outDir,
      runAt: RUN_AT,
      check: false,
      ownerReceipt: buildSignedOwnerReceipt(),
...
test("Review API exposes G1a source literal preflight as read-only applied data", async () => {
  ...
  assert.equal(body.summary.factory_g1a_source_literal_preflight_status, "source_literal_opening_commit_already_applied");
  assert.equal(body.summary.owner_gate_opening_receipt_signed_now, false);
  assert.equal(body.summary.source_literal_opening_commit_applied_now, true);
  assert.equal(body.summary.first_use_audit_present, true);
