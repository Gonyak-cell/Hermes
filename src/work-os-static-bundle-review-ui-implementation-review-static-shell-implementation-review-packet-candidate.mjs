...
} from "./work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-handoff-package.mjs";

export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_REVIEW_PACKET_OUT_DIR = "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-packet-candidate/latest";
export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_REVIEW_PACKET_INPUTS = {
  schemaPath: "schemas/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-packet-candidate.schema.json",
  packagePath: "package.json",
  roadmapDocPath: "docs/hermes-roadmap-p44401-p44800.md",
  architectureDocPath: "docs/architecture.md",
  sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackagePath: "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-handoff-package/latest/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-handoff-package.json",
};

const COMMAND_NAME = "platform:work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-packet-candidate";
const SCHEMA_VERSION = "work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-packet-candidate.v1";
const CAPABILITY_ID = "platform.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_packet_candidate";
const PROGRAM_RANGE = "P44401-P44800";
const SOURCE_PROGRAM_RANGE = "P44001-P44400";
const READY_STATUS = "ready_for_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_packet_candidate";
const BLOCK_PENDING_STATUS = "valid_block_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_packet_candidate_pending";
const BLOCKED_STATUS = "blocked_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_packet_candidate";
...
export const ALL_FALSE_FLAGS = [...new Set([...WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_FALSE_FLAGS, ...P44400_FALSE_FLAGS])];

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewPacketCandidate(options = {}) {
  const result = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewPacketCandidate(options);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Packet Candidate failed with ${result.validation.errors.length} error(s).`);
    error.validation = result.validation;
    throw error;
  }
  if (!options.check && options.write !== false) await writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewPacketCandidate(result, result.output_dir);
  return result;
}

export async function buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewPacketCandidate(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_REVIEW_PACKET_OUT_DIR);
  const inputs = normalizeInputs(options);
  const schema = await readJsonSource(inputs.schema_path);
  const packageJson = await readJsonSource(inputs.package_path);
  const roadmapDoc = await readTextSource(inputs.roadmap_doc_path);
  const architectureDoc = await readTextSource(inputs.architecture_doc_path);
  const source = Object.prototype.hasOwnProperty.call(options, "workOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackage")
    ? normalizeInlineJsonSource("inline.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package", options.workOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackage)
    : await readJsonOrBuildP44400(inputs.source_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_path, generatedAt);
  ...
  const result = {
    ...
    inputs,
    source_refs: {
      work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_path: source.path,
      source_commit_ref: commitRef || null,
    },
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_summary: source.data?.summary ?? null,
    ...
    p44400_source_binding_rows: sourceRows,
    ...
    p44800_clean_checkpoint_rows: checkpointRows,
  ...
  const schemaErrors = schema.available
    ? validateAgainstSchema(result, schema.data, {}, "work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_packet_candidate")
...
export async function writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewPacketCandidate(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-packet-candidate.json"), serializableResult(result));
  await writeJson(path.join(outDir, "p44400-source-binding-rows.json"), collectionEnvelope("p44400-source-binding-rows.v1", "p44400_source_binding_rows", result.p44400_source_binding_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "p44800-clean-checkpoint-rows.json"), collectionEnvelope("p44800-clean-checkpoint-rows.v1", "p44800_clean_checkpoint_rows", result.p44800_clean_checkpoint_rows, result.generated_at));
...
export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewPacketCandidateCli(argv = process.argv.slice(2)) {
  const args = parseArgs(argv);
  if (args.help) {
    printHelp();
    return null;
  }
  const result = await runWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationReviewPacketCandidate(args);
  console.log(`Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Packet Candidate ${args.check ? "validated" : "written"} at ${result.output_dir}`);
  console.log(`Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_status}`);
  console.log(`Program: ${result.program_range}`);
  console.log(`P44400 ready for implementation review packet: ${result.summary.source_p44400_ready_for_implementation_review_packet}`);
...
function buildSourceState(source) {
  ...
  const boundary = source.data?.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_boundary ?? {};
  return {
    available: source.available === true,
    programRangeOk: source.data?.program_range === SOURCE_PROGRAM_RANGE,
    validationValid: source.data?.validation?.valid === true,
    sourceReady: summary.ready_for_p44401_handoff === true,
    status: summary.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_status ?? "missing",
    p44400ContractReady: boundary.p44400_contract_ready === true,
    ...
    boundaryClosed: P44400_FALSE_FLAGS.every((flag) => boundary[flag] === false),
  };
}
...
function buildPhaseRows(roadmapText, generatedAt) {
  return PHASE_SPECS.map(([range, label, outputRef]) => row({
    ...
    evidence_ref: "docs/hermes-roadmap-p44401-p44800.md",
...
function buildSourceRows({ source, sourceState, commitRef, generatedAt }) {
  return [
    ["artifact_available", "P44400 implementation static shell handoff package source is available", sourceState.available],
    ["program_range", "P44400 source program range is P44001-P44400", sourceState.programRangeOk],
    ["validation_valid", "P44400 source validation is valid", sourceState.validationValid],
    ["implementation_review_packet_open", "P44400 opened implementation review packet metadata", sourceState.sourceReady],
    ["p44400_contract_ready", "P44400 source contract is ready", sourceState.p44400ContractReady],
    ["handoff_package_visible", "P44400 implementation static shell handoff package rows are visible", sourceState.handoffPackageVisible],
    ["manifest_visible", "P44400 implementation file manifest rows are visible", sourceState.manifestVisible],
    ["fixture_smoke_visible", "P44400 fixture/smoke plan rows are visible", sourceState.fixtureSmokeVisible],
    ["reviewer_note_visible", "P44400 reviewer handoff note rows are visible", sourceState.reviewerNoteVisible],
    ["no_implementation_closed", "P44400 no-implementation boundary is closed", sourceState.noImplementationClosed],
    ["commit_ref_present", "Current commit ref is present for implementation review packet candidate", Boolean(commitRef)],
    ["source_blocker_visible", "P44400 source blocker is visible when implementation review packet is closed", sourceState.available],
  ].map(([id, label, observed]) => row({
    row_id: `source_binding.${id}`,
    category: "p44400_source_binding",
    label,
    observed,
...
function buildWiringRows({ packageJson, roadmapDoc, architectureDoc, generatedAt }) {
  return [
    ...
    ["validate_chain", "Validate chain includes P44800 check", packageJson.text.includes(`${COMMAND_NAME} -- --check`), "package.json"],
    ["schema_file", "Schema file is configured", packageJson.available, "schemas/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-packet-candidate.schema.json"],
    ["roadmap_doc", "Roadmap documents all P44401-P44800 slices", PHASE_SPECS.every(([range]) => roadmapDoc.text.includes(range)), "docs/hermes-roadmap-p44401-p44800.md"],
    ["architecture_doc", "Architecture doc references P44401-P44800", architectureDoc.text.includes("P44401-P44800 Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Packet Candidate"), "docs/architecture.md"],
...
function buildCheckpointRows(context) {
  ...
  return [
    ["source_ready", "P44400 source is ready for implementation review packet handoff", context.sourceState.sourceReady],
    ...
    ["p44800_review_packet_handoff", "P44800 review packet handoff opens only as metadata", ready],
  ].map(([id, label, observed]) => row({
    row_id: `p44800_checkpoint.${id}`,
    category: "p44800_clean_checkpoint",
    label,
    observed,
    evidence_ref: "p44800_clean_checkpoint_rows",
    generated_at: context.generatedAt,
  }));
}

function buildBoundary(context) {
  const p44800ContractReady = allPass(context.phaseRows)
    ...
    && allPass(context.boundaryRows)
    && allPass(context.wiringRows)
    && visibleOrPassed(context.checkpointRows, "p44800_checkpoint.review_completion_blocked")
    && visibleOrPassed(context.checkpointRows, "p44800_checkpoint.review_receipt_create_blocked")
    && visibleOrPassed(context.checkpointRows, "p44800_checkpoint.file_apply_blocked");
  const ready = context.sourceState.sourceReady
    && context.sourceState.validationValid
    && context.sourceState.boundaryClosed
    && p44800ContractReady;
  return {
    p44800_contract_ready: p44800ContractReady,
    ready_for_work_os_static_bundle_review_ui_implementation_review_request_handoff: ready,
    source_p44400_ready_for_implementation_review_packet: context.sourceState.sourceReady,
...
function buildValidationItems(context) {
  return [
    validationItem("roadmap.phase_coverage", "roadmap", allPass(context.phaseRows), "P44401-P44800 phase rows are incomplete"),
    validationItem("source.visible", "source", visibleOrPassed(context.sourceRows, "source_binding.source_blocker_visible"), "P44400 source state is not visible"),
    ...
    validationItem("wiring.complete", "wiring", allPass(context.wiringRows), "P44401-P44800 wiring is incomplete"),
    ...
    validationItem("checkpoint.review_completion_blocked", "checkpoint", visibleOrPassed(context.checkpointRows, "p44800_checkpoint.review_completion_blocked"), "P44800 review completion blocker checkpoint is not visible"),
    validationItem("checkpoint.review_receipt_create_blocked", "checkpoint", visibleOrPassed(context.checkpointRows, "p44800_checkpoint.review_receipt_create_blocked"), "P44800 review receipt blocker checkpoint is not visible"),
    validationItem("checkpoint.file_apply_blocked", "checkpoint", visibleOrPassed(context.checkpointRows, "p44800_checkpoint.file_apply_blocked"), "P44800 file apply blocker checkpoint is not visible"),
  ];
}

function buildContract(generatedAt) {
  return {
    contract_id: "work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_review_packet_candidate.contract.v1",
    generated_at: generatedAt,
    source_p44400_required_or_rebuilt: true,
    ...
    p44800_is_not_review_receipt_review_completion_finding_resolution_reviewer_dispatch_claude_execution_human_adjudication_file_apply_approval_production_or_enterprise_trust: true,
...
function buildSummary({ boundary, validation }) {
  const status = validation.valid && boundary.ready_for_work_os_static_bundle_review_ui_implementation_review_request_handoff
    ? READY_STATUS
    : validation.valid && boundary.p44800_contract_ready
      ? BLOCK_PENDING_STATUS
      : BLOCKED_STATUS;
  return {
    work_os_static_bundle_review_ui_implementation_review_status: status,
    source_p44400_ready_for_implementation_review_packet: boundary.source_p44400_ready_for_implementation_review_packet,
...
function renderMarkdown(result) {
  return [
    "# Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Packet Candidate",
    "",
    `Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_status}`,
    `Program: ${result.program_range}`,
    `P44400 ready for implementation review packet: ${result.summary.source_p44400_ready_for_implementation_review_packet}`,
...
<head>
  ...
  <title>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Packet Candidate</title>
  <style>
...
<body>
  <main>
    <h1>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Packet Candidate</h1>
...
async function readJsonOrBuildP44400(filePath, generatedAt) {
  const source = await readJsonSource(filePath);
  if (source.available) return source;
  const built = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackage({ runAt: generatedAt, write: false });
  return normalizeInlineJsonSource("built.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package", built);
}
...
function normalizeInputs(options) {
  const defaults = DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_REVIEW_PACKET_INPUTS;
  ...
  return {
    ...
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_path: path.resolve(repoRoot, options.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackagePath ?? defaults.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackagePath),
  };
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--check") {
      ...
      args.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackagePath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-review-packet-candidate.mjs [--check] [--out-dir DIR] [--source FILE] [--commit-ref SHA]

Validates or writes the Hermes P44401-P44800 Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Review Packet Candidate.
`);
}
