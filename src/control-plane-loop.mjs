...
export const DEFAULT_CONTROL_PLANE_LOOP_STEPS = [
  ...
  step("dashboard_before_api_freeze", "Dashboard Before API Freeze", "dashboard", ["node", "scripts/review-dashboard.mjs", "--no-dashboard-api-freeze", "--no-threat-model-refresh", "--no-prompt-injection-test-suite", "--no-external-model-policy-audit", "--no-secrets-scan-gate", "--no-retention-deletion-policy", "--no-access-review-report", "--no-performance-cost-budget-report", "--no-backup-restore-drill", "--no-law-firm-e2e-report", "--no-personal-dev-e2e-report", "--no-creative-document-e2e-report", "--no-control-plane-loop", "--no-control-plane-goal-checkpoint"], ["artifacts/dashboard/latest/review-dashboard.json"]),
