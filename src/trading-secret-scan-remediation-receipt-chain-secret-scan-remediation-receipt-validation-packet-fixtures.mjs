...
import {
  ...
  buildTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptMergePreflightFixtures,
} from "./trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-merge-preflight-fixtures.mjs";

export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_OUT_DIR = "artifacts/trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-fixtures/latest";
export const DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS = {
  ...DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_MERGE_PREFLIGHT_FIXTURES_INPUTS,
  secretScanRemediationReceiptChainSecretScanRemediationReceiptMergePreflightSchemaPath: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_MERGE_PREFLIGHT_FIXTURES_INPUTS.schemaPath,
  schemaPath: "schemas/trading/trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-fixtures.schema.json",
};

const SCHEMA_VERSION = "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-fixtures.v1";
const CAPABILITY_ID = "trading.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_fixtures";
const PHASE_SLOT = "P461";
const PREVIOUS_PHASE_SLOT = "P460";
const NEXT_PHASE_SLOT = "P462";
const READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet";
const SOURCE_READY_STATUS = "ready_for_trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight";
const SOURCE_ROW_READY_STATUS = "ready_for_future_secret_scan_remediation_receipt_validation";
const PACKET_READY_STATUS = "ready_for_future_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet";
const COMMAND_NAME = "trading:secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-fixtures";
...
export async function runTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptValidationPacketFixtures(options = {}) {
  const result = await buildTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptValidationPacketFixtures(options);
  if (options.write !== false) await writeTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptValidationPacketFixtures(result, result.output_dir);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Trading secret scan remediation receipt chain secret scan remediation receipt validation packet fixtures failed with ${result.validation.errors.length} error(s).`);
...
export async function buildTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptValidationPacketFixtures(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_OUT_DIR);
  const inputs = normalizeInputs(options);
  const mergePreflight = await buildTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptMergePreflightFixtures({
    ...options,
    runAt: generatedAt,
    ...
    schemaPath: inputs.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_schema_path,
    write: false,
  });
  ...
  const result = {
    ...
    trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_fixtures_id: `trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-fixtures.${dateStamp(generatedAt)}`,
    capability_id: CAPABILITY_ID,
    output_dir: outputDir,
    inputs,
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_anchor: anchor,
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_rows: packetRows,
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_rows: gateRows,
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_boundary: boundary,
    validation_items: validationItems,
    validation: preliminaryValidation,
    summary,
  };
  const schema = await readJsonSource(inputs.schema_path);
  const schemaErrors = schema.available ? validateAgainstSchema(result, schema.data, {}, "trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_fixtures") : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  ...
  result.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_fixtures_id = result.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_fixtures_id;
  return { ...result, markdown: renderMarkdown(result) };
}

export async function writeTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptValidationPacketFixtures(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-fixtures.json"), serializableResult(result));
  await writeJson(path.join(outDir, "secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-rows.v1", "secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_rows", result.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-gate-rows.json"), collectionEnvelope("trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-gate-rows.v1", "secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_rows", result.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_rows, result.generated_at));
  await writeJson(path.join(outDir, "secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-boundary.json"), result.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_boundary);
  await writeJson(path.join(outDir, "validation-report.json"), {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-fixtures-validation-report.v1",
...
export async function runTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptValidationPacketFixturesCli(argv = process.argv.slice(2)) {
  ...
  try {
    const result = await runTradingSecretScanRemediationReceiptChainSecretScanRemediationReceiptValidationPacketFixtures(args);
    console.log(`Trading secret scan remediation receipt chain secret scan remediation receipt validation packet fixtures ${args.check ? "validated" : "written"} at ${result.output_dir}`);
    console.log(`Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_fixtures_status}`);
    console.log(`Secret scan remediation receipt chain secret scan remediation receipt validation packet rows: ${result.summary.ready_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_row_count}/${result.summary.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_row_count}`);
    console.log(`Secret scan remediation receipt chain secret scan remediation receipt validation packet gates: ${result.summary.ready_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_count}/${result.summary.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_count}`);
...
function buildPacketRows(mergePreflight) {
  const sourceReady = mergePreflight.validation.valid && mergePreflight.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_fixtures_status === SOURCE_READY_STATUS;
  return mergePreflight.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_rows.map((preflightRow, index) => {
    const packetReady = sourceReady && preflightRow.preflight_status === SOURCE_ROW_READY_STATUS;
    const row = {
      schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-row.v1",
      secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_row_id: `trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet.row.${preflightRow.template_key}`,
      phase_slot: PHASE_SLOT,
      source_merge_preflight_row_id: preflightRow.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_row_id,
      ...
      forbidden_receipt_fields_allowed_by_packet: false,
      secret_scan_remediation_action_allowed_by_packet: false,
    ...
    return withOrdinalAndHash(row, index, "secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_hash");
  });
}

function buildPacketCoverage({ mergePreflight, packetRows }) {
  const sourceReady = mergePreflight.validation.valid && mergePreflight.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_fixtures_status === SOURCE_READY_STATUS;
  const preflightRows = mergePreflight.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_rows ?? [];
  const allMergePreflightRowsCovered = preflightRows.length > 0 && preflightRows.every((preflightRow) => packetRows.some((row) => row.source_merge_preflight_row_id === preflightRow.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_row_id));
  ...
  const noSecretMaterialRead = mergePreflight.summary.no_secret_material_read === true
    ...
    && mergePreflight.summary.desktop_provider_key_visible === false
    && packetRows.every((row) => row.secret_values_read_by_packet === false
      ...
      && row.desktop_provider_key_visible_by_packet === false);
  const noSecretOrTradingMutation = mergePreflight.summary.no_secret_or_trading_mutation === true
    && mergePreflight.summary.secret_scan_remediation_action_allowed === false
    && packetRows.every((row) => row.auto_fix_command_registered_by_packet === false
      && row.secret_scan_remediation_action_allowed_by_packet === false
...
function buildPacketAnchor(mergePreflight, coverage) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-fixtures-anchor.v1",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_fixtures_id: mergePreflight.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_fixtures_id,
    source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_status: mergePreflight.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_fixtures_status,
    expected_validation_packet_row_count: coverage.expected_validation_packet_row_count,
    coverage_hash: hashValue(coverage),
    source_hash: hashValue({
      id: mergePreflight.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_fixtures_id,
      status: mergePreflight.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_fixtures_status,
      preflight_rows: mergePreflight.summary.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_row_count,
      gates: mergePreflight.summary.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_gate_count,
    }),
  };
}
...
function buildPacketBoundary({ generatedAt, writeRequested, mergePreflight, packetRows, coverage }) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-fixtures-boundary.v1",
    generated_at: generatedAt,
    boundary_status: "enforced",
    phase_slot: PHASE_SLOT,
    read_only: true,
    report_only: true,
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_artifact_write_requested: writeRequested,
    source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_status: mergePreflight.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_fixtures_status,
    source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_ready: coverage.p460_ready,
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_row_count: packetRows.length,
    ready_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_row_count: packetRows.filter((row) => row.validation_packet_status === PACKET_READY_STATUS).length,
...
function buildPacketGateRows({ mergePreflight, packageJson, platformOpsLedger, boundary, coverage }) {
  ...
  const gateInputs = [
    ["p460_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_ready", "P460 secret scan remediation receipt chain secret scan remediation receipt merge preflight source is ready.", mergePreflight.validation.valid && mergePreflight.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_fixtures_status === SOURCE_READY_STATUS],
    ["platform_package_script_registered", "package.json registers the P461 trading secret scan remediation receipt chain secret scan remediation receipt validation packet fixtures command.", typeof scripts[COMMAND_NAME] === "string" && scripts[COMMAND_NAME].length > 0],
    ["platform_validation_chain_registered", "Validation chain includes the P461 trading secret scan remediation receipt chain secret scan remediation receipt validation packet fixtures command.", validateScript.includes(`npm run ${COMMAND_NAME} -- --check`)],
    ["p461_ledger_acceptance_declared", "P461 acceptance row is declared in the platform operations ledger.", ledgerText.includes("P461: `trading:secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-fixtures`")],
    ["all_merge_preflight_rows_covered", "Every P460 merge preflight row has a matching future validation packet row.", coverage.all_merge_preflight_rows_covered],
    ...
    ["no_secret_material_read", "P461 reads no secret values, env files, Desktop config content, or Desktop provider keys.", boundary.no_secret_material_read && !boundary.secret_values_read && !boundary.env_file_read && !boundary.desktop_config_content_inspected && !boundary.desktop_config_read && !boundary.desktop_provider_key_visible],
    ["no_secret_or_trading_mutation", "P461 performs no remediation action, credential lookup, trading writes, artifact mutation, release, git, or protected action.", boundary.no_secret_or_trading_mutation && !boundary.secret_scan_remediation_action_allowed && !boundary.artifact_write_performed && !boundary.protected_action_executed],
  ];
  return gateInputs.map(([rowKey, description, passed], index) => withOrdinalAndHash(gateRow(rowKey, description, passed), index, "secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_hash"));
}

function gateRow(rowKey, description, passed) {
  return {
    schema_version: "trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-gate-row.v1",
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_row_id: `trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-fixtures.gate.${rowKey}`,
    phase_slot: PHASE_SLOT,
    row_key: rowKey,
    description,
...
function buildValidationItems({ mergePreflight, packageJson, platformOpsLedger, packetRows, gateRows, boundary, coverage }) {
  return [
    validationItem("source.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight", "p460_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_ready", mergePreflight.validation.valid && mergePreflight.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_fixtures_status === SOURCE_READY_STATUS, "P460 secret scan remediation receipt chain secret scan remediation receipt merge preflight fixtures must be ready."),
    validationItem("source.package_json", "package_json_available", packageJson.available, "package.json is readable for P461 secret scan remediation receipt chain secret scan remediation receipt validation packet fixtures."),
    validationItem("source.platform_ops_ledger", "platform_ops_ledger_available", platformOpsLedger.available, "Platform operations stability ledger is readable."),
    validationItem("secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_rows", "validation_packet_rows_ready", packetRows.length === coverage.expected_validation_packet_row_count && packetRows.length === 5 && packetRows.every((row) => row.validation_packet_status === PACKET_READY_STATUS && row.validation_packet_checks.length >= 8 && row.validation_packet_declared), "All secret scan remediation receipt chain secret scan remediation receipt validation packet rows must be ready without receipt payload validation."),
    validationItem("secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_rows", "validation_packet_gates_ready", gateRows.length >= 10 && gateRows.every((row) => row.gate_status === "ready" && !row.receipt_validated_by_packet && !row.protected_action_executed_by_packet), "P461 secret scan remediation receipt chain secret scan remediation receipt validation packet gates are ready."),
    validationItem("coverage.merge_preflight_rows", "all_merge_preflight_rows_covered", coverage.all_merge_preflight_rows_covered, "All P460 merge preflight rows must have validation packet rows."),
    ...
    validationItem("boundary.no_packet_input", "no_packet_input_materialized", boundary.no_packet_input_materialized && !boundary.actor_workspace_input_present && !boundary.actor_workspace_file_read && !boundary.actor_workspace_payload_read && !boundary.receipt_input_file_materialized && !boundary.merged_receipt_input_materialized && !boundary.receipt_payload_present && !boundary.ready_for_validation && !boundary.receipt_validated && !boundary.approval_applied, "P461 must not read actor workspace input or materialize/validate receipt payloads."),
    validationItem("boundary.no_secret_material_read", "no_secret_material_read", boundary.no_secret_material_read && !boundary.secret_values_read && !boundary.env_file_read && !boundary.desktop_config_content_inspected && !boundary.desktop_config_read && !boundary.desktop_provider_key_visible, "P461 reads no secret values, env files, Desktop config content, or Desktop provider keys."),
    validationItem("boundary.no_mutation", "no_secret_or_trading_mutation", boundary.no_secret_or_trading_mutation && !boundary.secret_scan_remediation_action_allowed && !boundary.credential_lookup_allowed && !boundary.broker_write_allowed && !boundary.exchange_write_allowed && !boundary.artifact_write_performed && !boundary.protected_action_executed, "P461 performs no remediation action, credential lookup, trading mutation, artifact mutation, or protected action."),
  ];
}

function buildSummary({ mergePreflight, packetRows, gateRows, boundary, coverage, validation }) {
  return {
    trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_fixtures_status: validation.valid ? READY_STATUS : "blocked",
    phase_slot: PHASE_SLOT,
    previous_phase_slot: PREVIOUS_PHASE_SLOT,
    next_phase_slot: NEXT_PHASE_SLOT,
    source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_status: mergePreflight.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_fixtures_status,
    source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_ready: boundary.source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_ready,
    expected_validation_packet_row_count: coverage.expected_validation_packet_row_count,
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_row_count: packetRows.length,
    ready_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_row_count: boundary.ready_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_row_count,
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_count: gateRows.length,
    ready_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_count: gateRows.filter((row) => row.gate_status === "ready").length,
...
function renderMarkdown(result) {
  const lines = [
    ...
    "",
    `Status: ${result.summary.trading_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_fixtures_status}`,
    `Phase: ${result.summary.phase_slot}`,
    `Source merge preflight: ${result.summary.source_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_status}`,
    `Packet rows: ${result.summary.ready_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_row_count}/${result.summary.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_row_count}`,
    `Packet gates: ${result.summary.ready_secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_count}/${result.summary.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_count}`,
    "",
    "## Rows",
    "",
    ...result.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_rows.map((row) => `- ${row.template_key}: ${row.validation_packet_status}`),
    "",
    "## Gates",
    "",
    ...result.secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_packet_gate_rows.map((row) => `- ${row.row_key}: ${row.gate_status}`),
  ];
  return `${lines.join("\n")}\n`;
}

function parseArgs(argv) {
  const parsed = { outDir: DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_OUT_DIR };
...
function printHelp() {
  console.log(`Usage: node scripts/trading-secret-scan-remediation-receipt-chain-secret-scan-remediation-receipt-validation-packet-fixtures.mjs [options]

Options:
  --out-dir <folder>                                                       Output directory. Default: ${DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_OUT_DIR}
...
function normalizeInputs(options = {}) {
  return {
    package_path: path.resolve(options.packagePath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.packagePath),
    platform_ops_ledger_path: path.resolve(options.platformOpsLedgerPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.platformOpsLedgerPath),
    control_plane_human_gate_receipts_path: path.resolve(options.controlPlaneHumanGateReceiptsPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.controlPlaneHumanGateReceiptsPath),
    secret_scan_remediation_receipt_chain_secret_scan_gate_fixtures_schema_path: path.resolve(options.secretScanRemediationReceiptChainSecretScanGateFixturesSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.secretScanRemediationReceiptChainSecretScanGateFixturesSchemaPath),
    secret_scan_remediation_receipt_chain_secret_scan_attention_schema_path: path.resolve(options.secretScanRemediationReceiptChainSecretScanAttentionSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.secretScanRemediationReceiptChainSecretScanAttentionSchemaPath),
    ...
    secret_scan_remediation_receipt_chain_secret_scan_remediation_schema_path: path.resolve(options.secretScanRemediationReceiptChainSecretScanRemediationSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.secretScanRemediationReceiptChainSecretScanRemediationSchemaPath),
    review_dashboard_path: path.resolve(options.reviewDashboardPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.reviewDashboardPath),
    control_plane_action_plan_path: path.resolve(options.controlPlaneActionPlanPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.controlPlaneActionPlanPath),
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_template_schema_path: path.resolve(options.secretScanRemediationReceiptChainSecretScanRemediationReceiptTemplateSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.secretScanRemediationReceiptChainSecretScanRemediationReceiptTemplateSchemaPath),
    ...
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_validation_rules_schema_path: path.resolve(options.secretScanRemediationReceiptChainSecretScanRemediationReceiptValidationRulesSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.secretScanRemediationReceiptChainSecretScanRemediationReceiptValidationRulesSchemaPath),
    ...
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_workspace_merge_schema_path: path.resolve(options.secretScanRemediationReceiptChainSecretScanRemediationReceiptWorkspaceMergeSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.secretScanRemediationReceiptChainSecretScanRemediationReceiptWorkspaceMergeSchemaPath),
    secret_scan_remediation_receipt_chain_secret_scan_remediation_receipt_merge_preflight_schema_path: path.resolve(options.secretScanRemediationReceiptChainSecretScanRemediationReceiptMergePreflightSchemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.secretScanRemediationReceiptChainSecretScanRemediationReceiptMergePreflightSchemaPath),
    schema_path: path.resolve(options.schemaPath ?? DEFAULT_TRADING_SECRET_SCAN_REMEDIATION_RECEIPT_CHAIN_SECRET_SCAN_REMEDIATION_RECEIPT_VALIDATION_PACKET_FIXTURES_INPUTS.schemaPath),
  };
}
