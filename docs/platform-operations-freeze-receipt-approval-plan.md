...
Status: P496 command specification.

`platform:operations-freeze-receipt-approval-plan` consumes the P495
operations-freeze receipt validation packet in memory and declares
deterministic approval-plan rows for future receipt approval planning.

The approval plan does not read actor workspace files, materialize receipt
...
```sh
npm run platform:operations-freeze-receipt-approval-plan -- --check
```

## Approval Plan Policy

Each approval-plan row is `ready_for_future_receipt_approval_plan`. This means
the source P495 validation packet row is ready and the approval-plan checks are
declared, but no actor workspace input, merged receipt payload, validation
result, approval-application readiness, or applied approval is present.

## Human Review Note

The approval plan is an operator preparation surface only. Later gated
