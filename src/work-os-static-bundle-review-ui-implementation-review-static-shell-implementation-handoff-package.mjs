...
import {
  ALL_FALSE_FLAGS as P44000_FALSE_FLAGS,
  buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationBindingCandidate,
} from "./work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-binding-candidate.mjs";

export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_HANDOFF_PACKAGE_OUT_DIR = "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-handoff-package/latest";
export const DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_HANDOFF_PACKAGE_INPUTS = {
  schemaPath: "schemas/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-handoff-package.schema.json",
  packagePath: "package.json",
  roadmapDocPath: "docs/hermes-roadmap-p44001-p44400.md",
  architectureDocPath: "docs/architecture.md",
  sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationBindingCandidatePath: "artifacts/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-binding-candidate/latest/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-binding-candidate.json",
};

const COMMAND_NAME = "platform:work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-handoff-package";
const SCHEMA_VERSION = "work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-handoff-package.v1";
const CAPABILITY_ID = "platform.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package";
const PROGRAM_RANGE = "P44001-P44400";
const SOURCE_PROGRAM_RANGE = "P43601-P44000";
const READY_STATUS = "ready_for_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package";
const BLOCK_PENDING_STATUS = "valid_block_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_pending";
const BLOCKED_STATUS = "blocked_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package";
...
export const ALL_FALSE_FLAGS = [...new Set([...STATIC_SHELL_IMPLEMENTATION_HANDOFF_PACKAGE_FALSE_FLAGS, ...P44000_FALSE_FLAGS])];

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackage(options = {}) {
  const result = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackage(options);
  if (options.check && !result.validation.valid) {
    const error = new Error(`Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Handoff Package failed with ${result.validation.errors.length} error(s).`);
    error.validation = result.validation;
    throw error;
  }
  if (!options.check && options.write !== false) await writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackage(result, result.output_dir);
  return result;
}

export async function buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackage(options = {}) {
  const generatedAt = new Date(options.runAt ?? new Date()).toISOString();
  const outputDir = path.resolve(options.outDir ?? DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_HANDOFF_PACKAGE_OUT_DIR);
  const inputs = normalizeInputs(options);
  const schema = await readJsonSource(inputs.schema_path);
  const packageJson = await readJsonSource(inputs.package_path);
  const roadmapDoc = await readTextSource(inputs.roadmap_doc_path);
  const architectureDoc = await readTextSource(inputs.architecture_doc_path);
  const source = Object.prototype.hasOwnProperty.call(options, "workOsStaticBundleReviewUiImplementationReviewStaticShellImplementationBindingCandidate")
    ? normalizeInlineJsonSource("inline.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_binding_candidate", options.workOsStaticBundleReviewUiImplementationReviewStaticShellImplementationBindingCandidate)
    : await readJsonOrBuildP44000(inputs.source_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_binding_candidate_path, generatedAt);
  ...
  const result = {
    ...
    inputs,
    source_refs: {
      work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_binding_candidate_path: source.path,
      source_commit_ref: commitRef || null,
    },
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_binding_candidate_summary: source.data?.summary ?? null,
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_contract: buildContract(generatedAt),
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_phase_rows: phaseRows,
    p44000_source_binding_rows: sourceRows,
    ...
    p44400_clean_checkpoint_rows: checkpointRows,
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_boundary: boundary,
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_validation_items: validationItems,
    validation: preliminaryValidation,
    summary: buildSummary({ boundary, validation: preliminaryValidation }),
  };

  const schemaErrors = schema.available
    ? validateAgainstSchema(result, schema.data, {}, "work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package")
    : [{ path: "schema", message: schema.error ?? "Schema unavailable" }];
  const schemaValidationItems = schemaErrors.map((error, index) => validationItem(`schema.${index}`, "schema_validation", false, error.message));
  result.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_validation_items = [...validationItems, ...schemaValidationItems];
  result.validation = summarizeValidation(result.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_validation_items);
  result.summary = buildSummary({ boundary, validation: result.validation });
  return { ...result, markdown: renderMarkdown(result), html: renderHtml(result) };
}

export async function writeWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackage(result, outDir = result.output_dir) {
  await mkdir(outDir, { recursive: true });
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-handoff-package.json"), serializableResult(result));
  await writeJson(path.join(outDir, "p44000-source-binding-rows.json"), collectionEnvelope("p44000-source-binding-rows.v1", "p44000_source_binding_rows", result.p44000_source_binding_rows, result.generated_at));
  ...
  await writeJson(path.join(outDir, "p44400-clean-checkpoint-rows.json"), collectionEnvelope("p44400-clean-checkpoint-rows.v1", "p44400_clean_checkpoint_rows", result.p44400_clean_checkpoint_rows, result.generated_at));
  await writeJson(path.join(outDir, "work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-handoff-package-boundary.json"), result.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_boundary);
  await writeFile(path.join(outDir, "summary.md"), result.markdown, "utf8");
  await writeFile(path.join(outDir, "index.html"), result.html, "utf8");
}

export async function runWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackageCli(argv = process.argv.slice(2)) {
  const args = parseArgs(argv);
  if (args.help) {
    printHelp();
    return null;
  }
  const result = await runWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationHandoffPackage(args);
  console.log(`Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Handoff Package ${args.check ? "validated" : "written"} at ${result.output_dir}`);
  console.log(`Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_status}`);
  console.log(`Program: ${result.program_range}`);
  console.log(`P44000 ready for P44001 handoff: ${result.summary.source_p44000_ready_for_p44001_handoff}`);
  ...
  console.log(`Ready for P44401 handoff: ${result.summary.ready_for_p44401_handoff}`);
...
function buildSourceState(source) {
  ...
  const boundary = source.data?.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_binding_candidate_boundary ?? {};
  return {
    available: source.available === true,
    programRangeOk: source.data?.program_range === SOURCE_PROGRAM_RANGE,
    validationValid: source.data?.validation?.valid === true,
    sourceReady: summary.ready_for_p44001_handoff === true,
    status: summary.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_binding_candidate_status ?? "missing",
    p44000ContractReady: boundary.p44000_contract_ready === true,
    ...
    boundaryClosed: P44000_FALSE_FLAGS.every((flag) => boundary[flag] === false),
  };
}
...
function buildPhaseRows(roadmapText, generatedAt) {
  return PHASE_SPECS.map(([range, label, outputRef]) => row({
    ...
    evidence_ref: "docs/hermes-roadmap-p44001-p44400.md",
...
function buildSourceRows({ source, sourceState, commitRef, generatedAt }) {
  return [
    ["artifact_available", "P44000 implementation review static shell implementation binding candidate source is available", sourceState.available],
    ["program_range", "P44000 source program range is P43601-P44000", sourceState.programRangeOk],
    ["validation_valid", "P44000 source validation is valid", sourceState.validationValid],
    ["p44001_handoff_open", "P44000 source opened P44001 handoff", sourceState.sourceReady],
    ["p44000_contract_ready", "P44000 source contract is ready", sourceState.p44000ContractReady],
    ["implementation_placement_visible", "P44000 implementation placement rows are visible", sourceState.placementVisible],
    ["component_template_visible", "P44000 component/template binding rows are visible", sourceState.componentVisible],
    ["read_only_data_visible", "P44000 read-only data binding rows are visible", sourceState.dataVisible],
    ["visual_token_visible", "P44000 visual token binding rows are visible", sourceState.tokenVisible],
    ["no_authority_closed", "P44000 no-authority boundary is closed", sourceState.noAuthorityClosed],
    ["commit_ref_present", "Current commit ref is present for static shell implementation handoff package", Boolean(commitRef)],
    ["source_blocker_visible", "P44000 source blocker is visible when implementation handoff package is closed", sourceState.available],
  ].map(([id, label, observed]) => row({
    row_id: `source_binding.${id}`,
    category: "p44000_source_binding",
    label,
    observed,
...
function buildFixtureSmokePlanCandidateRows({ source, handoffRows, manifestRows, generatedAt }) {
  ...
  return handoffRows.map((handoff, index) => {
    ...
    return row({
      ...
      smoke_command_hint: `npm run platform:work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-handoff-package -- --check`,
...
function buildNoImplementationBoundaryRows(generatedAt) {
  return ALL_FALSE_FLAGS.map((flag) => row({
    ...
    observed: true,
    evidence_ref: "work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_boundary",
    boundary_flag: flag,
    allowed_now: false,
    generated_at: generatedAt,
  }));
...
function buildCheckpointRows(context) {
  ...
  return [
    ["source_ready", "P44000 source is ready for P44001", context.sourceState.sourceReady],
    ...
    ["p44401_handoff_gate", "P44401 handoff opens only when static shell implementation handoff package conditions pass", handoffReady],
    ["p44401_handoff_blocker_visible", "P44401 handoff blocker is visible when the gate is closed", true],
  ].map(([id, label, observed]) => row({
    row_id: `p44400_checkpoint.${id}`,
    category: "p44400_clean_checkpoint",
    label,
    observed,
    evidence_ref: "p44400_clean_checkpoint_rows",
    generated_at: context.generatedAt,
  }));
}

function buildBoundary(context) {
  const p44400ContractReady = allPass(context.phaseRows)
    ...
    && allPass(context.reviewerRows)
    && allPass(context.boundaryRows)
    && visibleOrPassed(context.checkpointRows, "p44400_checkpoint.p44401_handoff_blocker_visible");
  const handoffReady = context.sourceState.sourceReady
    && context.sourceState.validationValid
    && context.sourceState.boundaryClosed
    && allPass(context.handoffRows)
    ...
    && allPass(context.boundaryRows);
  return {
    p44400_contract_ready: p44400ContractReady,
    ready_for_p44401_handoff: handoffReady,
    source_p44000_ready_for_p44001_handoff: context.sourceState.sourceReady,
...
function buildValidationItems(context) {
  return [
    ...
    validationItem("roadmap.phase_coverage", "roadmap", allPass(context.phaseRows), "P44001-P44400 phase rows are incomplete"),
    validationItem("architecture.reference", "architecture", context.architectureDoc.text.includes("P44001-P44400 Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Handoff Package"), "Architecture doc missing P44001-P44400 reference"),
    validationItem("source.visible", "source", visibleOrPassed(context.sourceRows, "source_binding.source_blocker_visible"), "P44000 source state is not visible"),
    ...
    validationItem("checkpoint.visible", "checkpoint", visibleOrPassed(context.checkpointRows, "p44400_checkpoint.p44401_handoff_blocker_visible"), "P44400 checkpoint blocker is not visible"),
  ];
}

function buildContract(generatedAt) {
  return {
    contract_id: "work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package.contract.v1",
    generated_at: generatedAt,
    source_p44000_required_or_rebuilt: true,
    ...
    p44401_handoff_is_not_file_create_write_apply_component_write_fixture_run_build_route_dom_browser_hydration_action_review_completion_approval_closeout_or_enterprise_trust: true,
    protected_authority: "closed",
  };
}

function buildSummary({ boundary, validation }) {
  const status = validation.valid && boundary.ready_for_p44401_handoff
    ? READY_STATUS
    : validation.valid && boundary.p44400_contract_ready
      ? BLOCK_PENDING_STATUS
      : BLOCKED_STATUS;
  return {
    work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_status: status,
    source_p44000_ready_for_p44001_handoff: boundary.source_p44000_ready_for_p44001_handoff,
    ...
    ready_for_p44401_handoff: validation.valid && boundary.ready_for_p44401_handoff,
...
function renderMarkdown(result) {
  return [
    "# Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Handoff Package",
    "",
    `Status: ${result.summary.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_handoff_package_status}`,
    `Program: ${result.program_range}`,
    `P44000 ready for P44001 handoff: ${result.summary.source_p44000_ready_for_p44001_handoff}`,
    ...
    `Ready for P44401 handoff: ${result.summary.ready_for_p44401_handoff}`,
...
<head>
  ...
  <title>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Handoff Package</title>
  <style>
...
<body>
  <main>
    <h1>Hermes Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Handoff Package</h1>
...
async function readJsonOrBuildP44000(filePath, generatedAt) {
  const source = await readJsonSource(filePath);
  if (source.available) return source;
  const built = await buildWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationBindingCandidate({ runAt: generatedAt, write: false });
  return normalizeInlineJsonSource("built.work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_binding_candidate", built);
}
...
function normalizeInputs(options) {
  const defaults = DEFAULT_WORK_OS_STATIC_BUNDLE_REVIEW_UI_IMPLEMENTATION_REVIEW_STATIC_SHELL_IMPLEMENTATION_HANDOFF_PACKAGE_INPUTS;
  ...
  return {
    ...
    source_work_os_static_bundle_review_ui_implementation_review_static_shell_implementation_binding_candidate_path: path.resolve(repoRoot, options.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationBindingCandidatePath ?? defaults.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationBindingCandidatePath),
  };
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--check") {
      ...
      args.sourceWorkOsStaticBundleReviewUiImplementationReviewStaticShellImplementationBindingCandidatePath = argv[++index];
...
function printHelp() {
  console.log(`Usage: node scripts/work-os-static-bundle-review-ui-implementation-review-static-shell-implementation-handoff-package.mjs [--check] [--out-dir DIR] [--source FILE] [--commit-ref SHA]

Validates or writes the Hermes P44001-P44400 Work OS Static Bundle Review UI Implementation Review Static Shell Implementation Handoff Package.
`);
}
